...
describe('MultiUserService', () => {
  ...
  it('adds user with role', () => { expect(svc.addUser('Bob', 'bob@test.com', 'admin')).toContain('admin'); });
  ...
  it('updates user role', () => { const id = svc.addUser('Dave', 'dave@test.com').match(/user_\w+/)?.[0] || ''; expect(svc.updateUser(id, { role: 'admin' })).toContain('updated'); });
  it('checks permissions', () => { const id = svc.addUser('Eve', 'eve@test.com', 'admin').match(/user_\w+/)?.[0] || ''; expect(svc.hasPermission(id, 'read')).toBe(true); });
  it('gets user', () => { const id = svc.addUser('Frank', 'frank@test.com').match(/user_\w+/)?.[0] || ''; expect(svc.getUser(id)).toBeTruthy(); });
  it('returns null for unknown user', () => { expect(svc.getUser('unknown')).toBeNull(); });
  it('gets stats', () => { svc.addUser('Grace', 'grace@test.com'); expect(svc.getStats()).toContain('Total users: 1'); });
  it('lists when empty', () => { expect(svc.listUsers()).toContain('No users'); });
});
...
describe('SharedWorkspaceService', () => {
  ...
  it('gets workspace', () => { const id = svc.createWorkspace('Test', 'desc', 'user1').match(/ws_\w+/)?.[0] || ''; expect(svc.getWorkspace(id)).toBeTruthy(); });
  ...
  it('returns null for unknown workspace', () => { expect(svc.getWorkspace('unknown')).toBeNull(); });
  it('gets stats', () => { svc.createWorkspace('Test', 'desc', 'user1'); expect(svc.getStats()).toContain('Workspaces: 1'); });
  it('lists when empty', () => { expect(svc.listWorkspaces()).toContain('No shared'); });
});
...
describe('RoleBasedAccessService', () => {
  ...
  it('gets role', () => { expect(svc.getRole('owner')).toBeTruthy(); });
  it('returns null for unknown role', () => { expect(svc.getRole('unknown')).toBeNull(); });
  it('lists policies', () => { svc.addPolicy('test', 'read', ['viewer']); expect(svc.listPolicies()).toContain('test'); });
