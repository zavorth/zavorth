...
describe('Native Zavorth Enhancements Suite', () => {
  describe('ZavorthDenseContextEncoderService', () => {
    ...
    it('rejects input that would be silently truncated', async () => {
      const encoder = new ZavorthDenseContextEncoderService({ maxCanvasWidth: 80, maxCanvasHeight: 16 });
      await expect(encoder.encodeTextToBitmap('a'.repeat(11))).rejects.toThrow(/does not fit/i);
    });
  });
  ...
  describe('ZavorthDeterministicEventLedger', () => {
    it('records events in a tamper-proof hash chain', () => {
      const ledger = new ZavorthDeterministicEventLedger(undefined, `unit_test_session_${Date.now()}`);
