...
import { UniversalSkillApprovedDashboardCanaryService } from '../../src/services/UniversalSkillApprovedDashboardCanaryService.js';
...
describe('UniversalSkillApprovedDashboardCanaryService Phase 10', () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-approved-dashboard-canary-'));
  ...
  it('builds the approved dashboard endpoint model without mutating visual files', async () => {
    writeSkills(source, 4);

    const snapshot = await new UniversalSkillApprovedDashboardCanaryService({
    ...
    expect(snapshot.dashboardImplementation.endpoint).toBe('/api/skills/scale-hardening');
    expect(snapshot.dashboardImplementation.cards.length).toBeGreaterThanOrEqual(5);
    expect(snapshot.dashboardImplementation.table.rows).toHaveLength(2);
    expect(snapshot.dashboardImplementation.filters.length).toBeGreaterThanOrEqual(3);
    expect(snapshot.dashboardImplementation.visualFilesChanged).toBe(false);
    expect(snapshot.dashboardImplementation.layoutMutationPerformed).toBe(false);
  ...
  it('keeps preview coverage limits visible without downgrading a clean dry-run canary', async () => {
    ...
    expect(snapshot.dashboardImplementation.cards).toContainEqual(expect.objectContaining({
  ...
  it('propagates blocked scale gates into dashboard and canary readiness', async () => {
    ...
    expect(snapshot.rollout.readyForDashboardUse).toBe(false);
  });
});
