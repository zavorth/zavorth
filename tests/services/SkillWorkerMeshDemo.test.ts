/**
 * W8 — hermetic demo journeys J1 (skill) + J2 (worker).
...
describe('W8 SkillWorkerMeshDemo J1+J2', () => {
  let tempRoot: string;
  ...
  it('J2: list workers → route → dry-run invoke + untrusted merge', async () => {
    ...
    const workerRoute = router.route({
      task: 'Delegate research to internal:researcher about architecture',
      workers: mesh.listWorkers(),
    });
