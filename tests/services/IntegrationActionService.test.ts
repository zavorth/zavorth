    ...
    const sidecarStatusService = {
      readSummary: () => ({
        AIGateway: {
          ...
          message: ready ? 'AIGateway iniciado pelo teste.' : 'Sidecar ainda not esta ready.',
        },
  ...
  it('offers and executes AIGateway upstream sync/promote/rollback recipes through the hub', async () => {
    ...
    const GatewayUpstreamSyncService = {
      ...
      rollback: jest.fn(async () => ({
        ok: true,
        action: 'rollback',
        summary: 'AIGateway restored to the previous lock and revalidated.',
        status: 'rolled_back',
  ...
  it('offers and executes ZavorthBridge Remote start and upstream recipes through the hub', async () => {
    ...
    const sidecarStatusService = {
      readSummary: () => ({
        ...
        ZavorthTerminal: {
          ...
          checkedAt: '2026-04-01T15:00:00.000Z',
          message: ready ? 'ZavorthBridge Remote iniciado.' : 'Sidecar ainda not esta ready.',
        },
      }),
    } as any;
    const probe = new IntegrationProbeService({
    ...
    const upstreamSyncService = {
      ...
      promote: jest.fn(async () => ({
        ok: true,
        action: 'promote',
        summary: 'Upstream ZavorthBridge Remote promovido com doctor revalidated.',
        status: 'promoted',
      ...
      rollback: jest.fn(async () => ({
        ok: true,
        action: 'rollback',
        summary: 'ZavorthBridge Remote restored to the previous lock and revalidated.',
        status: 'rolled_back',
