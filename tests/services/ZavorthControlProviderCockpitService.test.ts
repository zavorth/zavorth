...
describe('ZavorthControlProviderCockpitService', () => {
  it('projects provider live matrix into a read-only ZavorthControl cockpit contract', async () => {
    ...
    expect(projection).toEqual(expect.objectContaining({
      ...
      selectedProviderId: 'openai',
      zavorthControlProjection: expect.objectContaining({
        route: '/control',
  ...
  it('keeps blocked provider actions as guidance instead of zavorthControl execution authority', async () => {
    const service = new ZavorthControlProviderCockpitService({
      now: () => new Date('2026-05-13T12:00:00.000Z'),
      providerReadiness: {
    ...
    expect(projection.actions.every((action) => action.zavorthControlCanExecute === false)).toBe(true);
  });
});
