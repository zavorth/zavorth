...
describe('PluginRouterService', () => {
  ...
  it('scores web-search highest for search the web intent', async () => {
  ...
  it('accepts explicit candidates without filesystem', async () => {
    const service = new PluginRouterService();
    const result = await service.recommend({
      root: process.cwd(),
      intent: 'github pull requests',
      candidates: [
        { pluginId: 'github', summary: 'PRs and issues', tags: ['github', 'pr'], capabilities: [{ id: 'github.pr.list' }] },
