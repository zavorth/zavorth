...
describe('ZavorthControlExperienceHomeService', () => {
  it('builds a simple zavorthControl home without execution authority', () => {
    const snapshot = new ZavorthControlExperienceHomeService({
      now: () => new Date('2026-05-15T12:00:00.000Z'),
    }).buildSnapshot();

    expect(snapshot.surface).toBe('zavorthControl-experience-home');
    expect(snapshot.route).toBe('/control');
    ...
    expect(snapshot.safety.zavorthControlCanExecuteTargetAction).toBe(false);
    expect(snapshot.safety.policyBrokerRequiredForActions).toBe(true);
  });

  it('keeps mission starters as prompts, not hidden actions', () => {
    const snapshot = new ZavorthControlExperienceHomeService().buildSnapshot();
