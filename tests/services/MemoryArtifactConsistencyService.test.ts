...
describe('MemoryArtifactConsistencyService Surface controls', () => {
  it('builds Memory/Artifact consistency over the current Zavorth memory plane without writes', () => {
    const snapshot = new MemoryArtifactConsistencyService({
  ...
  it('maps the private memory extension inventory into Zavorth-native surfaces', () => {
    const snapshot = new MemoryArtifactConsistencyService().buildSnapshot();
  ...
  it('runs dry proof through Artifact Memory, Memory With Receipts, and Run Artifact Replay', () => {
    const proof = new MemoryArtifactConsistencyService({
  ...
  it('keeps partial import/export memory support visible instead of overstating consistency', () => {
    const service = new MemoryArtifactConsistencyService({
      files: {
  ...
  it('emits a Memory/Artifact Plugin OS manifest that can be installed and invoked as a plan', async () => {
    const manifest = new MemoryArtifactConsistencyService({
