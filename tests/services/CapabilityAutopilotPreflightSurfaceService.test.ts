...
const fallbackHint: CapabilityPreflightHintResult = {
  ...
  headline: 'I found a similar case that used the fallback selected by the user.',
  userSummary: 'Procedural memory found. I can show this alternative, but I will not switch executor unless you choose it.',
...
const readyHint: CapabilityPreflightHintResult = {
  ...
  headline: 'I have seen a similar case that ended ready to resume.',
  userSummary: 'Vou tratar isso como dica de preflight, not como ordem automatica.',
...
const noHint: CapabilityPreflightHintResult = {
  ...
  userSummary: 'Gemini CLI ainda not esta ready. No safe procedural memory found for this context.',
  recommendedNextAction: null,
  shouldAskPermission: false,
  requiresExplicitUserChoice: false,
};
...
describe('CapabilityAutopilotPreflightSurfaceService', () => {
  it('builds CLI fallback payload with explicit-only actions', () => {
    ...
    expect(payload).toMatchObject({
      ...
      shouldRunAutomatically: false,
      metadata: {
        stage: 'capability-autopilot-preflight-surface',
        autoExecute: false,
      },
    });
  ...
  it('keeps API and web payloads aligned while using their own routes', () => {
    ...
    expect(web).toMatchObject({
      surface: 'web',
      audience: 'technical_operator',
      headline: 'I have seen a similar case that ended ready to resume. (hint_available/ready)',
      technicalBody: readyHint.technicalSummary,
      status: 'hint_available',
      hintKind: 'ready',
    });
