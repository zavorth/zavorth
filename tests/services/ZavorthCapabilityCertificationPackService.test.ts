...
describe('ZavorthCapabilityCertificationPackService', () => {
  it('builds a complete certification snapshot across providers, channels, backends, skills and dashboard', async () => {
    const snapshot = await new ZavorthCapabilityCertificationPackService({
      now: () => new Date('2026-05-18T12:00:00.000Z'),
    }).buildSnapshot();

    expect(snapshot.contractVersion).toBe('zavorth-capability-certification-pack/1');
    expect(snapshot.surface).toBe('capability-certification-pack');
    expect(snapshot.providerCertification.missingRoutes).toEqual([]);
    expect(snapshot.providerCertification.routeCount).toBeGreaterThanOrEqual(80);
    ...
    const service = new ZavorthCapabilityCertificationPackService({
      ...
      maturity: {
        buildSnapshot: () => ({
          ...
          distinctions: { identityScanClean: true },
        }),
      } as any,
