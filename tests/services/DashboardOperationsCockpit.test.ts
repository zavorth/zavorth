...
describe('DashboardService operations cockpit', () => {
  const logRepo = createTestLogRepo();

  it('serves the cockpit snapshot endpoint and references it from the dashboard html', async () => {
    ...
    const [pageResponse, { status: cockpitStatus, payload: cockpit }] = await Promise.all([
      ...
      fetchDashboardJson(baseUrl, '/api/operations/cockpit'),
    ]);
