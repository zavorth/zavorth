...
const permissionReceipt: CapabilityReceipt = {
  ...
  headline: 'Gemini CLI needs da sua permission antes de eu mexer nisso.',
  userSummary: 'Preciso de permission antes de qualquer action. Tambem tenho fallbacks visiveis.',
...
describe('CapabilityAutopilotSurfaceUxService', () => {
  it('builds a CLI payload with explicit approval, validation and fallback actions', () => {
    ...
    expect(payload).toMatchObject({
      ...
      stage: 'permission',
      tone: 'attention',
      permissionSummary: '1 permission(oes) pendente(s) com escopo explicito.',
      fallbackSummary: '2 fallback(s) available, none automatic.',
      metadata: {
