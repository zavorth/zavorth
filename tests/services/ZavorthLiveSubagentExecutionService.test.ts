...
describe('ZavorthLiveSubagentExecutionService governed tools', () => {
  it('allows read-only workspace tools through Policy Broker and feeds results back to the worker', async () => {
    ...
    const result = await service.executeTeam({
      ...
      runId: 'run-1',
      sessionId: 'session-1',
      task: 'use delegated review e leia o file README.md do projeto',
      mode: 'oneshot',
      channel: 'cli',
      actorId: 'user-1',
  ...
  it('denies mutating tools even when a provider asks for them', async () => {
    ...
    const result = await service.executeTeam({
      ...
      runId: 'run-1',
      sessionId: 'session-1',
      task: 'use delegated review e analise files do projeto',
      mode: 'oneshot',
      channel: 'cli',
      actorId: 'user-1',
      profiles: [profile('planner')],
  ...
  it('allows public web_search but blocks sensitive network targets', async () => {
    ...
    const result = await service.executeTeam({
      ...
      runId: 'run-1',
      sessionId: 'session-1',
      task: 'use delegated review e research public sources about AI security',
      mode: 'oneshot',
      channel: 'cli',
      actorId: 'user-1',
