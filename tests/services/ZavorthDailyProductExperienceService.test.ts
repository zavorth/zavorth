...
describe('ZavorthDailyProductExperienceService', () => {
  ...
  it('maps blocked, attention and ready states from setup and capability sources', async () => {
    ...
    const attentionSetup = setupSnapshot({
      status: 'attention',
    ...
    const attention = await new ZavorthDailyProductExperienceService({
      now,
      setupChecklist: { buildSnapshot: () => attentionSetup },
      capabilityFlow: { buildSnapshot: async () => capabilitySnapshot({ status: 'attention' }) },
    }).buildSnapshot();

    const readySetup = setupSnapshot({
      status: 'ready',
      summary: { total: 7, done: 7, next: 0, needsSetup: 0, blocked: 0 },
    });
    readySetup.items = readySetup.items.map((item) => ({ ...item, status: 'done' }));
    const ready = await new ZavorthDailyProductExperienceService({
      now,
      setupChecklist: { buildSnapshot: () => readySetup },
      capabilityFlow: { buildSnapshot: async () => capabilitySnapshot({ status: 'ready', continuousEvals: { title: 'Rodar avaliactions', status: 'ready', commands: [], summary: 'All checks passed.' } }) },
    }).buildSnapshot();

    expect(blocked.status).toBe('blocked');
    expect(attention.status).toBe('attention');
    expect(attention.chatReady).toBe(true);
    expect(ready.status).toBe('ready');
    expect(ready.chatReady).toBe(true);
    expect(ready.platformSetupComplete).toBe(true);
  });
