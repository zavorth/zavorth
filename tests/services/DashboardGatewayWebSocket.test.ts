...
async function waitForDashboardSocketMessage(
  ...
  throw new Error('Mensagem esperada nao chegou a tempo na dashboard WebSocket.');
}

describe('DashboardService gateway websocket', () => {
  ...
  it('upgrades the dashboard to the canonical gateway websocket control plane', async () => {
    config.zavorthWebAuthToken = 'dashboard-gateway-secret';
    ...
    try {
      service.attachChatRuntime({
        ...
        webUserId: 'dashboard-web-user',
      } as any);
      await service.start();
      const query = new URLSearchParams({
        token: 'dashboard-gateway-secret',
        sessionId: 'dashboard-ws-1',
        replay: 'state',
      });
      ...
      const ready = await waitForDashboardSocketMessage(messages, (payload) => payload.type === 'ready');
      const runtime = await waitForDashboardSocketMessage(messages, (payload) => payload.type === 'runtime');
      const hydrate = await waitForDashboardSocketMessage(messages, (payload) => payload.type === 'hydrate');
      ...
      const stateResponse = await waitForDashboardSocketMessage(
        messages,
