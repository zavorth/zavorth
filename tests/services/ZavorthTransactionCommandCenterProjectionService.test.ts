...
describe('ZavorthTransactionCommandCenterProjectionService', () => {
  let tempDir: string;
  let service: ZavorthTransactionCommandCenterProjectionService;
  let credentialRefs: ZavorthTransactionCredentialRefService;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-transaction-command-center-test-'));
    ...
    service = new ZavorthTransactionCommandCenterProjectionService({
      now: () => now,
