...
import {
  DashboardLegacyRouteService,
  type DashboardLegacyRouteDeps,
} from '../../src/services/DashboardLegacyRouteService.js';

describe('Dashboard skill catalog routes', () => {
  function createDeps(overrides: Partial<DashboardLegacyRouteDeps> = {}): DashboardLegacyRouteDeps {
    return {
      host: '127.0.0.1',
      port: 33333,
      snippetUserId: '1',
      getPublicBaseUrl: jest.fn(() => 'http://127.0.0.1:33333'),
      getClassicDashboardHtml: jest.fn(() => '<html></html>'),
      getStats: jest.fn(() => ({})),
      getSidecars: jest.fn(() => ({})),
  ...
  it('serves the shared skill catalog in the legacy dashboard API', async () => {
    const routeService = new DashboardLegacyRouteService();
  ...
  it('serves the MCP sidecar snapshot for skills in the legacy dashboard API', async () => {
    const routeService = new DashboardLegacyRouteService();
    const res = {} as http.ServerResponse;
    const req = { method: 'GET' } as http.IncomingMessage;
    const writeJson = jest.fn();
