import type { SatelliteAppParitySurface } from '../../src/contracts/SatelliteAppParityContract.js';
...
const entry = (service: SatelliteAppParityService, surface: SatelliteAppParitySurface) =>
...
describe('SatelliteAppParityService Phase 6', () => {
  ...
  it('keeps browser API gaps visible when a Satellite template only declares device features', () => {
    const partial = new SatelliteAppParityService({
      files: {
  ...
  it('emits a Satellite Plugin OS manifest that can be installed and invoked as a plan', async () => {
    const manifest = new SatelliteAppParityService({
