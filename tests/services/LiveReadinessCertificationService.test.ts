...
import {
  ZAVORTH_LIVE_READINESS_CERTIFICATION_CONTRACT_VERSION,
} from '../../src/contracts/LiveReadinessCertificationContract.js';

describe('LiveReadinessCertificationService', () => {
  const now = new Date('2026-05-05T15:00:00.000Z');
  const build = () => new LiveReadinessCertificationService({ now: () => now });
  ...
  it('builds a certified staging-live snapshot for the tracked Source surface', () => {
    ...
    expect(snapshot.summary.acceptedSourceModules).toBe(snapshot.summary.sourceModules);
    expect(snapshot.summary.providers).toBeGreaterThanOrEqual(47);
    ...
    expect(snapshot.summary.stagingLiveSmokeCommands).toBeGreaterThanOrEqual(snapshot.summary.sourceModules);
  ...
  it('aggregates all live activation phase reports and check commands', () => {
    ...
    expect(snapshot.phases.reduce((sum, phase) => sum + phase.stagingLiveSmokeCommands, 0)).toBe(snapshot.summary.stagingLiveSmokeCommands);
  });
  ...
  it('formats a concise operator-readable certification report', () => {
    ...
    expect(report).toMatch(/Tracked modules: \d+\/\d+/);
    expect(report).toMatch(/Staging-live smoke commands: \d+/);
