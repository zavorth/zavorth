...
describe('Dashboard gateway endpoints', () => {
  ...
  it('serves unified gateway snapshots through operations and web endpoints', async () => {
    ...
    const { payload: operationsPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/operations/gateway',
    );
    const { payload: webPayload } = await fetchDashboardJson(baseUrl, '/api/web/gateway', {
      token: 'gateway-secret',
    });
    const { payload: operationsControlPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/operations/control-plane',
    );
    const { payload: operationsDomainPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/operations/gateway/domains',
    );
    const { payload: operationsDomainFullPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/operations/gateway/domains?detail=full',
    );
    const { payload: webControlPayload } = await fetchDashboardJson(baseUrl, '/api/web/control-plane', {
      token: 'gateway-secret',
    });
