...
describe('ZavorthLlmRuntimeService — Comprehensive', () => {
  it('should create with no args', () => {
    ...
    expect(typeof svc.getPreferredProviderName()).toBe('string');
    expect(svc.getPreferredProviderName().length).toBeGreaterThan(0);
  });
  ...
  it('should report availability for known providers', () => {
    ...
    expect(typeof svc.isProviderAvailable('nonexistent-provider-xyz')).toBe('boolean');
  });

  it('should return false for nonexistent provider', () => {
    const svc = new ZavorthLlmRuntimeService();
    expect(svc.isProviderAvailable('nonexistent-provider-xyz-12345')).toBe(false);
  });
...
describe('UserModelDialecticReasoningService — Comprehensive', () => {
  ...
  it('should persist and load synthesis', async () => {
    ...
    await svc.synthesize([
      { user: 'Trabalho com Rust e Go', assistant: 'Legal!' },
    ]);
