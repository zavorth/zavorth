...
describe('ZavorthTransactionDashboardProjectionService', () => {
  let tempDir: string;
  let service: ZavorthTransactionDashboardProjectionService;
  let credentialRefs: ZavorthTransactionCredentialRefService;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-transaction-dashboard-test-'));
    ...
    service = new ZavorthTransactionDashboardProjectionService({
      now: () => now,
  ...
  it('projects a web trade into cockpit lanes, tiles and approval actions', () => {
    ...
    expect(projection.version).toBe('zavorth-transaction-dashboard/checkpoint-8');
