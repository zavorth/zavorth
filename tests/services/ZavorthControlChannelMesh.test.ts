...
describe('ZavorthControl channel mesh', () => {
  ...
  it('serves the channel mesh through the classic zavorthControl api', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-mesh-'));
    tempDirs.push(root);
  ...
  it('executes channel actions through the classic zavorthControl api', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-actions-'));
    tempDirs.push(root);
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('late-binds Discord broadcast gateways into the classic zavorthControl action plane', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-discord-'));
    tempDirs.push(root);
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('late-binds Slack broadcast gateways into the classic zavorthControl action plane', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-slack-'));
    tempDirs.push(root);
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('reflects web sessionSend and sessionSpawn when the runtime is attached', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-web-runtime-'));
    tempDirs.push(root);
  ...
  it('reflects discord runtime descriptors from a live broadcast gateway', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-discord-runtime-'));
    tempDirs.push(root);
    config.zavorthWebAuthToken = 'web-secret';
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('reflects telegram runtime adapters from a live broadcast gateway', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-telegram-runtime-'));
    tempDirs.push(root);
    config.zavorthWebAuthToken = 'web-secret';
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('reflects slack runtime adapters from a live broadcast gateway', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-slack-runtime-'));
    tempDirs.push(root);
    config.zavorthWebAuthToken = 'web-secret';
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
  ...
  it('reflects whatsapp runtime adapters from a live broadcast gateway', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-channel-whatsapp-runtime-'));
    tempDirs.push(root);
    config.zavorthWebAuthToken = 'web-secret';
    config.zavorthControlRuntimeStateFile = path.join(root, 'zavorthControl-runtime.json');
