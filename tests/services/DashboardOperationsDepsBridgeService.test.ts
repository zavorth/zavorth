import { DashboardOperationsDepsBridgeService } from '../../src/domain/surface/presentation/dashboard/DashboardOperationsDepsBridgeService.js';

describe('DashboardOperationsDepsBridgeService', () => {
  it('builds operational route deps, snapshot deps, and overview deps from a shared source', async () => {
    const bridge = new DashboardOperationsDepsBridgeService();
    const source = {
      continuityUserId: '42',
      authService: { validate: jest.fn() },
      classicAccess: {
        isLoopbackAddress: jest.fn(),
        resolveDashboardToken: jest.fn(),
      },
