...
describe('ZavorthControlAccessService', () => {
  it('opens the ZavorthControl with a fragment token without exposing query-token URLs', async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-access-'));
    const tokenFile = path.join(tmp, 'web-api-token.txt');
    const spawn = jest.fn(() => ({ unref: jest.fn() })) as any;

    const service = new ZavorthControlAccessService({
      env: {},
      spawn,
      config: {
        host: '0.0.0.0',
    ...
    expect(snapshot.publicUrl).toBe('http://127.0.0.1:3000/control');
    expect(snapshot.url).toContain('http://127.0.0.1:3000/control#token=');
  ...
  it('prefers ZAVORTH_WEB_AUTH_TOKEN and only prints token when explicitly requested', async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-env-'));
    const service = new ZavorthControlAccessService({
    ...
    expect(snapshot.url).toBe('http://127.0.0.1:3100/control#token=env-token-for-test');
  });

  it('ignores placeholder env tokens and generates a strong runtime token instead', async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-weak-env-'));
    const tokenFile = path.join(tmp, 'web-api-token.txt');
    const service = new ZavorthControlAccessService({
  ...
  it('parses human zavorthControl actions', () => {
    ...
    expect(parseZavorthControlAccessAction('repair')).toBe('repair');
    expect(parseZavorthControlAccessAction('generate-token')).toBe('generate-token');
  });

  it('diagnoses and repairs a missing runtime-file token without exposing it by default', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-repair-'));
  ...
  it('does not rotate runtime-file token when env token is active', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-control-env-locked-'));
    const tokenFile = path.join(tmp, 'web-api-token.txt');
