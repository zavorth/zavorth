...
describe('Dashboard plugin plane actions', () => {
  ...
  it('serves plugin plane snapshots and action results through the classic dashboard api', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-dashboard-plugin-plane-'));
    tempDirs.push(root);
