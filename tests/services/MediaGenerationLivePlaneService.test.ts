...
describe('MediaGenerationLivePlaneService Phase 6', () => {
  ...
  it('closes Phase 6 media generation gates without live IO', () => {
    const snapshot = new MediaGenerationLivePlaneService({
      now: () => new Date('2026-05-04T23:00:00.000Z'),
    }).buildSnapshot();

    expect(snapshot.contractVersion).toBe('2026-05-04.live-phase-6');
    expect(snapshot.phase).toBe('Phase 6 - Media Generation Live Plane');
  ...
  it('stores direct image output as a GeneratedMediaArtifact', async () => {
    ...
    expect(await fs.promises.readFile(result.artifacts[0].storageRef, 'utf8')).toBe('phase-6-image');
  });

  it('stores async video output as a GeneratedMediaArtifact', async () => {
    const videoBytes = Buffer.from('phase-6-video');
    const calls: string[] = [];
    ...
    expect(await fs.promises.readFile(result.artifacts[0].storageRef, 'utf8')).toBe('phase-6-video');
  });
