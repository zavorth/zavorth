import { DashboardPresentationDepsBridgeService } from '../../src/domain/surface/presentation/dashboard/DashboardPresentationDepsBridgeService.js';

describe('DashboardPresentationDepsBridgeService', () => {
  it('builds classic, core, legacy, and CORS deps from a shared presentation source', () => {
    const bridge = new DashboardPresentationDepsBridgeService();
    const source = {
    ...
    expect(legacy.getClassicDashboardHtml()).toBe('<html></html>');
    expect(source.getClassicDashboardHtml).toHaveBeenCalled();
