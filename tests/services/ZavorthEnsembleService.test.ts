...
import { ZavorthEnsembleService as ExperimentalZavorthEnsembleService } from '@zavorth/agents/ZavorthEnsembleService.js';
...
describe('ExperimentalZavorthEnsembleService', () => {
  it('launches and tracks an experimental swarm until completion', async () => {
    const service = new ExperimentalZavorthEnsembleService({
  ...
  it('persists custom role library entries for future swarms', async () => {
    ...
    try {
      ...
      const role = service.upsertRoleLibraryEntry({
        ...
        label: 'Docs Auditor',
        kind: 'researcher',
        systemPrompt: 'Leia documentaction em modo seguro e aponte lacunas.',
        scope: 'read_only',
        risk: 'safe',
      });
  ...
  it('uses LLM synthesis when a runtime is attached and keeps deterministic fallback available', async () => {
    ...
    try {
      ...
      expect(llmRuntime.chat).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          allowFallback: true,
          telemetry: expect.objectContaining({ surface: 'zavorth-ensemble-official' }),
        }),
      );
    } finally {
      await fs.promises.rm(tempDir, { recursive: true, force: true });
  ...
  it('wraps command roles with a Docker runner when strong Docker isolation is selected', async () => {
    ...
    const launched = service.launchOfficialSwarm({
      objective: 'Executar mutaction isolada',
      roles: [{
        id: 'docker-worker',
        label: 'Docker Worker',
