/**
 * ExtensionLeaseBetaStabilityRcSafety.test.ts
 *
 * Verifies critical safety boundaries during stability RC evaluation.
 */
...
const CORR = 'corr-beta-stability-rc-safety';

describe('ExtensionLeaseBetaStabilityRcSafety', () => {
  ...
  test('critical remains impossible and high risk remains disabled by default', () => {
    ...
    // Validate critical fails validation
    expect(() => {
  ...
  test('incomplete gate receipt, revoked/expired lease, and mismatched parameters fail closed', async () => {
    ...
    const resultDrift = adapter.evaluate({
      ...
      channelId: 'Slack',
      toolQualifiedName: reg.qualifiedName,
      toolFingerprint: 'drifted-fingerprint',
      riskClass: 'safe',
      requestedOperation: 'execute',
      auditCorrelationId: CORR,
      existingGateResult: validReceipt,
    ...
    // 8. Handler is not executed during RC checks
