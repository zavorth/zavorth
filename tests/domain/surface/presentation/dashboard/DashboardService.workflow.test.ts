...
describe('DashboardService', () => {
  ...
  it('serves web assets, protects the web api, and exposes preview and SSE endpoints', async () => {
    ...
    const { response: stateResponse, payload: statePayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/state?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
    const { response: hostStatusResponse, payload: hostStatusPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/status?detail=full',
      { token },
    );
    const { response: officialRemoteAccessResponse, payload: officialRemoteAccessPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/official-remote-access',
      { token },
    );
    const { response: remoteAccessResponse, payload: remoteAccessPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/remote-access',
      { token },
    );
    const { response: catalogResponse, payload: catalogPayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/catalog?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
    const { response: previewResponse, payload: previewPayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/file-preview?path=${encodeURIComponent(packagePreviewPath)}`,
      { token },
    );
    const { response: blockedPreviewResponse, payload: blockedPreviewPayload } = await fetchDashboardJson(
      baseUrl,
  ...
  it('exposes telegram continuity context through the web state endpoint', async () => {
    config.zavorthWebAuthToken = 'test-web-token';
    const workflowRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-dashboard-workflows-'));
    ...
    const { status: stateStatus, payload: statePayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/state?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
