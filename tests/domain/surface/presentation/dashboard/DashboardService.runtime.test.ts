...
describe('DashboardService', () => {
  ...
  it('serves the authenticated web app and routes a web chat message through the orchestrator', async () => {
    ...
    const { status: continuityStatus, payload: continuityPayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/session/continuity?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
    const { status: briefStatus, payload: briefPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/operations/brief',
      { token },
    );
    const { status: capabilitiesStatus, payload: capabilitiesPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/capabilities',
      { token },
    );
    const { status: observabilityStatus, payload: observabilityPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/operations/product-observability',
      { token },
    );
    const { status: accessManifestStatus, payload: accessManifestPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/access-manifest',
      { token },
    );
    const { status: installJourneyStatus, payload: installJourneyPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/install-journey',
      { token },
    );
    const { status: remoteAccessStatus, payload: remoteAccessPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/remote-access',
      { token },
    );
    const { status: officialRemoteAccessStatus, payload: officialRemoteAccessPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/host/official-remote-access',
      { token },
    );
    const { status: surfaceConsistencyStatus, payload: surfaceConsistencyPayload } = await fetchDashboardJson(
      baseUrl,
