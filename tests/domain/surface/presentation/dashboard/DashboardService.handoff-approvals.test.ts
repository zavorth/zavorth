...
describe('DashboardService', () => {
  ...
  it('serves the runtime handoff shell while preserving SDD workflow catalog and teams data', async () => {
    ...
    const workflowRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-dashboard-sdd-workflow-'));
    ...
    const { status: catalogStatus, payload: catalogPayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/catalog?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
    const { status: teamsStatus, payload: teamsPayload } = await fetchDashboardJson(
      baseUrl,
      '/api/web/teams',
      { token },
    );
  ...
  it('processes live task approvals and rejections through the web app endpoints', async () => {
    ...
    const { status: approveStatus, payload: approvePayload } = await fetchDashboardJson(
      baseUrl,
    ...
    const { status: rejectStatus, payload: rejectPayload } = await fetchDashboardJson(
      baseUrl,
  ...
  it('updates paused workflow continuity after web approval and exposes the final delivery preview', async () => {
    config.zavorthWebAuthToken = 'test-web-token';
    const workflowRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zavorth-dashboard-approval-workflow-'));
    tempDirs.push(workflowRoot);
    config.workflowRunDir = workflowRoot;

    const finalArtifactPath = path.join(process.cwd(), `dashboard-final-delivery-${Date.now()}.md`);
    fs.writeFileSync(
      finalArtifactPath,
          ...
          continuity_recommendations: [
            {
              ...
              reason: 'O briefing está pronto para revisao final e liberacao.',
            },
          ],
        },
      },
          ...
          continuity_recommendations: [
            {
              ...
              reason: 'A aprovação foi registrada e o arquivo final já está pronto.',
            },
          ],
        };
        task.artifacts = [
    ...
    const { status: beforeStatus, payload: beforePayload } = await fetchDashboardJson(
      baseUrl,
    ...
    const { status: afterStatus, payload: afterPayload } = await fetchDashboardJson(
      baseUrl,
      `/api/web/state?sessionId=${encodeURIComponent(sessionPayload.sessionId)}`,
      { token },
    );
    const { status: previewStatus, payload: previewPayload } = await fetchDashboardJson(
      baseUrl,
