...
function createIdFactory() {
  let index = 0;
  return (prefix: string) => `${prefix}-agent-feedback-loop-${++index}`;
}
...
describe('AgentRunService Feedback Telemetry Product Loop Feedback Telemetry', () => {
  it('publishes run.metadata.feedbackTelemetryProductLoop after public sync', async () => {
    ...
    const result = await service.run({
      userId: 'grey',
      channel: 'cli',
      sessionId: 'session-agent-feedback-loop',
      text: 'go feedback loop',
      workspace: 'C:\\TESTES DEV\\zavorth-core\\Zavorth',
      requestedTools: ['workspace.read'],
      metadata: publicMetadata(),
    });

    const feedbackLoop = result.run.metadata.feedbackTelemetryProductLoop as any;
    expect(result.run.metadata.publicSiteDocsDemoSync).toBeTruthy();
    expect(feedbackLoop).toEqual(expect.objectContaining({
