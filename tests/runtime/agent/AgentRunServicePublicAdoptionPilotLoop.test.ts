...
function createIdFactory() {
  let index = 0;
  return (prefix: string) => `${prefix}-agent-public-adoption-pilot-${++index}`;
}
...
describe('AgentRunService Public Adoption Pilot Loop Public Adoption Pilot', () => {
  it('publishes run.metadata.publicAdoptionPilotLoop after feedbackTelemetryProductLoop', async () => {
    ...
    const result = await service.run({
      userId: 'grey',
      channel: 'cli',
      sessionId: 'session-agent-public-adoption-pilot',
      text: 'go public adoption pilot',
      workspace: 'C:\\TESTES DEV\\zavorth-core\\Zavorth',
      requestedTools: ['workspace.read'],
      metadata: publicMetadata(),
    });

    const pilotLoop = result.run.metadata.publicAdoptionPilotLoop as any;
    expect(result.run.metadata.feedbackTelemetryProductLoop).toBeTruthy();
    expect(pilotLoop).toEqual(expect.objectContaining({
