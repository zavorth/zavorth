...
const SLICE_DOC = 'docs/122-wave-1-event-stream-boundary-slice.md';
const BOUNDARY_FILE = 'src/runtime/external-agents/ExternalAgentGatewayEventStreamBoundary.ts';
...
describe('Wave 1 event stream boundary slice gate', () => {
  it('records gw-event-stream as the only selected implementation slice', () => {
    const content = read(SLICE_DOC);

    expect(content).toContain('Status: wave-1-event-stream-boundary-ready');
    expect(content).toContain('gw-event-stream');
  ...
  it('documents the Zavorth-owned event stream boundary, fixture handoff, and public export', () => {
    const content = read(SLICE_DOC);
    const boundary = read(BOUNDARY_FILE);
    const fixtures = read(FIXTURE_FILE);
    const index = read(INDEX_FILE);

    expect(content).toContain('ExternalAgentGatewayEventStreamBoundary.ts');
    ...
    expect(boundary).toContain('normalizeExternalAgentGatewayEventStream');
    expect(boundary).toContain("sourceEventBusIntroduced: false");
    expect(boundary).toContain('duplicateEventIds');
    expect(boundary).toContain('sourceEventCount');
    expect(fixtures).toContain('normalizeExternalAgentGatewayEventStream(events');
    expect(index).toContain("from './ExternalAgentGatewayEventStreamBoundary.js'");
  });

  it('keeps live integration and source event bus import blocked while naming the next narrow row', () => {
    const content = read(SLICE_DOC);

    expect(content).toContain('source event bus import');
    expect(content).toContain('The boundary does not connect, execute, mutate, send, launch, load source');
    expect(content).toContain('sourceEventBusIntroduced: false');
