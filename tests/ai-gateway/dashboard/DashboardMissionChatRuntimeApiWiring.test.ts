...
describe('Dashboard mission and chat runtime API wiring', () => {
  it('exposes a web-safe chat route that delegates preview and live submit to Runtime API v1', () => {
    ...
    expect(source).toContain("'/api/web/dashboard/chat-v1'");
  ...
  it('submits chat from the Dashboard through chat-v1 instead of the legacy session send route', () => {
    const hook = read('src/ai-gateway/app/(dashboard)/dashboard/useControlPageClient.ts');

    expect(hook).toContain('fetchJson<Record<string, any>>(`/api/web/dashboard/chat-v1`');
  ...
  it('renders mission rows and explicit preview/live controls from the canonical contract', () => {
    ...
    const utils = read('src/ai-gateway/app/(dashboard)/dashboard/dashboardPageClient.utils.ts');
