...
import { buildDashboardViewModelFromZavorthAgentGatewaySnapshot } from '../../../src/ai-gateway/app/(dashboard)/dashboard/dashboard/adapters/zavorthAgentGatewayDashboardAdapter.js';
...
describe('DashboardUniversalRuntimeReadiness', () => {
  ...
  it('projects ZavorthAgentGatewaySnapshot into the dashboard view model without source-runtime shapes', async () => {
    const gateway = new ZavorthAgentGateway({
      ...
      executor: ({ run }) => ({
        ...
        artifacts: [
          {
            id: 'artifact-dashboard-readiness',
    ...
    const completed = await gateway.handle({
      userId: 'grey',
      channel: 'web',
      sessionId: 'session-dashboard-readiness',
    ...
    const viewModel = buildDashboardViewModelFromZavorthAgentGatewaySnapshot(
  ...
  it('represents approval waiting, worker queued, offline and loading states as Zavorth UI concepts', async () => {
    ...
    const pending = await gateway.handle({
      userId: 'grey',
      channel: 'web',
      sessionId: 'session-dashboard-approval',
    ...
    const queuedViewModel = buildDashboardViewModelFromZavorthAgentGatewaySnapshot(
      gateway.buildSnapshot({ activeRunId: approved?.run.id }),
    );
    const offlineViewModel = buildDashboardDashboardViewModel({
