...
describe('DashboardVisualPolish', () => {
  it('uses the real Dashboard shell with prototype-grade visual primitives', () => {
    const primitives = readFileSync(
      join(dashboardDir, 'components/DashboardPrimitives.tsx'),
      'utf8',
    );
    const chatSurface = readFileSync(
      join(dashboardDir, 'components/DashboardChatSurface.tsx'),
      'utf8',
    );

    expect(primitives).toContain('DashboardFoxMark');
