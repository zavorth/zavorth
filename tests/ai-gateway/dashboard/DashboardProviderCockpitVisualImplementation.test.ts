...
import { buildDashboardAdapterInputFromDashboardRuntimeProjection } from '../../../src/ai-gateway/app/(dashboard)/dashboard/dashboard/projections/dashboardRuntimeProjection.js';
...
describe('Dashboard Provider Cockpit Visual Implementation', () => {
  ...
  it('passes provider cockpit through runtime projections for Web and CLI parity', () => {
    const adapterInput = buildDashboardAdapterInputFromDashboardRuntimeProjection({
      projectionVersion: 'dashboard-runtime-projection/v1',
      generatedAt: '2026-05-13T20:00:00.000Z',
      adapterSource: {
    ...
    const viewModel = buildDashboardDashboardViewModel(adapterInput);
  ...
  it('renders from projected state and prepares commands instead of executing provider probes', () => {
    const panelSource = readFileSync(
      join(process.cwd(), 'src/ai-gateway/app/(dashboard)/dashboard/dashboard/components/DashboardOperationsPanel.tsx'),
      'utf8',
    );
    const previewSource = readFileSync(
      join(process.cwd(), 'scripts/dashboard-browser-preview.ts'),
      'utf8',
    );
    const visualQaSource = readFileSync(
      join(process.cwd(), 'scripts/dashboard-provider-cockpit-visual-qa.ts'),
      'utf8',
    );
    const liveSmokeSource = readFileSync(
      join(process.cwd(), 'scripts/dashboard-provider-cockpit-live-smoke.ts'),
      'utf8',
    );
    ...
    const fixturesSource = readFileSync(
      join(process.cwd(), 'src/ai-gateway/app/(dashboard)/dashboard/dashboard/fixtures/dashboardDashboardFixtures.ts'),
      'utf8',
    );
    ...
    expect(panelSource).toContain('DashboardProviderCockpitPanel');
    ...
    expect(liveSmokeSource).toContain('dashboard-snapshot-consumes-provider-cockpit');
    ...
    expect(packageJson.scripts['qa:dashboard-provider-cockpit-visual']).toContain('dashboard-provider-cockpit-visual-qa.ts --require-pass');
    expect(packageJson.scripts['qa:dashboard-provider-cockpit-live']).toContain('dashboard-provider-cockpit-live-smoke.ts --require-pass');
    expect(packageJson.scripts['qa:dashboard']).toContain('qa:dashboard-provider-cockpit-visual');
    expect(packageJson.scripts['qa:dashboard']).toContain('qa:dashboard-provider-cockpit-live');
  });
});
