...
describe('Dashboard daily-use GUI certification wiring', () => {
  it('exposes the daily-use GUI certification as a projection-only Dashboard route', () => {
    const route = read('src/domain/surface/presentation/web-app/WebAppRuntimeStateRouteService.ts');

    expect(route).toContain("'/api/web/dashboard/gui-certification-v1'");
  ...
  it('keeps the certification documented as a pre-desktop gate', () => {
    ...
    expect(docs).toContain('/api/web/dashboard/gui-certification-v1');
    ...
    expect(packageJson).toContain('DashboardDailyUseGuiCertification.test.ts');
  });
});
