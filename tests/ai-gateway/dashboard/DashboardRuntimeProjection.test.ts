...
import {
  buildDashboardAdapterInputFromZavorthAgentGatewaySnapshot,
...
describe('DashboardRuntimeProjection', () => {
  it('projects ZavorthAgentGateway snapshots through a formal projection before the ViewModel', async () => {
    ...
    const projection = buildDashboardRuntimeProjectionFromZavorthAgentGatewaySnapshot(snapshot);
    const adapterInput = buildDashboardAdapterInputFromZavorthAgentGatewaySnapshot(snapshot);
    const adapterInputFromProjection = buildDashboardAdapterInputFromDashboardRuntimeProjection(projection);
    const viewModel = buildDashboardViewModelFromZavorthAgentGatewaySnapshot(snapshot);
  ...
  it('projects Run Observatory queries from the gateway snapshot into the Dashboard view model', async () => {
    const gateway = new ZavorthAgentGateway({
