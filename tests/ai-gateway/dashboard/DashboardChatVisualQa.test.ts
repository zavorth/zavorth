...
describe('DashboardChatVisualQa', () => {
  it('exposes a real browser chat visual QA gate without redesigning the dashboard', () => {
    ...
    expect(packageJson.scripts['qa:dashboard-chat-visual']).toBe(
      'npx tsx scripts/dashboard-chat-visual-qa.ts --require-pass',
    );
    expect(packageJson.scripts['qa:dashboard']).toContain('qa:dashboard-chat-visual');
    expect(script).toContain('assets", "dashboard"');
    ...
    expect(docs).toContain('qa:dashboard-chat-visual');
