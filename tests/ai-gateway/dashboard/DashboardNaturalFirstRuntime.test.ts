...
describe('Dashboard Natural First Runtime Dashboard controls', () => {
  it('projects Natural First metadata into a Dashboard UX snapshot', () => {
    ...
    expect(viewModel.naturalFirstRuntime).toEqual(expect.objectContaining({
      contractVersion: 'natural-first-dashboard-ux/8',
      route: 'approval-proposal',
      routeLabel: 'Approval',
      status: 'approval-required',
      tone: 'degraded',
  ...
  it('maps gateway snapshots into the Dashboard Natural First panel contract', async () => {
    ...
    const viewModel = buildDashboardDashboardViewModel(
      buildDashboardAdapterInputFromDashboardRuntimeProjection(projection),
    );
