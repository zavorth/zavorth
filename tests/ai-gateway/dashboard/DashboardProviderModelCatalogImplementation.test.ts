...
describe('Dashboard Provider & Model Catalog Implementation', () => {
  it('exposes a read-only provider model catalog route, scripts, and dashboard panel', () => {
    ...
    const pagesSource = readFileSync(
      join(process.cwd(), 'assets/dashboard/scripts/pages.js'),
      'utf8',
    );
    const runtimeBridgeSource = readFileSync(
      join(process.cwd(), 'assets/dashboard/scripts/runtime-bridge.js'),
      'utf8',
    );
