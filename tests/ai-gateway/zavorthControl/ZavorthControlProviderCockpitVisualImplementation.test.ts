...
describe('ZavorthControl Provider Cockpit Visual Implementation', () => {
  ...
  it('renders from projected state and prepares commands instead of executing provider probes', () => {
    const panelSource = readFileSync(
      join(process.cwd(), 'src/zavorth-control/app/(zavorthControl)/control/zavorth-control/components/ZavorthControlOperationsPanel.tsx'),
      'utf8',
    );
    ...
    const fixturesSource = readFileSync(
      join(process.cwd(), 'src/zavorth-control/app/(zavorthControl)/control/zavorth-control/fixtures/zavorthControlZavorthControlFixtures.ts'),
      'utf8',
    );
