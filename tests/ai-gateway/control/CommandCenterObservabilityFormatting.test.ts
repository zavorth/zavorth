import {
  ...
  normalizeCommandCenterRunStatus,
} from '../../../src/ai-gateway/app/(dashboard)/control/command-center/components/CommandCenterObservability.js';
...
import type { DashboardRunObservatorySnapshot } from '../../../src/ai-gateway/app/(dashboard)/control/command-center/contracts/index.js';

describe('CommandCenterObservabilityFormatting', () => {
  it('formats budget units before falling back to token budget or raw status', () => {
    expect(formatCommandCenterBudgetLabel({
      status: 'ok',
  ...
  it('formats Model Picker route detail without hiding fallback/readiness context', () => {
    ...
    expect(formatCommandCenterModelRouteLabel(profile)).toBe('gemini');
    expect(formatCommandCenterModelRouteDetail(profile)).toContain('Gemini/gemini-2.5-flash');
    expect(formatCommandCenterModelRouteDetail(profile)).toContain('fonte current-config');
    expect(formatCommandCenterModelRouteDetail(profile)).toContain('fallback gemini -> openai');
  });
  ...
  it('formats Run Observatory query, status index and matched run identity', () => {
    ...
    expect(formatCommandCenterRunObservatoryQuery(observatory)).toBe('trace trace-1; status failed');
    expect(formatCommandCenterRunStatusIndex(observatory)).toBe('completed:2 | failed:1');
    expect(formatCommandCenterRunIdentity(observatory.runs[0])).toBe('trace-1');
    expect(formatCommandCenterRunMatchedBy(observatory.runs[0].matchedBy)).toBe('trace + status');
    expect(formatCommandCenterRunMatchedBy(['recent'])).toBe('recente');
  });
  ...
  it('normalizes and filters Run Observatory snapshots for URL-driven Z7 queries', () => {
    ...
    expect(normalizeCommandCenterRunStatus('waiting approval')).toBe('waiting_approval');
    expect(commandCenterRunObservatoryHasQuery({ status: 'failed' })).toBe(true);
    expect(normalizeCommandCenterRunObservatoryQuery({
      runId: ' run-1 ',
    ...
    const filtered = filterCommandCenterRunObservatory(observatory, {
  ...
  it('builds run observability rows from the dashboard view model', () => {
    ...
    expect(formatCommandCenterBudgetDetail(viewModel.budget)).toContain('fonte RunBudgetPolicy');
    expect(buildCommandCenterRunObservabilityRows(viewModel)).toEqual(expect.arrayContaining([
