...
describe('Command Center mission and chat runtime API wiring', () => {
  it('exposes a web-safe chat route that delegates preview and live submit to Runtime API v1', () => {
    ...
    expect(source).toContain("'/api/web/command-center/chat-v1'");
  ...
  it('submits chat from the Command Center through chat-v1 instead of the legacy session send route', () => {
    const hook = read('src/ai-gateway/app/(dashboard)/control/useControlPageClient.ts');

    expect(hook).toContain('fetchJson<Record<string, any>>(`/api/web/command-center/chat-v1`');
  ...
  it('renders mission rows and explicit preview/live controls from the canonical contract', () => {
    ...
    const sidebar = read('src/ai-gateway/app/(dashboard)/control/controlPageClient.sidebar.tsx');
    const utils = read('src/ai-gateway/app/(dashboard)/control/controlPageClient.utils.ts');
