...
describe('CommandCenterBrowserPreview', () => {
  it('keeps a local browser preview script wired to official Command Center fixtures', () => {
    const script = readFileSync(
      join(rootDir, 'scripts/command-center-browser-preview.ts'),
      'utf8',
    );

    expect(script).toContain('buildDashboardCommandCenterFixturePreviewViewModel');
    expect(script).toContain('listDashboardCommandCenterFixturePreviewOptions');
    expect(script).toContain('DashboardCommandCenterViewModel');
    ...
    expect(script).toContain('/api/web/command-center');
    expect(script).toContain('buildLiveViewModelFromSnapshot');
    expect(script).toContain('command-center-auth-form');
    expect(script).toContain('/api/auth/validate');
    expect(script).toContain('sessionStorage');
  });

  it('exposes the browser preview through package scripts and Command Center QA', () => {
    const packageJson = JSON.parse(
      readFileSync(join(rootDir, 'package.json'), 'utf8'),
    ) as { scripts: Record<string, string> };

    expect(packageJson.scripts['command-center:preview']).toContain('scripts/command-center-browser-preview.ts');
    expect(packageJson.scripts['qa:command-center-browser-preview']).toContain('CommandCenterBrowserPreview.test.ts');
    expect(packageJson.scripts['qa:command-center']).toContain('qa:command-center-browser-preview');
  });
  ...
  it('exposes the generated fixture review inside the real dashboard routes', () => {
    ...
    expect(webAppService).toContain('buildPublicCommandCenterFallbackSnapshot');
  ...
  it('keeps the user-provided Command Center visual as the main /control shell and bridges data non-invasively', () => {
    const shell = readFileSync(
      join(rootDir, 'assets/command-center/index.html'),
      'utf8',
    );
    ...
    const pagesScript = readFileSync(
      join(rootDir, 'assets/command-center/scripts/pages.js'),
      'utf8',
    );
    const layoutCss = readFileSync(
      join(rootDir, 'assets/command-center/styles/layout.css'),
      'utf8',
    );
    ...
    expect(appScript).toContain('ZavorthCommandCenterChat');
    ...
    const chatCss = readFileSync(
      join(rootDir, 'assets/command-center/styles/chat.css'),
      'utf8',
    );
