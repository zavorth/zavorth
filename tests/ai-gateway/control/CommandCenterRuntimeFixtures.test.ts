...
const REQUIRED_FIXTURE_IDS: DashboardCommandCenterFixtureId[] = [
  'safe-run',
  'awaiting-approval',
...
describe('CommandCenterRuntimeFixtures', () => {
  it('freezes the required product scenarios for the Command Center cockpit', () => {
    expect([...DASHBOARD_COMMAND_CENTER_FIXTURE_IDS]).toEqual(REQUIRED_FIXTURE_IDS);
    expect(listDashboardCommandCenterFixtures()).toHaveLength(REQUIRED_FIXTURE_IDS.length);
  });

  it.each(REQUIRED_FIXTURE_IDS)('builds a valid view model for fixture %s', (fixtureId) => {
    const fixture = getDashboardCommandCenterFixture(fixtureId);
    const viewModel = buildDashboardCommandCenterFixture(fixtureId);
  ...
  it('models a safe run without approvals or restricted tools', () => {
    const viewModel = buildDashboardCommandCenterFixture('safe-run');
  ...
  it('models automatic subagent delegation as governed telemetry', () => {
    const viewModel = buildDashboardCommandCenterFixture('auto-subagents');
  ...
  it('models a pending approval as a visible blocker and approval action', () => {
    const viewModel = buildDashboardCommandCenterFixture('awaiting-approval');
  ...
  it('models execution failure with blocked health and error logs', () => {
    const viewModel = buildDashboardCommandCenterFixture('failed-run');
  ...
  it('models artifact and replay states separately', () => {
    const artifactFixture = buildDashboardCommandCenterFixture('artifact-ready');
    const replayFixture = buildDashboardCommandCenterFixture('replay-available');
  ...
  it('models policy and budget gates without pretending execution succeeded', () => {
    const policyFixture = buildDashboardCommandCenterFixture('policy-blocked');
    const budgetFixture = buildDashboardCommandCenterFixture('budget-exceeded');
