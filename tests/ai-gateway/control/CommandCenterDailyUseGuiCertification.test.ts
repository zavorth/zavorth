...
describe('Command Center daily-use GUI certification wiring', () => {
  it('exposes the daily-use GUI certification as a projection-only Command Center route', () => {
    const route = read('src/domain/surface/presentation/web-app/WebAppRuntimeStateRouteService.ts');

    expect(route).toContain("'/api/web/command-center/gui-certification-v1'");
  ...
  it('keeps the certification documented as a pre-desktop gate', () => {
    ...
    expect(docs).toContain('/api/web/command-center/gui-certification-v1');
    ...
    expect(packageJson).toContain('CommandCenterDailyUseGuiCertification.test.ts');
  });
});
