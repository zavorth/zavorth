...
describe('CommandCenterShellWiring', () => {
  it('wires /control to the Command Center shell instead of the legacy layout', () => {
  ...
  it('does not expose /control as a routable product page', () => {
  ...
  it('keeps the shell connected to the real control model and adapter', () => {
    ...
    expect(shell).toContain('buildDashboardCommandCenterViewModel');
    expect(shell).toContain('CommandCenterOnboardingPanel');
    expect(shell).toContain('CommandCenterDeveloperWorkspace');
    expect(shell).toContain('CommandCenterGatewayConsole');
    ...
    expect(shell).toContain('filterCommandCenterRunObservatory');
    expect(shell).toContain('readCommandCenterRunObservatoryUrlQuery');
    expect(shell).toContain('useCommandCenterNexusWorkbench');
    ...
    const overview = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterOverviewSector.tsx'),
      'utf8',
    );
    const nexusHook = readFileSync(
      join(controlDir, 'command-center/components/useCommandCenterNexusWorkbench.ts'),
      'utf8',
    );
  ...
  it('keeps the premium chat surface wired to real sending, events, artifacts and memory context', () => {
    ...
    const chatSurface = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterChatSurface.tsx'),
      'utf8',
    );

    expect(shell).toContain('CommandCenterChatSurface');
    ...
    expect(chatSurface).toContain('CommandCenterChatContextStrip');
  });
  ...
  it('adds the phase 6 operation layer without creating destructive automatic actions', () => {
    ...
    const commandPalette = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterCommandPalette.tsx'),
      'utf8',
    );

    expect(shell).toContain('CommandCenterOperationsPanel');
    expect(shell).toContain('CommandCenterCommandPalette');
  ...
  it('renders every Command Center sector from real control data or honest empty states', () => {
    const shell = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterControlShell.tsx'),
      'utf8',
    );
  ...
  it('renders the Nexus Workbench in the Command Center overview from the canonical API', () => {
    const overview = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterOverviewSector.tsx'),
      'utf8',
    );
    const adapter = readFileSync(
      join(controlDir, 'command-center/adapters/dashboardCommandCenterAdapter.ts'),
      'utf8',
    );
    const nexusAdapter = readFileSync(
      join(controlDir, 'command-center/adapters/dashboardCommandCenterNexusWorkbenchAdapter.ts'),
      'utf8',
    );
  ...
  it('ships the Developer Workspace as an official Command Center sector', () => {
    const shell = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterControlShell.tsx'),
      'utf8',
    );
    const workspace = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterDeveloperWorkspace.tsx'),
      'utf8',
    );
  ...
  it('ships the Gateway Console as a controlled Gateway Control API surface', () => {
    const gatewayConsole = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterGatewayConsole.tsx'),
      'utf8',
    );
  ...
  it('does not port demo metrics from the fake dashboard into the real /control shell', () => {
    const shell = readFileSync(
      join(controlDir, 'command-center/components/CommandCenterControlShell.tsx'),
      'utf8',
    );
