...
describe('CommandCenterComposerAffordancesQa', () => {
  it('guards attachments, skills and voice as real composer affordances', () => {
    ...
    const qa = readFileSync(
      join(rootDir, 'scripts/command-center-composer-affordances-qa.ts'),
      'utf8',
    );
    const liveQa = readFileSync(
      join(rootDir, 'scripts/command-center-live-composer-affordances-qa.ts'),
      'utf8',
    );
    ...
    expect(packageJson.scripts['qa:command-center-composer-affordances']).toBe(
      'npx tsx scripts/command-center-composer-affordances-qa.ts --require-pass',
    );
    expect(packageJson.scripts['qa:command-center-live-composer']).toBe(
      'npx tsx scripts/command-center-live-composer-affordances-qa.ts --allow-send --require-live --require-pass',
    );
    expect(packageJson.scripts['qa:command-center']).toContain('qa:command-center-composer-affordances');
    expect(packageJson.scripts['qa:command-center']).not.toContain('qa:command-center-live-composer');
    ...
    expect(docs).toContain('qa:command-center-composer-affordances');
    expect(docs).toContain('qa:command-center-live-composer');
