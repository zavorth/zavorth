...
describe('Desktop Read-Only Approvals Panel', () => {
  ...
  describe('sanitizeApproval — pure function', () => {
    ...
    it('never reads, uses, or propagates summary field', () => {
      ...
      expect((result as unknown as Record<string, unknown>).details).toBeUndefined();
    ...
    it('does not call filesystem, IPC, MCP, localStorage, or sessionStorage', () => {
      ...
      const origFS = (global as Record<string, unknown>).fs;
      const origIPC = (global as Record<string, unknown>).ipcRenderer;
      const origLS = (global as Record<string, unknown>).localStorage;
      (global as Record<string, unknown>).fs = undefined;
      (global as Record<string, unknown>).ipcRenderer = undefined;
      ...
      expect(result).not.toBeNull();
      (global as Record<string, unknown>).fs = origFS;
      (global as Record<string, unknown>).ipcRenderer = origIPC;
      (global as Record<string, unknown>).localStorage = origLS;
    });
  });
