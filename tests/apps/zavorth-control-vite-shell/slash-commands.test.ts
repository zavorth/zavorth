...
describe('zavorth-control slash commands', () => {
  it('parses local commands, aliases, and colon arguments', () => {
    ...
    expect(parseSlashCommand('/stop')).toEqual(expect.objectContaining({
      command: expect.objectContaining({ key: 'stop', category: 'session' }),
    }));
    expect(parseSlashCommand('/model openai/gpt-5.5')).toEqual(expect.objectContaining({
      args: 'openai/gpt-5.5',
      command: expect.objectContaining({ key: 'model', category: 'runtime' }),
    }));
    expect(parseSlashCommand('/reasoning deep')).toEqual(expect.objectContaining({
      args: 'deep',
      command: expect.objectContaining({ key: 'think' }),
    }));
    expect(parseSlashCommand('/tasks')).toEqual(expect.objectContaining({
      command: expect.objectContaining({ key: 'agents' }),
    }));
    expect(parseSlashCommand('/approve abc123')).toEqual(expect.objectContaining({
      args: 'abc123',
      name: 'approve',
      command: expect.objectContaining({ key: 'approvals' }),
    }));
    expect(parseSlashCommand('/deny abc123')).toEqual(expect.objectContaining({
      args: 'abc123',
      name: 'deny',
      command: expect.objectContaining({ key: 'approvals' }),
    }));
    expect(parseSlashCommand('/grill-me should we ship this plan?')).toEqual(expect.objectContaining({
      args: 'should we ship this plan?',
      command: expect.objectContaining({ key: 'plan-review', category: 'runtime' }),
    }));
    expect(parseSlashCommand('/brief mobile update')).toEqual(expect.objectContaining({
      args: 'mobile update',
      command: expect.objectContaining({ key: 'brief-reply', category: 'runtime' }),
    }));
    expect(parseSlashCommand('/caveman release note')).toEqual(expect.objectContaining({
      args: 'release note',
      name: 'caveman',
      command: expect.objectContaining({ key: 'brief-reply' }),
    }));
    expect(parseSlashCommand('/tdd command aliases')).toEqual(expect.objectContaining({
  ...
  it('returns autocomplete entries and help from the same registry', () => {
    ...
    expect(help).toContain('/approvals [approve|deny] <id> [permission|task|agent-run]');
