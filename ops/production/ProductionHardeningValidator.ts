  ...
  public validate(): ProductionValidationReport {
    ...
    const checks: ProductionValidationCheck[] = [
      ...
      assertCheck(
        ...
        'compose blocks automatic startup installation',
      ),
      ...
      assertCheck(/USER\s+zavorth/.test(dockerfileRaw), 'dockerfile.user', 'Production Dockerfile runs as user zavorth'),
      assertCheck(/EXPOSE\s+33333/.test(dockerfileRaw), 'dockerfile.port', 'Production Dockerfile exposes port 33333'),
      ...
      assertCheck(/ZAVORTH_ALLOW_STARTUP_INSTALL=false/.test(dockerfileRaw), 'dockerfile.no_startup_install', 'Dockerfile blocks automatic startup install'),
