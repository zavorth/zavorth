# Spec: PRD — Zavorth Core

> Status: documento histórico de produto. O fluxo real em execução hoje está descrito em [architecture.md](./architecture.md) e implementado no bootstrap de `src/index.ts`.
...
O Zavorth é um agente pessoal de Inteligência Artificial para operar 100% localmente no desktop do usuário. Ele recebe comandos exclusivamente pelo Telegram, processa-os através de um pipeline que suporta múltiplos LLMs dinamicamente, e tem acesso persistente à memória em SQLite.
...
**Problema:**
Agentes hospedados na nuvem e serviços de terceiros requerem expor dados privados ou têm custos recorrentes altos, além da falta de total governança sobre as próprias "skills" customizadas. O usuário não tem controle pleno de instâncias como o External Executor sem esbarrar na complexidade da nuvem ou lock-in.

**Evidências:**
Tentativas anteriores baseadas no External Executor funcionavam, mas a intenção primária agora é manter uma base minimalista sob controle total do usuário, operando no próprio SO.
...
O usuário envia um chat no Telegram, o Zavorth roda local em background num terminal, chama LLMs, lê Skills em pastas locais, aciona ferramentas e responde no mesmo chat de forma orgânica.
