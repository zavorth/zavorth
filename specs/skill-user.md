...
> Status: documento histórico de design. Em caso de conflito, prevalecem [architecture.md](./architecture.md) e o runtime atual do diretório `src/`.
...
O runtime atual de skills tambem inclui:

- `source registry` e `allowlist` para governar intake
- `workspace-imported-library` para skills materializadas no workspace
- `ORIGIN.json` e `ATTRIBUTION.md` para provenance
- `bundle tags`, `license` e catalogo local de skills

Este documento continua valido para o fluxo basico Loader -> Router -> Executor, mas a governanca completa da plataforma agora depende tambem dos contratos em `config/skill-sources.json`, `config/skill-allowlist.json` e `docs/29-skill-platform.md`.

## 16. Addendum (Fase 3)

O intake externo agora tambem inclui uma etapa seletiva de materializacao:

- preview de import com policy e provenance
- classificacao de licenca antes da ingestao
- scanner de conteudo para bloquear arquivos inseguros ou irrelevantes
- biblioteca curada em `skill-library/imported`

Na pratica, o runtime so consome skills externas depois que elas passam por esse pipeline. O contrato historico de hot-reload continua valido para skills locais, mas skills vindas de fontes externas agora seguem uma trilha de trust, allowlist e atribuicao explicita.

## 17. Addendum (Fase 4)

O catalogo de skills agora tambem ganhou superficies oficiais de runtime:

- API protegida do app web
- API legada do dashboard
- comando `/skills` no Telegram
- comando `skills` na CLI compartilhada
- projecao MCP para discovery e recomendacao

Com isso, o fluxo Loader -> Router -> Executor continua sendo o nucleo de execucao, mas agora existe tambem um plano oficial de consulta e recomendacao de skills, com recipes compostas e visao MCP coerente com o mesmo catalogo local/importado.

## 18. Addendum (Fase 7)

O intake e o catalogo agora tambem carregam governanca operacional:

- score de risco por skill importada
- policy de licenca derivada da classificacao efetiva
- trilha de auditoria para preview e import
- scripts oficiais para security scan e license audit

Na pratica, a plataforma nao responde mais apenas "a skill entrou ou nao entrou"; ela responde tambem "qual o risco", "qual a policy de uso" e "qual evento gravou esse import".

## 19. Addendum (Fase 8)

O plano de skills agora tambem possui release proprio:

- snapshot consolidado do catalogo
- snapshot da projecao MCP
- artefatos gravados em `data/release/skills`
- playbook operacional de release

Com isso, a plataforma de skills deixa de ser apenas um runtime curado e passa a ter tambem um pacote explicito de distribuicao continua.
