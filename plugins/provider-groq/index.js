/**
 * Soft-fail Groq Provider plugin (OpenAI-compatible HTTP).
 */
function register(ctx) {
  ...
  function statusPayload() {
    ...
    return {
      ...
      message: keyPresent
        ? 'GROQ_API_KEY present; complete available when network.external permission granted.'
        : 'Set GROQ_API_KEY to enable completions.',
      setup: keyPresent ? null : ['export GROQ_API_KEY=...'],
    };
  }

  ctx.bindCapability('provider.groq.status', async () => ({
    output: statusPayload(),
  }));

  ctx.bindProvider({
    id: 'groq',
    capabilityId: 'provider.groq.complete',
    name: 'groq',
    label: 'Groq',
    ...
    async complete(input) {
      ...
      if (!status.keyPresent) {
        return { ...status, ok: false, message: 'GROQ_API_KEY not set' };
      }
      const allowed = await ctx.requestPermission('network.external', 'Groq chat completion');
      if (!allowed) {
      ...
      const model = String((input && input.model) || process.env.GROQ_MODEL || 'llama-3.3-70b-versatile');
      const baseUrl = String(process.env.GROQ_BASE_URL || 'https://api.groq.com/openai/v1').replace(/\/$/u, '');
      ...
      try {
        ...
        logger.warn('provider-groq complete failed', {
  ...
  logger.info('provider-groq registered');
}
...
function postJson(url, body, apiKey, extraHeaders) {
  return new Promise((resolve, reject) => {
    ...
    const headers = {
      ...
      'User-Agent': 'zavorth-provider-groq/1.0',
      ...(extraHeaders || {}),
    };
    if (apiKey) {
