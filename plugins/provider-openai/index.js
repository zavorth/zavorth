/**
 * Soft-fail OpenAI Provider plugin (OpenAI-compatible HTTP).
 */
function register(ctx) {
  ...
  function statusPayload() {
    ...
    return {
      ...
      message: keyPresent
        ? 'OPENAI_API_KEY present; complete available when network.external permission granted.'
        : 'Set OPENAI_API_KEY to enable completions.',
      setup: keyPresent ? null : ['export OPENAI_API_KEY=...'],
    };
  }

  ctx.bindCapability('provider.openai.status', async () => ({
    output: statusPayload(),
  }));

  ctx.bindProvider({
    id: 'openai',
    capabilityId: 'provider.openai.complete',
    name: 'openai',
    label: 'OpenAI',
    ...
    async complete(input) {
      ...
      const allowed = await ctx.requestPermission('network.external', 'OpenAI chat completion');
      if (!allowed) {
      ...
      try {
        ...
        logger.warn('provider-openai complete failed', {
...
function postJson(url, body, apiKey, extraHeaders) {
  return new Promise((resolve, reject) => {
    ...
    const headers = {
      ...
      'User-Agent': 'zavorth-provider-openai/1.0',
      ...(extraHeaders || {}),
    };
    if (apiKey) {
