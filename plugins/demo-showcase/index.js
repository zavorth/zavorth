/**
 * Demo showcase (soft-fail).
 * Exercises as many specialized register_* APIs as possible in one package.
 * No real network. moduleKind=diagnostics → skips registerPlatform / bindChannel.
 */

const PACK = 'lifestyle';
...
function register(ctx) {
 ...
 try {
 const allowed = await ctx.requestPermission('secret.read', 'Probe presence of DEMO_SHOWCASE_SECRET');
 if (!allowed) {
 return {
 ok: false,
 wave: PACK,
 ...
 const token = String((input && (input.token || input.bearer || input.authorization || input.value)) || '').trim();
