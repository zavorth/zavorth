/**
 * Soft-fail DeepSeek Provider plugin (OpenAI-compatible HTTP).
 */
function register(ctx) {
  ...
  function statusPayload() {
    ...
    return {
      ...
      message: keyPresent
        ? 'DEEPSEEK_API_KEY present; complete available when network.external permission granted.'
        : 'Set DEEPSEEK_API_KEY to enable completions.',
      setup: keyPresent ? null : ['export DEEPSEEK_API_KEY=...'],
    };
  }

  ctx.bindCapability('provider.deepseek.status', async () => ({
    output: statusPayload(),
  }));

  ctx.bindProvider({
    id: 'deepseek',
    capabilityId: 'provider.deepseek.complete',
    name: 'deepseek',
    label: 'DeepSeek',
    ...
    async complete(input) {
      ...
      if (!status.keyPresent) {
        return { ...status, ok: false, message: 'DEEPSEEK_API_KEY not set' };
      }
      const allowed = await ctx.requestPermission('network.external', 'DeepSeek chat completion');
      if (!allowed) {
      ...
      const model = String((input && input.model) || process.env.DEEPSEEK_MODEL || 'deepseek-chat');
      ...
      try {
        ...
        logger.warn('provider-deepseek complete failed', {
  ...
  logger.info('provider-deepseek registered');
}
...
function postJson(url, body, apiKey, extraHeaders) {
  return new Promise((resolve, reject) => {
    ...
    const headers = {
      ...
      'User-Agent': 'zavorth-provider-deepseek/1.0',
      ...(extraHeaders || {}),
    };
    if (apiKey) {
