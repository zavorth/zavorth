/**
 * platform-discord — soft-fail Discord Bot REST channel.
...
function register(ctx) {
 ...
 const allowed = await ctx.requestPermission('network.external', 'Discord REST channel message create');
 if (!allowed) {
 return {
 ok: false,
 delivered: false,
 ...
 const content = String((input && (input.content || input.text || input.message || input.body)) || '').trim();
 ...
 try {
 const result = await postJson(`https://discord.com${path}`, { content: content.slice(0, 2000) }, token);
