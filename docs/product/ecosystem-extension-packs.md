...
Machine index: `config/ecosystem-extension-packs.json`.
...
Define a **canonical intermediate representation** for any ingested pack so later groups never branch on brand names.
...
- `docs/product/ecosystem-extension-packs.md` (this doc)
...
- Over-normalizing breaks existing install tests — keep dual-write IR + legacy fields for one group.
...
## Cross-cutting rules (all groups)
...
4. **Tests:** each pack adds unit + at least one integration/pipeline test; update ecosystem pack status in `config/ecosystem-extension-packs.json`.
...
- [ ] pack status file updated to `shipped` per group with date.
...
| Field                 | Location                                                       |
| --------------------- | -------------------------------------------------------------- |
| Pack status JSON      | `config/ecosystem-extension-packs.json`                        |
| This plan             | `docs/product/ecosystem-extension-packs.md`                    |
| Prior Plugin OS groups | `config/plugin-os-capability-groups.json` (orthogonal; do not regress) |
