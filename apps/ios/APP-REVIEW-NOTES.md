...
Use the Zavorth iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open Zavorth`.
...
command `printf 'Zavorth App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send Zavorth to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while Zavorth is backgrounded.
Reopen Zavorth, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send Zavorth to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `Zavorth` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because Zavorth ends the Live Activity after
...
4. Immediately send Zavorth to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `Zavorth`
notification with this body:

```text
Zavorth App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If Zavorth opens on
...
3. Immediately send Zavorth to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `Zavorth` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If Zavorth opens on
   `Control`, tap `Chat`.
...
4. Tap `Share`.
5. Tap `Zavorth`.
6. Confirm the Zavorth share extension appears and shows
   `Edit text, then tap Send.` and `Send to Zavorth`.
7. Tap `Send to Zavorth`.

Expected result: the Zavorth share extension sends the shared Safari page to
the live review Gateway and shows `Sent to Zavorth.` Returning to Zavorth
