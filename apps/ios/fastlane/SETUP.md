...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=zavorth-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=dev.zavorth.companion
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical Zavorth iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the Zavorth gateway host separately with these gateway env vars (legacy key names): `ZAVORTH_APNS_TEAM_ID`, `ZAVORTH_APNS_KEY_ID`, and either `ZAVORTH_APNS_PRIVATE_KEY_P8` or `ZAVORTH_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/app-store/Zavorth-<version>.ipa` exists
