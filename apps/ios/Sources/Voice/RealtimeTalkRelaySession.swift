...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "zavorth_agent_control"
    ...
    private let logger = Logger(subsystem: "dev.zavorth.companion", category: "RealtimeTalkRelay")
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: ZavorthChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
