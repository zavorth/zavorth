...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<ZavorthTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "dev.zavorth.companion", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ZavorthTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ZavorthTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return ZavorthTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ZavorthTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ZavorthTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ZavorthTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ZavorthTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ZavorthTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ZavorthTalkPTTStopPayload) {
    ...
    private static func chatSendHistorySince(
        response: ZavorthChatSendResponse,
        startedAt: Double) -> Double?
    {
        self.isTerminalChatSendSuccess(response.status) ? nil : startedAt
    }

    private func sendChat(_ message: String, gateway: GatewayNodeSession) async throws -> ZavorthChatSendResponse {
        ...
        return try JSONDecoder().decode(ZavorthChatSendResponse.self, from: res)
    }
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = ZavorthChatEventText.assistantText(from: chatEvent) {
