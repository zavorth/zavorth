...
struct CommandCenterTab: View {
        ...
        NavigationStack {
            GeometryReader { geometry in
                ZStack {
                    ...
                    .safeAreaPadding(.bottom, ZavorthProMetric.bottomScrollInset)
                }
            }
    ...
    private var header: some View {
            ...
            subtitleLineLimit: 1)
        {
            ...
            {
                ZavorthProMark(size: 28, shadowRadius: 5)
            }
        } accessory: {
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(ZavorthBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? ZavorthBrand.ok : .secondary
    }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? ZavorthBrand.accent : ZavorthBrand.ok,
            progress: nil,
            route: .chat(nil))
    }
    ...
    private static func sessionChoices(
        _ sessions: [ZavorthChatSessionEntry],
        currentSessionKey: String,
        defaultSessionKey: String) -> [ZavorthChatSessionEntry]
    {
        let sorted = sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var result: [ZavorthChatSessionEntry] = []
    ...
    static func sessionWorkItem(
        for session: ZavorthChatSessionEntry,
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? ZavorthBrand.accent : ZavorthBrand.ok,
            progress: nil,
            route: .chat(session.key))
    }

    fileprivate static func sessionTitle(_ session: ZavorthChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: ZavorthChatSessionEntry) -> String {
