...
extension SettingsProTab {
        ...
        ProCard(padding: 0, radius: SettingsLayout.cardRadius) {
            VStack(spacing: 0) {
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.gatewayController.gateways.isEmpty ? .secondary : ZavorthBrand.accent)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "waveform",
                    title: "Talk Config",
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.appModel.screenRecordActive ? ZavorthBrand.ok : .secondary)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "mic",
                    title: "Voice Wake",
                    ...
                    color: self.voiceWakeEnabled ? ZavorthBrand.ok : .secondary)
            }
        }
        .padding(.horizontal, ZavorthProMetric.pagePadding)
    }
    ...
    func handleLocationModeChange(_ newValue: String) {
        ...
        guard let mode = ZavorthLocationMode(rawValue: newValue) else { return }
    ...
    func handleNotificationAction() {
        ...
        if PushBuildConfig.current.usesZavorthHostedRelay {
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return ZavorthBrand.accent }
        return self.gatewayConnected ? ZavorthBrand.ok : .secondary
    }
    ...
    var approvalEmptyDetail: String {
        ...
        if self.notificationsNeedAttention {
            return "Foreground approvals still appear while Zavorth is connected."
        }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? ZavorthBrand.ok : .secondary
    }
    ...
    var gatewayServer: String {
        self.appModel.gatewayServerName ?? "Zavorth Gateway"
    }

    var permissionsDetail: String {
        var enabled = 0
        if self.cameraEnabled { enabled += 1 }
        if self.locationModeRaw != ZavorthLocationMode.off.rawValue { enabled += 1 }
    ...
    var diagnosticsRunColor: Color {
        ...
        return diagnosticsIssueCount == 0 ? ZavorthBrand.ok : ZavorthBrand.warn
    }

    var privacyDetail: String {
        let location = ZavorthLocationMode(rawValue: self.locationModeRaw) ?? .off
        return location == .off ? "Location off" : "Location \(self.locationLabel)"
    }

    var locationLabel: String {
        switch ZavorthLocationMode(rawValue: self.locationModeRaw) ?? .off {
        case .off: "Off"
    ...
    var notificationStatusDetail: String {
        ...
        case .allowed:
            "Zavorth can show approval prompts and event alerts when the app is not active."
        case .notAllowed:
        ...
        case .unknown:
            "Zavorth cannot determine the current notification permission state."
        }
    }
    ...
    var notificationRelayDetail: String {
        if PushBuildConfig.current.usesZavorthHostedRelay {
            ...
            } ?? "ios-push-relay.zavorth.ai"
            return """
            This build uses Zavorth's hosted push relay at \(host) for notification \
            delivery data.
            """
        }
        return "This build is not configured to use Zavorth's hosted push relay."
    }

    var notificationRelayDisclosureMessage: String {
        "Enabling this sends delivery data through Zavorth's hosted push relay."
    }
}
