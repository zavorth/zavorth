...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return ZavorthBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? ZavorthBrand.ok : ZavorthBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return ZavorthBrand.ok }
        if self.lastError != nil { return ZavorthBrand.warn }
        return self.configured ? ZavorthBrand.accent : .secondary
    }
