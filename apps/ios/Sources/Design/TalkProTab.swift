...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            ZavorthProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking Zavorth" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return ZavorthBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return ZavorthBrand.warn
        default:
            if !self.isConfigLoaded { return ZavorthBrand.warn }
            return self.isEnabled ? ZavorthBrand.ok : ZavorthBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(ZavorthBrand.danger)
        case .waiting:
            AnyShapeStyle(ZavorthBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), ZavorthBrand.accent],
