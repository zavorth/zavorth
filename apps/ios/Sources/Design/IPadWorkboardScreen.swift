...
struct IPadWorkboardScreen: View {
    ...
    private func compactStatusChip(_ status: String) -> some View {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? ZavorthBrand.accent : .primary)
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": ZavorthBrand.ok
        case "review": ZavorthBrand.accent
        case "blocked": ZavorthBrand.warn
        case "done": .secondary
        default: ZavorthBrand.accentHot
        }
    }
