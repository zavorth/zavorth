...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : ZavorthBrand.warn,
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, ZavorthProMetric.bottomScrollInset)
            }
        }
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top, spacing: 12) {
                    ProIconBadge(systemName: "bell.slash.fill", color: ZavorthBrand.warn)
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Notifications are off")
                            .font(.subheadline.weight(.semibold))
                        Text(
                            """
                            Enable Notifications to receive approval notifications while Zavorth is not open.
                            """)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(ZavorthBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: ZavorthBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while Zavorth is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while Zavorth is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? ZavorthBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? ZavorthBrand.ok : ZavorthBrand.warn)
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context Zavorth can expose to the gateway.",
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: "network")
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(ZavorthBrand.accent)
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? ZavorthBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == ZavorthLocationMode.off.rawValue ? .secondary : ZavorthBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(ZavorthLocationMode.off.rawValue)
                    Text("While Using").tag(ZavorthLocationMode.whileUsing.rawValue)
                    Text("Always").tag(ZavorthLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
                ...
                if let locationStatusText {
                    Text(locationStatusText)
                        .font(.caption2)
                        .foregroundStyle(ZavorthBrand.warn)
                }
            }
        }
        .padding(.horizontal, ZavorthProMetric.pagePadding)
    ...
    func settingsSwitchIndicator(isOn: Bool) -> some View {
        Capsule()
            .fill(isOn ? ZavorthBrand.accent : Color.secondary.opacity(0.35))
