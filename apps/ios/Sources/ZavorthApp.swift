...
@MainActor
final class ZavorthAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "dev.zavorth.companion", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "dev.zavorth.companion", category: "BackgroundWake")
...
enum WatchPromptNotificationBridge {
    static let typeKey = "zavorth.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "zavorth.watch.promptId"
    static let sessionKeyKey = "zavorth.watch.sessionKey"
    static let actionPrimaryIDKey = "zavorth.watch.action.primary.id"
    static let actionPrimaryLabelKey = "zavorth.watch.action.primary.label"
    static let actionSecondaryIDKey = "zavorth.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "zavorth.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "zavorth.watch.action.primary"
    static let actionSecondaryIdentifier = "zavorth.watch.action.secondary"
    static let actionIdentifierPrefix = "zavorth.watch.action."
    static let actionIDKeyPrefix = "zavorth.watch.action.id."
    static let actionLabelKeyPrefix = "zavorth.watch.action.label."
    static let categoryPrefix = "zavorth.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ZavorthWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ZavorthWatchAction... in
            ...
            return ZavorthWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Zavorth" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ZavorthWatchAction]) -> [UNNotificationAction] {
...
struct ZavorthApp: App {
    ...
    init() {
        ...
        #endif
        ZavorthAppModelRegistry.appModel = appModel
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--zavorth-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--zavorth-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private func applyAppearancePreference() {
        ...
        UIApplication.shared.connectedScenes
            ...
            .forEach { window in
                ...
                window.tintColor = ZavorthBrand.uiAccent
            }
    }
}

extension ZavorthApp {
    @MainActor
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("Zavorth: installing uncaught exception handler")
