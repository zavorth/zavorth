...
final class RemindersService: RemindersServicing {
    func list(params: ZavorthRemindersListParams) async throws -> ZavorthRemindersListPayload {
        ...
        let payload: [ZavorthReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return ZavorthRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ZavorthRemindersAddParams) async throws -> ZavorthRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ZavorthReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ZavorthRemindersAddPayload(reminder: payload)
    }
