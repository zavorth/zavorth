...
enum ScreenshotFixtureMode {
    static let gatewayName = "Zavorth Gateway"
...
struct LocalChatFixture {
    ...
    static let appScreenshots = LocalChatFixture(
        ...
        modelID: "gpt-5.5",
        modelName: "GPT-5.5",
        responsePrefix: "Zavorth is connected to your gateway.",
        seedMessages: [
            """
            Zavorth is connected to your gateway. I can coordinate agents, inspect project context, and prepare \
...
struct LocalFixtureChatTransport: ZavorthChatTransport {
    ...
    func requestHistory(sessionKey: String) async throws -> ZavorthChatHistoryPayload {
        try await self.store.history(sessionKey: sessionKey)
    }

    func listModels() async throws -> [ZavorthChatModelChoice] {
        [
            ZavorthChatModelChoice(
    ...
    func sendMessage(
        ...
        attachments _: [ZavorthChatAttachmentPayload]) async throws -> ZavorthChatSendResponse
    {
...
struct AppleReviewDemoChatTransport: ZavorthChatTransport {
    ...
    func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        attachments: [ZavorthChatAttachmentPayload]) async throws -> ZavorthChatSendResponse
    {
...
private actor LocalFixtureChatStore {
    private let fixture: LocalChatFixture
    private var messages: [ZavorthChatMessage]
    ...
    func createSession(key: String) throws -> ZavorthChatCreateSessionResponse {
        try Self.decode(
            CreateSessionPayload(ok: true, key: key, sessionId: "\(self.fixture.sessionIDPrefix)-\(key)"),
            as: ZavorthChatCreateSessionResponse.self)
    }

    func history(sessionKey: String) throws -> ZavorthChatHistoryPayload {
        ...
        return try Self.decode(
            HistoryPayload(
                ...
                thinkingLevel: "auto"),
            as: ZavorthChatHistoryPayload.self)
    }

    func sendMessage(sessionKey _: String, message: String, runId: String) throws -> ZavorthChatSendResponse {
        ...
        return try Self.decode(
            ...
            as: ZavorthChatSendResponse.self)
    }

    func sessions() throws -> ZavorthChatSessionsListResponse {
        let entry = ZavorthChatSessionEntry(
            ...
            thinkingDefault: "auto")
        return ZavorthChatSessionsListResponse(
            ts: Date().timeIntervalSince1970 * 1000,
            path: nil,
            count: 1,
            defaults: ZavorthChatSessionsDefaults(
                modelProvider: self.fixture.modelProvider,
                model: self.fixture.modelID,
                contextTokens: 128_000,
                thinkingLevels: Self.thinkingLevels,
    ...
    private static var thinkingLevels: [ZavorthChatThinkingLevelOption] {
        [
            ZavorthChatThinkingLevelOption(id: "auto", label: "Auto"),
            ZavorthChatThinkingLevelOption(id: "low", label: "Low"),
            ZavorthChatThinkingLevelOption(id: "medium", label: "Medium"),
            ZavorthChatThinkingLevelOption(id: "high", label: "High"),
        ]
    }

    private static func seedMessages(fixture: LocalChatFixture) -> [ZavorthChatMessage] {
        let now = Date().timeIntervalSince1970 * 1000
    ...
    private static func message(role: String, text: String, timestamp: Double) -> ZavorthChatMessage {
        ZavorthChatMessage(
            role: role,
            content: [
