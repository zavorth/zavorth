...
final class MotionService: MotionServicing {
    func activities(params: ZavorthMotionActivityParams) async throws -> ZavorthMotionActivityPayload {
        ...
        let mapped: [ZavorthMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ZavorthMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = ZavorthPedometerPayload(
