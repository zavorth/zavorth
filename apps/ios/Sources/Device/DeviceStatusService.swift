...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ZavorthDeviceStatusPayload {
        ...
        return ZavorthDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> ZavorthDeviceInfoPayload {
        ...
        return ZavorthDeviceInfoPayload(
    ...
    private func batteryStatus() -> ZavorthBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ZavorthBatteryState = switch device.batteryState {
        ...
        return ZavorthBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ZavorthThermalStatusPayload {
        let state: ZavorthThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ZavorthThermalStatusPayload(state: state)
    }
