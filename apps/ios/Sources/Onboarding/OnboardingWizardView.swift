            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `zavorth devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "zavorth devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your Zavorth chat\n"
                            + "\(requestLine)\n"
                            + "Zavorth will also retry automatically when you return to this app.")
                }
            }
    ...
    private var successStep: some View {
        VStack(spacing: 0) {
            ...
            Image(systemName: "checkmark.circle.fill")
                .font(.system(size: 64))
                .foregroundStyle(ZavorthBrand.ok)
                .padding(.bottom, 20)
...
extension OnboardingWizardView {
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your Zavorth chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "zavorth.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your Zavorth chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "zavorth.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "zavorth.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "zavorth.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
