...
private struct WatchChatPreview {
    var items: [ZavorthWatchChatItem]
...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "dev.zavorth.companion", category: "DeepLink")
    ...
    private let pushWakeLogger = Logger(subsystem: "dev.zavorth.companion", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "dev.zavorth.companion", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "dev.zavorth.companion", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "dev.zavorth.companion", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "dev.zavorth.companion", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "dev.zavorth.companion",
    ...
    func makeChatTransport() -> any ZavorthChatTransport {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ZavorthCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ZavorthCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ZavorthCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ZavorthCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ZavorthCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: ZavorthLocationMode) async -> Bool {
            ...
            check: { [weak self] in
                ...
                do {
                    let data = try await self.operatorGateway.request(
                        ...
                        paramsJSON: nil,
                        timeoutSeconds: 6)
                    guard let decoded = try? JSONDecoder().decode(ZavorthGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ZavorthNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ZavorthNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZavorthNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ZavorthLocationGetParams.self, from: req.paramsJSON)) ??
            ZavorthLocationGetParams()
        ...
        let payload = ZavorthLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(ZavorthCanvasPresentParams.self, from: req.paramsJSON)) ??
                ZavorthCanvasPresentParams()
        ...
        case ZavorthCanvasCommand.hide.rawValue:
            self.screen.hideCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ZavorthCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ZavorthCanvasNavigateParams.self, from: req.paramsJSON)
            let trimmedURL = params.url.trimmingCharacters(in: .whitespacesAndNewlines)
            self.screen.present(urlString: trimmedURL)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ZavorthCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ZavorthCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ZavorthCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ZavorthCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case ZavorthCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.zavorthA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing zavorthA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case ZavorthCanvasA2UICommand.push.rawValue, ZavorthCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [ZavorthKit.AnyCodable]
            if command == ZavorthCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ZavorthCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ZavorthCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ZavorthCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ZavorthCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.zavorthA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ZavorthCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ZavorthCameraSnapParams.self, from: req.paramsJSON)) ??
                ZavorthCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ZavorthCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ZavorthCameraClipParams.self, from: req.paramsJSON)) ??
                ZavorthCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ZavorthScreenRecordParams.self, from: req.paramsJSON)) ??
            ZavorthScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZavorthSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZavorthNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard Self.isNotificationAuthorizationAllowed(status) else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZavorthNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZavorthNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZavorthChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZavorthNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = ZavorthChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ZavorthPhotosLatestParams.self, from: req.paramsJSON)) ??
            ZavorthPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(ZavorthContactsSearchParams.self, from: req.paramsJSON)) ??
                ZavorthContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthContactsCommand.add.rawValue:
            let params = try Self.decodeParams(ZavorthContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(ZavorthCalendarEventsParams.self, from: req.paramsJSON)) ??
                ZavorthCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(ZavorthCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(ZavorthRemindersListParams.self, from: req.paramsJSON)) ??
                ZavorthRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(ZavorthRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(ZavorthMotionActivityParams.self, from: req.paramsJSON)) ??
                ZavorthMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(ZavorthPedometerParams.self, from: req.paramsJSON)) ??
                ZavorthPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
extension NodeAppModel {
    ...
    private func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([ZavorthLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            ZavorthCanvasCommand.present.rawValue,
            ZavorthCanvasCommand.hide.rawValue,
            ZavorthCanvasCommand.navigate.rawValue,
            ZavorthCanvasCommand.evalJS.rawValue,
            ZavorthCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ZavorthCanvasA2UICommand.reset.rawValue,
            ZavorthCanvasA2UICommand.push.rawValue,
            ZavorthCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([ZavorthScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([ZavorthSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([ZavorthChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ZavorthWatchCommand.status.rawValue,
            ZavorthWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([ZavorthPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            ZavorthRemindersCommand.list.rawValue,
            ZavorthRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    private func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ZavorthWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = ZavorthWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ZavorthWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(ZavorthWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = ZavorthWatchNotifyPayload(
    ...
    private func locationMode() -> ZavorthLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ZavorthLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    private func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "zavorth-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Zavorth Share Self-Test",
            url: URL(string: "https://zavorth.ai/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> ZavorthWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return ZavorthWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return ZavorthWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = ZavorthWatchExecApprovalPromptMessage(
    ...
    private func publishWatchExecApprovalResolved(
        approvalId: String,
        decision: ZavorthWatchExecApprovalDecision?,
        source: String) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = ZavorthWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: ZavorthWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = ZavorthWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = ZavorthWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    private func makeWatchChatPreview() async -> WatchChatPreview {
        do {
            let payload: ZavorthChatHistoryPayload
            if self.isAppleReviewDemoModeEnabled {
    ...
    private nonisolated static func decodeWatchChatMessage(
        _ raw: ZavorthKit.AnyCodable) -> ZavorthChatMessage?
    {
    ...
    private nonisolated static func makeWatchChatItems(
        from raw: [ZavorthKit.AnyCodable]) -> [ZavorthWatchChatItem]
    {
        var readableMessages: [(ZavorthChatMessage, String)] = []
    ...
    private nonisolated static func watchChatText(from message: ZavorthChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            ...
            if let dict = content.content?.value as? [String: ZavorthKit.AnyCodable],
    ...
    private func makeWatchAppSnapshot(
        chatPreview: WatchChatPreview? = nil) -> ZavorthWatchAppSnapshotMessage
    {
        self.pruneExpiredWatchExecApprovalPrompts()
        ...
        return ZavorthWatchAppSnapshotMessage(
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.zavorthPushKind(userInfo)
        let receivedMessage =
    ...
    private static func zavorthPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        ...
        sourceReason: String? = nil) async -> ExecApprovalResolutionOutcome
    {
        ...
        guard connected else {
            ...
            return .failed(message: "Zavorth couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: ZavorthWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "Zavorth couldn't resolve this approval right now. Try again.")
        }
    }
...
extension NodeAppModel {
    ...
    nonisolated static func _test_makeWatchChatItems(from raw: [ZavorthKit.AnyCodable]) -> [ZavorthWatchChatItem] {
