...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ZavorthContactsSearchParams) async throws -> ZavorthContactsSearchPayload {
        ...
        return ZavorthContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ZavorthContactsAddParams) async throws -> ZavorthContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ZavorthContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ZavorthContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ZavorthContactPayload {
        ZavorthContactPayload(
