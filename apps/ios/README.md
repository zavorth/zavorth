...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical Zavorth team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical Zavorth team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `dev.zavorth.companion`
...
- Local development uses the canonical `dev.zavorth.companion*` bundle IDs when the Zavorth team is available, and unique `dev.zavorth.companion.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `dev.zavorth.companion*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `ZavorthPushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.zavorth.ai`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `dev.zavorth.companion`
   - `dev.zavorth.companion.share`
   - `dev.zavorth.companion.activitywidget`
   - `dev.zavorth.companion.watchkitapp`
...
7. Expected outputs after a successful run:
   - `apps/ios/build/app-store/Zavorth-<version>.ipa`
   - `apps/ios/build/app-store/Zavorth-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Zavorth.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/ZavorthAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
...
- Recommended gateway-host storage for the APNs `.p8` file is `~/.zavorth/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `OPENCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `zavorth.json`. `OPENCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Zavorth official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Zavorth node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
