...
{
    let message = ZavorthChatMessage(
        role: role,
        content: [
            ZavorthChatMessageContent(
                type: type,
                text: text,
                mimeType: nil,
                fileName: nil,
...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    func sendNotification(id: String, params: ZavorthWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(ZavorthCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func watchAppSnapshotUsesConfiguredAgentAvatar() async throws {
        ...
        appModel.gatewayAgents = [
            AgentSummary(
                ...
                name: "Main",
                identity: [
                    "avatarUrl": AnyCodable("https://example.com/zavorth.png"),
                    "emoji": AnyCodable("OC"),
                ],
                workspace: nil,
                model: nil,
        ...
        let snapshot = try #require(watchService.lastSentAppSnapshot)
        #expect(snapshot.agentAvatarURL == "https://example.com/zavorth.png")
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: ZavorthCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: ZavorthCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func systemNotifyReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = ZavorthSystemNotifyParams(title: "Approval", body: "Review request")
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "notify-off",
            command: ZavorthSystemCommand.notify.rawValue,
    ...
    @Test @MainActor func chatPushWithoutSpeechReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = ZavorthChatPushParams(text: "Build finished", speak: false)
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "chat-push-off",
            command: ZavorthChatCommand.push.rawValue,
            paramsJSON: String(decoding: paramsData, as: UTF8.self))
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        ...
        let req = BridgeInvokeRequest(
            id: "screen",
            command: ZavorthScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: ZavorthCanvasCommand.present.rawValue)
        ...
        let navigateParams = ZavorthCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: ZavorthCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = ZavorthCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: ZavorthCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenLocalHostUnavailable() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: ZavorthCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = ZavorthCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: ZavorthCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: ZavorthWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(ZavorthWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: ZavorthWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "Zavorth")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(ZavorthWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = ZavorthWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = ZavorthWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                ZavorthWatchAction(id: "a1", label: "A1"),
                ZavorthWatchAction(id: "a2", label: "A2"),
                ZavorthWatchAction(id: "a3", label: "A3"),
                ZavorthWatchAction(id: "a4", label: "A4"),
                ZavorthWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = ZavorthWatchNotifyParams(title: "Zavorth", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func operatorAdminScopeCacheRefreshesFromStoredToken() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["ZAVORTH_STATE_DIR"]
        setenv("ZAVORTH_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("ZAVORTH_STATE_DIR", previousStateDir, 1)
            } else {
