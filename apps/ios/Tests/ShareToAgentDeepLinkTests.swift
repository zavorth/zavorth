...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalZavorthGroup() {
        #expect(ZavorthAppGroup.canonicalIdentifier == "group.dev.zavorth.companion.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected zavorth://agent deep link")
            return
        }
