...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": ZavorthLocationMode.always.rawValue,
            ...
            #expect(caps.contains(ZavorthCapability.canvas.rawValue))
            #expect(caps.contains(ZavorthCapability.screen.rawValue))
            #expect(caps.contains(ZavorthCapability.camera.rawValue))
            #expect(caps.contains(ZavorthCapability.location.rawValue))
            #expect(caps.contains(ZavorthCapability.voiceWake.rawValue))
            #expect(caps.contains(ZavorthCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": ZavorthLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(ZavorthLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(ZavorthSystemCommand.notify.rawValue))
            #expect(!commands.contains(ZavorthSystemCommand.run.rawValue))
            #expect(!commands.contains(ZavorthSystemCommand.which.rawValue))
            #expect(!commands.contains(ZavorthSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(ZavorthSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "dev.zavorth.companion.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(
                    prior,
                    service: "dev.zavorth.companion.gateway",
                    account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "dev.zavorth.companion.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "dev.zavorth.companion.gateway", account: "lastConnection")
