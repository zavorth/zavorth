...
struct ZavorthWatchApp: App {
    ...
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--zavorth-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["ZAVORTH_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: ZavorthWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if ZavorthWatchApp.isScreenshotMode {
