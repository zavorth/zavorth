...
final class ShareViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Zavorth", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        do {
            ...
            self.showStatus("Sent to Zavorth.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "ZavorthShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Zavorth is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Zavorth Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("zavorth-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
