...
export function createRuntimeOperationsPanels(options: RuntimeOperationsPanelsOptions) {
  ...
  function companionRows(companions: any[]): string {
    return companions.slice(0, 8).map((node) => {
  ...
  function memoryFactRows(): string {
    ...
    if (persistedFacts.length) {
      return persistedFacts.slice(0, 12).map((fact: any) => {
        ...
        const confidenceLabel = Number.isFinite(confidence) && confidence > 0 ? `${Math.round(confidence * 100)}%` : trustLevel;
        return `
  ...
  function bindForgetButtons(root: ParentNode) {
    root.querySelectorAll('.fact-action-btn, .fact-forget-btn').forEach((btn) => {
      btn.addEventListener('click', async (ev) => {
        ...
        const content = action === 'correct'
          ? window.prompt?.('Correct this memory fact:', button.getAttribute('data-memory-content') || factItem?.querySelector('span')?.textContent?.trim() || '')
          : '';
        ...
        try {
          if (key && (window as any).ZavorthRuntimeBridge?.memoryFactAction) {
            ...
            window.emitSignal?.('info', 'Memory action unavailable', 'No runtime memory bridge is available in this dashboard session.');
          }
          if (action === 'forget') {
  ...
  function bindMnemosRecall(root: ParentNode) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      result.innerHTML = '<div class="mnemos-recall-empty">Searching Mnemos...</div>';
      try {
        ...
        if (!hits.length) {
          result.innerHTML = '<div class="mnemos-recall-empty">No Mnemos hits for this query.</div>';
          return;
        }
        result.innerHTML = hits.map((hit: any) => `
          <article class="mnemos-recall-hit">
            <div>
              <strong>${options.escapeHtml(hit.title || hit.pageId || 'Mnemos hit')}</strong>
    ...
    if (treeContainer && !treeContainer.dataset.treeBound) {
      ...
      const nodeDetails: Record<string, string> = {
        ...
        'mem-node-recall': `
          <div class="zavorth-inspection-card">
            <h3>Mnemos recall</h3>
            <p>Local FTS5 recall over Zavorth wiki memory. No provider call is made.</p>
            <form class="mnemos-recall-form" data-mnemos-recall-form>
              <input name="query" type="search" placeholder="Search memory..." autocomplete="off">
              <button type="submit">Search</button>
            </form>
            <div class="mnemos-recall-results" data-mnemos-recall-results>
              <div class="mnemos-recall-empty">Type a query to recall local memory.</div>
            </div>
          </div>
        `,
        'mem-node-agents': `
