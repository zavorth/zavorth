...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide these Android signing properties (legacy key names) — `ZAVORTH_ANDROID_STORE_FILE`, `ZAVORTH_ANDROID_STORE_PASSWORD`, `ZAVORTH_ANDROID_KEY_ALIAS`, and `ZAVORTH_ANDROID_KEY_PASSWORD` — through your local Gradle user properties before running release tasks.
