...
import dev.zavorth.companion.ui.design.ClawErrorState
import dev.zavorth.companion.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    ZavorthSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.zavorth_logo), contentDescription = "Zavorth logo", modifier = Modifier.fillMaxSize())
    }
  }
}
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep Zavorth secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with Zavorth. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nZavorth will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "Zavorth is checking gateway and node access.",
  ),
  Failed(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      recoveryGatewayApprovalCommand(gatewayConnectionProblem)
        ...
        ?: "Gateway approval is pending. Run zavorth devices list on the gateway host, approve this phone, then retry."
      ...
      "Gateway approval is in progress. Zavorth will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun recoveryGatewayApprovalCommand(gatewayConnectionProblem: GatewayConnectionProblem...): String... {
  ...
  return if (requestId != null) {
    "zavorth devices approve $requestId"
  } else {
    "zavorth devices list"
  }
}
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Zavorth pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "Zavorth Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("Zavorth gateway diagnostic", diagnostic))
