...
private const val NOTIFICATION_CHANNEL_BASE_ID = "zavorth.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Zavorth Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Zavorth Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Zavorth Active")
      }
