...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    // Command strings come from ZavorthProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      ZavorthCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      ZavorthCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      ZavorthCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      ZavorthCanvasA2UICommand.Push.rawValue, ZavorthCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      ZavorthCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      ZavorthCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      ZavorthCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      ZavorthLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      ZavorthDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      ZavorthDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      ZavorthDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      ZavorthDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      ZavorthDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      ZavorthNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      ZavorthNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      ZavorthSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      ZavorthTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      ZavorthTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      ZavorthTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      ZavorthTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      dev.zavorth.companion.protocol.ZavorthPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      ZavorthContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      ZavorthContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      ZavorthCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      ZavorthCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      ZavorthMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      ZavorthMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      ZavorthSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      ZavorthSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      ZavorthCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
