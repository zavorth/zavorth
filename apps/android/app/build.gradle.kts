...
android {
  namespace = "dev.zavorth.companion"
  compileSdk = 36
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/ZavorthKit/Sources/ZavorthKit/Resources")
    }
  }
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "zavorth-$versionName-$buildType.apk"
          } else {
            "zavorth-$versionName-$flavorName-$buildType.apk"
          }
