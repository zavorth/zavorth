/**
 * Layout for the Zavorth capability topology map (skills, channels, agents, power, trust, product).
 * Pure geometry — no DOM.
 */

export type CapabilityDomain = 'skills' | 'channels' | 'agents' | 'power' | 'trust' | 'product';

export type CapabilityMapNodeInput = {
  id: string;
  label: string;
...
export type CapabilityMapNode = CapabilityMapNodeInput & {
  x: number;
  y: number;
  r: number;
  angle: number;
...
export type CapabilityMapEdge = {
  id: string;
  from: string;
  to: string;
};

export type CapabilityMapGraph = {
  nodes: CapabilityMapNode[];
  edges: CapabilityMapEdge[];
  width: number;
  height: number;
  cx: number;
  cy: number;
...
const DOMAIN_RING: Record<CapabilityDomain, number> = {
  trust: 0,
  product: 0,
  skills: 1,
  channels: 2,
...
export function nodeRadius(weight = 1, status?: CapabilityMapNodeInput['status']): number {
...
export function layoutCapabilityMap(
  ...
  const byDomain = new Map<CapabilityDomain, CapabilityMapNodeInput[]>();
...
export function filterCapabilityMapNodes(
  nodes: CapabilityMapNodeInput[],
  query: string,
): CapabilityMapNodeInput[] {
...
export function buildCapabilityMapFromRuntime(input: {
  ...
  const nodes: CapabilityMapNodeInput[] = [
    {
      id: 'core:trust',
      label: 'Trust',
      domain: 'trust',
