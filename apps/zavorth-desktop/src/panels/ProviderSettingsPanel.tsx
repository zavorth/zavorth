...
export function ProviderSettingsPanel() {
  ...
  const handleSave = async (data: ProviderConfigPayload) => {
    ...
    // On create, the backend generates providerId. On update, it is sent in the body or URL.
    const res = await fetch('/api/v2/providers', {
      method: 'POST',
    ...
    <div className="flex flex-col gap-4 mt-4 w-full">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Configure models and credentials securely (AES-256-GCM fallback store).
          </p>
        </div>
        <button
      ...
      {providers.length === 0 && !error ? (
        <div className="flex flex-col items-center justify-center p-12 border border-dashed border-gray-700 rounded-lg text-gray-500">
          ...
          <p className="text-sm mt-1">Add a provider to use AI.</p>
        </div>
      ) : (
            ...
            <div key={p.providerId} className="bg-gray-800 border border-gray-700 rounded-xl p-5 flex flex-col relative group">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingProvider(p); setIsModalOpen(true); }} className="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded" title="Edit">
                    <Edit2 size={16} />
                  </button>
                  <button onClick={() => handleDelete(p.providerId!)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-gray-700 rounded" title="Remove">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-gray-700 flex flex-col gap-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  {p.requiresApiKey ? (
                    p.secretRef ? (
                      <span className="text-green-400 flex items-center gap-1"><CheckCircle2 size={12} /> Configured</span>
                    ) : (
                      <span className="text-red-400 flex items-center gap-1"><XCircle size={12} /> Missing</span>
                    )
                  ) : (
