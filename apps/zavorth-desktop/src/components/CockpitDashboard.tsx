...
export function CockpitDashboard({
  ...
  if (isRuntimeReady) {
    ...
    overallMessage = 'Trusted workspace and active AI provider. The agent is ready to perform tasks.';
  } else if (!trusted) {
    overallStatus = 'error';
    ...
    overallMessage = 'No API key or AI provider has been configured. Configure a provider to start.';
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '16px' }} className="cockpit-dashboard">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <SurfaceCard title="System and Runtime Health">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <StatusBadge status={isProviderConfigured ? 'success' : 'warning'}>
                  {isProviderConfigured ? 'Ready' : 'Pending'}
                </StatusBadge>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '13px', color: '#595959' }}>AI Model</span>
          ...
          <SurfaceCard title="Controls and Security Posture">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <ActionHint message="Safe defaults: dangerous privileges are disabled by default." />
          </SurfaceCard>
        </div>

        {/* Right Column: Diagnostics, Approvals and Onboarding */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <SurfaceCard title="Approvals and Diagnostics">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '13px', color: '#595959' }}>Pending Approvals</span>
              ...
              {hasDiagnosticsWarnings && (
                <div style={{ marginTop: '8px', padding: '8px', borderRadius: '4px', backgroundColor: '#fffbe6', border: '1px solid #ffe58f' }}>
                  <span style={{ fontSize: '12px', color: '#ad6800' }}>
                    The diagnostics panel has alerts that require attention.
                  </span>
                </div>
              )}
            </div>
          ...
          <SurfaceCard title="Next Steps - Guia de Onboarding">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '13px' }}>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', opacity: trusted ? 0.6 : 1 }}>
                <span>1️⃣</span>
                <span>Choose a workspace and grant <strong>Trust</strong> permission.</span>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', opacity: isProviderConfigured ? 0.6 : 1 }}>
                <span>2️⃣</span>
                <span>Configure provider credentials in the <strong>Providers</strong> menu.</span>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', opacity: config?.defaultModelId ? 0.6 : 1 }}>
                <span>3️⃣</span>
                <span>Select the provider and the system default model.</span>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <span>4️⃣</span>
                <span>Check runtime health in the <strong>Beta Checklist</strong> tab.</span>
              </div>
            </div>
          </SurfaceCard>
        </div>
