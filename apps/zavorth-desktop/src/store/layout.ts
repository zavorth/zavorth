...
/** Imperative open request for the right rail (vibe coding loop, command center helpers). */
export type RightRailOpenRequest = {
  tab: RightRailTab;
  nonce: number;
};

export const $rightRailOpenRequest = atom<RightRailOpenRequest | null>(null);

export function setActivePanel(p: DesktopPanel) {
  $activePanel.set(p);
}
export function toggleCommandPalette() {
  $commandPaletteOpen.set(!$commandPaletteOpen.get());
}
export function setCommandPaletteOpen(o: boolean) {
  $commandPaletteOpen.set(o);
}
...
export function setInspectorOpen(o: boolean) {
  $inspectorOpen.set(o);
}
...
export function requestRightRailOpen(tab: RightRailTab): void {
  $rightRailOpenRequest.set({ tab, nonce: Date.now() });
}
