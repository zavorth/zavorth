...
} from '../src/services/UniversalSkillScaleHardeningService.js';
import type {
...
type CliOptions = UniversalSkillScaleHardeningInput & {
  json: boolean;
  requirePass: boolean;
...
async function main(): Promise<void> {
  ...
  const service = new UniversalSkillScaleHardeningService({
...
main().catch((error) => {
  console.error(`[universal-skill-scale-hardening] ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
