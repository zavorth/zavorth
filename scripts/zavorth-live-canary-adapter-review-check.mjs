...
if (asJson) {
  ...
  console.log('[zavorth-live-canary-adapter-review] checking Phase 8');
...
function ruleFilesExist() {
  ...
  return rule('live-canary-adapter-files', 'Phase 8 files exist', missing.length === 0, `${files.length - missing.length}/${files.length}`, 'contract, service, CLI, check, tests and docs are present', missing);
}
...
function ruleMarkers() {
  const checks = [
    ...
    ['src/services/ZavorthLiveCanaryExecutionAdapterReviewService.ts', ['phase-8-live-canary-execution-adapter-review', 'ZavorthUxRolloutEvidenceCanaryService', 'executionEnabled: false', 'rollback-boundary']],
  ...
  return rule('live-canary-adapter-markers', 'Phase 8 markers are wired', missing.length === 0, missing.length === 0 ? 'all markers' : `${missing.length} missing`, 'adapter review, SDK and CLI markers exist', missing);
}
...
function runNeedsEvidenceFixture() {
  ...
  return jsonRule('live-canary-needs-evidence', 'Adapter review needs lower UX evidence first', result, (snapshot) =>
    snapshot.contractVersion === '2026-05-11.live-canary-execution-adapter-review-phase-8'
...
function ruleWorkspaceCheck() {
  ...
  return rule('workspace-check-wire', 'workspace:check includes Phase 8 gate', text.includes(marker), text.includes(marker) ? 'wired' : 'missing', marker, []);
}
...
function ruleNoPublicExternalNames() {
  ...
  return rule('no-public-external-names', 'Phase 8 public core remains neutral', hits.length === 0, hits.length === 0 ? 'neutral' : `${hits.length} hit(s)`, 'no external product names in public core files', hits);
}
...
function jsonRule(id, label, result, expect) {
  ...
  try {
    ...
    return rule(id, label, passed, `status=${snapshot.status}; mode=${snapshot.mode}`, 'expected Phase 8 adapter review snapshot', passed ? [] : [JSON.stringify(snapshot, null, 2)]);
  } catch (error) {
