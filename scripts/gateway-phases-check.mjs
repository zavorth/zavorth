...
for (const selectedPhase of phases) {
  ...
  if (!tests) {
    console.error(`[gateway-check] fase invalida: ${selectedPhase}`);
    process.exit(1);
  }

  console.log(`[gateway-check] fase ${selectedPhase}: build`);
  const buildResult = runNpm(['run', 'build', '--silent']);
  if (buildResult.status !== 0) {
    process.exit(buildResult.status || 1);
  }

  console.log(`[gateway-check] fase ${selectedPhase}: testes`);
...
console.log('[gateway-check] todas as fases solicitadas passaram.');
