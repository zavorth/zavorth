...
const rules = [];
const requiredFiles = [
  'src/contracts/ZavorthDashboardVisualApprovalPackContract.ts',
  'src/services/ZavorthDashboardVisualApprovalPackService.ts',
  'scripts/zavorth-dashboard-visual-approval-pack.ts',
  'tests/services/ZavorthDashboardVisualApprovalPackService.test.ts',
];
...
if (pack) {
  ...
  assertRule('surface:pack', pack.surface === 'dashboard-visual-approval-pack', 'Visual approval surface is exposed');
...
if (asJson) {
  ...
  console.log('[dashboard-visual-approval-pack] certification');
  for (const rule of rules) {
    console.log(`[dashboard-visual-approval-pack] ${rule.status === 'passed' ? 'ok' : 'fail'} ${rule.id}: ${rule.summary}`);
  }
}
...
function runPack(args) {
  try {
    ...
    const fullArgs = process.platform === 'win32'
      ? ['/d', '/s', '/c', 'npx', 'tsx', 'scripts/zavorth-dashboard-visual-approval-pack.ts', ...args]
      : ['tsx', 'scripts/zavorth-dashboard-visual-approval-pack.ts', ...args];
