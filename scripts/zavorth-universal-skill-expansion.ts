...
} from '../src/services/UniversalSkillExpansionService.js';
import type {
...
type CliOptions = UniversalSkillExpansionInput & {
  json: boolean;
  requirePass: boolean;
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--source-id') {
      if (options.sources.length > 0) {
        options.sources[options.sources.length - 1].sourceId = String(argv[index + 1] || '').trim();
      }
      index += 1;
      continue;
    }
...
async function main(): Promise<void> {
  ...
  const service = new UniversalSkillExpansionService({
...
main().catch((error) => {
  console.error(`[universal-skill-expansion] ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
