...
function ruleFilesExist() {
  ...
  return rule('live-canary-apply-gate-files', 'Certification matrix files exist', missing.length === 0, `${missing.length}/${files.length}`, 'contract, service, CLI, check, tests and docs are present', missing);
}

function ruleMarkers() {
  const checks = [
    ['src/contracts/release/ZavorthLiveCanaryApplyGateRollbackDrillContract.ts', ['ZAVORTH_LIVE_CANARY_APPLY_GATE_ROLLBACK_DRILL_CONTRACT_VERSION', 'APPLY ZAVORTH LIVE CANARY', 'requiresSeparateLiveInvocation', 'rollbackDrillRequiredBeforeLive']],
