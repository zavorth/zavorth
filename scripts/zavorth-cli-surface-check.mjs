...
if (failures.length > 0) {
  console.error('[cli-surface] failed');
...
console.log('[cli-surface] ok: CLI tool card frame, managed profiles, wrappers, and legacy header guard passed.');
...
function checkLegacyHeaderRemoved() {
  if (fs.existsSync(files.legacyHeader)) {
    failures.push('CodexToolCardHeader.tsx should remain removed after Track 2');
  }
