...
- regenerates apps/ios/Zavorth.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical Zavorth Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${ZAVORTH_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
ZAVORTH_APP_BUNDLE_ID = dev.zavorth.companion
ZAVORTH_SHARE_BUNDLE_ID = dev.zavorth.companion.share
ZAVORTH_ACTIVITY_WIDGET_BUNDLE_ID = dev.zavorth.companion.activitywidget
ZAVORTH_WATCH_APP_BUNDLE_ID = dev.zavorth.companion.watchkitapp
ZAVORTH_CODE_SIGN_ENTITLEMENTS = Sources/ZavorthAppAttest.entitlements
ZAVORTH_APNS_ENTITLEMENT_ENVIRONMENT = production
ZAVORTH_APP_ATTEST_ENVIRONMENT = production
ZAVORTH_PUSH_MODE = appStore
ZAVORTH_PUSH_RELAY_BASE_URL =
EOF
