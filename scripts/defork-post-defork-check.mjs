...
const historicalProvenanceFiles = new Map([
  ['docs/12-third-party-integration.md', 'Status: historical provenance only'],
  ['docs/14-god-mode-remediation-architecture.md', 'Status: historical provenance only'],
]);
...
if (failures.length > 0) {
  console.error('[defork-post] failed');
...
console.log('[defork-post] ok: cross-surface test script, fixtures, shared-surface aliases, and repo-wide legacy residue policy passed.');
for (const note of notes) {
  console.log(`[defork-post] ${note}`);
}
...
function checkRepoWideLegacyResidues() {
  ...
  for (const filePath of files) {
    ...
    const isPolicyBoundary = /^scripts\/defork.*\.mjs$/.test(filePath);
  ...
  if (policyBoundaryHits > 0) {
    notes.push(`allowed ${policyBoundaryHits} legacy token definition(s) inside de-fork guard policy scripts`);
  }
}
