...
const defaultOutDir = path.join(rootDir, ".tmp", "command-center-live-composer-affordances-qa");
...
function writeReport(report: QaReport): void {
  ...
  fs.writeFileSync(path.join(report.outDir, "summary.md"), [
    "# Command Center Live Composer Affordances QA",
    "",
...
runQa(options)
  ...
  .catch((error) => {
    ...
    writeReport(report);
    console.error(`[command-center-live-composer-affordances-qa] FAIL ${error?.message || error}`);
