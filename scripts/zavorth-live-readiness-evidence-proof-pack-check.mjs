...
function filesExist() {
  ...
  return rule('files', 'Certification matrix files exist', missing.length === 0, `${missing.length}/${files.length}`, 'all files present', missing);
}
...
function runProofPackSnapshot() {
  ...
  try {
    ...
    const pass = data.contractVersion === '2026-05-14.gate-9-live-readiness-evidence-proof-pack'
      && data.status !== 'blocked'
