...
const base = read('assets/command-center/styles/base.css');
const components = read('assets/command-center/styles/components.css');
const pages = read('assets/command-center/styles/pages.css');
const chat = read('assets/command-center/styles/chat.css');
...
if (failures.length) {
  console.error('[command-center-design-system] failed');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('[command-center-design-system] passed');
