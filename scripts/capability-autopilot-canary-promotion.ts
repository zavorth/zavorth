...
function main(): void {
  try {
    ...
    const snapshot = service.buildCanaryPromotionSnapshot(source, {
      ...
      skipApprovalEnabled,
      actorId: 'phase-83-gate',
      canaryPromotionReceiptId: 'phase-83-canary-promotion',
      observationWindowId: canaryObservationComplete ? 'phase-83-observation-window' : null,
      telemetrySnapshotId: telemetryFresh ? 'phase-83-telemetry-snapshot' : null,
      incidentReviewId: 'phase-83-incident-review',
      feedbackSummaryId: 'phase-83-feedback-summary',
      nextCohortId: 'phase-83-next-cohort',
      promotionApprovalId: promotionApproved ? 'phase-83-promotion-approval' : null,
      rollbackRunbookId: rollbackRunbookReady ? 'phase-83-rollback-runbook' : null,
      observabilityReviewId: observabilityReviewReady ? 'phase-83-observability-review' : null,
      auditReceiptId: auditPersisted ? 'phase-83-audit' : null,
      reason: 'phase-83-canary-monitoring-promotion-gate',
    });
