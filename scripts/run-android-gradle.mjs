...
export function shouldSkipLinuxArmAndroidGradle(options = {}) {
  ...
  if (env.ZAVORTH_ANDROID_GRADLE_ALLOW_LINUX_ARM === "1") {
    return false;
  }
...
export function linuxArmAndroidGradleSkipMessage(platform = process.platform, arch = process.arch) {
  return (
    ...
    "Run this task on x64 Linux/macOS or set ZAVORTH_ANDROID_GRADLE_ALLOW_LINUX_ARM=1 to try anyway."
  );
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const gradleStatus = await run(gradlewCmd, gradleArgs, androidDir);
