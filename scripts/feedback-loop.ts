...
import { FeedbackTelemetryContractService } from '../src/services/FeedbackTelemetryContractService.js';
import {
  FEEDBACK_TELEMETRY_SCREENSHOTS,
  type FeedbackTelemetryScreenshotSpec,
} from '../src/contracts/FeedbackTelemetryContract.js';
...
const screenshotDir = path.join(websiteRoot, '.qa', 'feedback-loop');

async function main(): Promise<void> {
  if (argv.includes('--preview')) {
    renderFeedbackAction('preview');
    return;
  }
  if (argv.includes('--revoke')) {
    renderFeedbackAction('revoke');
    return;
  }
  if (argv.includes('--delete')) {
    renderFeedbackAction('delete');
    return;
  }
  ...
  if (shouldCaptureScreenshots) {
    await captureFeedbackScreenshots(websiteRoot, screenshotDir);
  }

  const service = new FeedbackTelemetryContractService({
    websiteRoot,
...
function renderFeedbackAction(action: 'preview' | 'revoke' | 'delete'): void {
  const payload = {
    action,
    telemetry: 'disabled-by-default',
    sendsData: false,
    previewFile: 'feedback-preview-redacted.json',
    ...
    result:
      action === 'preview'
        ? 'preview redigido gerado em fixture; nenhum envio foi realizado.'
        : action === 'revoke'
          ? 'consentimento opt-in revogado em fixture; nenhum envio foi realizado.'
          : 'delete local limitado ao ledger de feedback em fixture; nenhum arquivo fora do escopo e removido.',
  };
  ...
  process.stdout.write(`[feedback-loop] ${payload.result}\n`);
  process.stdout.write(`telemetry: ${payload.telemetry}\n`);
  process.stdout.write(`preview: ${payload.previewFile}\n`);
  process.stdout.write(`ledger: ${payload.ledger}\n`);
  process.stdout.write(`redaction: ${payload.redacted.join(', ')}\n`);
}
...
async function captureFeedbackScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    for (const screenshot of FEEDBACK_TELEMETRY_SCREENSHOTS) {
      await captureScreenshot(browser, `${server.url}feedback`, screenshot, targetDir);
    }
  } finally {
    if (browser) {
      await browser.close();
...
async function captureScreenshot(
  ...
  url: string,
  screenshot: FeedbackTelemetryScreenshotSpec,
  targetDir: string,
): Promise<void> {
...
main().catch((error) => {
  console.error('[feedback-loop] falhou:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
