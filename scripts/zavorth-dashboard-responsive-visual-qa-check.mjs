...
function main() {
  ...
  assertContains(files.css, css, '.bsk-dashboard,\n.bsk-dashboard *');
  ...
  assertContains(files.packageJson, JSON.stringify(packageJson.scripts || {}), 'zavorth:dashboard-responsive-visual-qa:check');
  assertContains('workspace:check', workspaceCheck, 'zavorth:dashboard-responsive-visual-qa:check');
  assertOrder(
    'workspace:check',
    workspaceCheck,
    'zavorth:dashboard-daily-use-polish:check',
    'zavorth:dashboard-responsive-visual-qa:check',
  );

  console.log('[dashboard-responsive-visual-qa] ok responsive visual QA guard is wired');
}
