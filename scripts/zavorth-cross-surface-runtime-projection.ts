#!/usr/bin/env tsx
import {
  ZavorthCrossSurfaceRuntimeProjectionService,
} from '../src/services/ZavorthCrossSurfaceRuntimeProjectionService.js';
import type {
...
type Args = {
  ...
  availableSurfaces: ZavorthCrossSurfaceRuntimeProjectionInput['availableSurfaces'];
...
function parseArgs(argv: string[]): Args {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === '--surfaces') out.availableSurfaces = parseAvailableSurfaces(argv[++index]);
    else if (arg.startsWith('--surfaces=')) out.availableSurfaces = parseAvailableSurfaces(arg.slice('--surfaces='.length));
...
function parseAvailableSurfaces(value: unknown): ZavorthCrossSurfaceRuntimeProjectionInput['availableSurfaces'] {
  const allowed = new Set(['files', 'web', 'browser', 'computer', 'android', 'skills', 'subagents']);
  const items = String(value || '')
    .split(',')
    .map((item) => item.trim().toLowerCase())
    .filter((item) => allowed.has(item));
  return items as ZavorthCrossSurfaceRuntimeProjectionInput['availableSurfaces'];
}
