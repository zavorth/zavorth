...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase14-15-check-signing-key-0000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-live-micro-rollout-certification:phase14-15:check',
...
try {
  ...
  for (const marker of [
    'zavorth-transaction-live-micro-rollout-certification/phase-14-15',
  ...
  const credential = runCredential([
    ...
    '--register',
    '--label',
    'Phase 14-15 exchange paper ref',
    '--connector-kind',
    'exchange',
    '--environment',
    'paper',
  ...
  if (
    ...
    failures.push('Phase 14-15 must keep live rollout and live execution disabled');
  }
  if (!certified.gates.every((gate) => gate.passed === true)) {
    failures.push('micro-rollout-certified should pass every Phase 14-15 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-live-micro-rollout-certification-phase14-15-check] failed');
  ...
  console.log('[transaction-live-micro-rollout-certification-phase14-15-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- Phase 14-15 consumes Phase 13 sandbox-executed receipts');
...
function readyPhase13Args(ref) {
  return [
    ...readyBeforeSandboxArgs(ref),
    '--sandbox-execution-confirm',
    '--sandbox-execution-phrase-default',
    '--sandbox-run-id',
    'phase14-15-sandbox-run',
  ];
}
...
function killSwitchArgs() {
  return [
    '--kill-switch-id',
    'phase14-15-kill-switch',
    '--kill-switch-enabled',
    '--kill-switch-tested',
    '--kill-switch-command',
    'zavorth transaction disable-live --scope phase14-15',
  ];
}
...
function rollbackArgs() {
  return [
    ...
    '--rollback-artifact',
    'data/runtime/phase14-15-rollback-receipt.json',
  ];
}
