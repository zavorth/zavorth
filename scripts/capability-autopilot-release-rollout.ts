...
function main(): void {
  try {
    ...
    const snapshot = service.buildRolloutPlanSnapshot(source, {
      ...
      autoRolloutEnabled,
      actorId: 'phase-81-gate',
      rolloutPlanReceiptId: 'phase-81-release-rollout-plan',
      canaryCohortId: stagedCohortsDefined ? 'phase-81-canary-cohort' : null,
      stagedCohortPlanId: stagedCohortsDefined ? 'phase-81-staged-cohorts' : null,
      rollbackRunbookId: rollbackRunbookReady ? 'phase-81-rollback-runbook' : null,
      changelogId: changelogReady ? 'phase-81-changelog' : null,
      releaseBundleId: releaseBundleReady ? 'phase-81-release-bundle' : null,
      installerSmokeReceiptId: installerSmokePassed ? 'phase-81-installer-smoke' : null,
      docsPublicationId: docsPublicationReady ? 'phase-81-docs-publication' : null,
      commsPlanId: supportCommsReady ? 'phase-81-support-comms' : null,
      telemetryDashboardId: telemetryDashboardsReady ? 'phase-81-telemetry-dashboard' : null,
      releaseOwnerId: releaseOwnerAssigned ? 'phase-81-release-owner' : null,
      releaseTrainSlotId: releaseTrainSlotReserved ? 'phase-81-release-train-slot' : null,
      artifactRetentionPolicyId: artifactRetentionReady ? 'phase-81-artifact-retention' : null,
      reason: 'phase-81-release-rollout-plan',
    });
