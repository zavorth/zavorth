...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase16-check-signing-key-0000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-live-executor-gate:phase16:check',
...
try {
  ...
  for (const marker of [
    'zavorth-transaction-live-executor-gate/phase-16',
  ...
  if (!ready.gates.every((gate) => gate.passed === true)) {
    failures.push('live-ready-held should pass every Phase 16 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-live-executor-gate-phase16-check] failed');
  ...
  console.log('[transaction-live-executor-gate-phase16-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- Phase 16 consumes Phase 14-15 micro-rollout-certified packets');
...
function killSwitchArgs() {
  return [
    ...
    '--kill-switch-tested',
    '--kill-switch-command',
    'zavorth transaction disable-live --scope phase16',
  ];
}
...
function rollbackArgs() {
  return [
    ...
    '--rollback-artifact',
    'data/runtime/phase16-rollback-receipt.json',
  ];
}
