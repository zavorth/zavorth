...
if (!snapshot?.gate || !Array.isArray(snapshot.gate.checks)) {
  throw new Error('Snapshot de estabilidade nao contem gate/checks da Fase 11.');
}

if (snapshot.gate.status === 'failed') {
  console.warn(
    `[phase-11-check] warning: gate da Fase 11 esta ${snapshot.gate.status} no ambiente atual: ${(
      snapshot.gate.blockingReasons || []
    ).join('; ')}`,
  );
}

console.log(`\n[phase-11-check] Gate da Fase 11: ${snapshot.gate.status}.`);
console.log('[phase-11-check] Fase 11 passou na validacao oficial.');
