...
import { PublicDemoContractService } from '../src/services/PublicDemoContractService.js';
import {
  PUBLIC_DEMO_SCREENSHOTS,
  type PublicDemoScreenshotSpec,
} from '../src/contracts/PublicDemoContract.js';
...
const screenshotDir = path.join(websiteRoot, '.qa', 'public-demo');
...
async function main(): Promise<void> {
  ...
  if (shouldCaptureScreenshots) {
    await captureDemoScreenshots(websiteRoot, screenshotDir);
  }

  const service = new PublicDemoContractService({
    websiteRoot,
...
async function captureDemoScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    for (const screenshot of PUBLIC_DEMO_SCREENSHOTS) {
      await captureScreenshot(browser, `${server.url}demo`, screenshot, targetDir);
    }
  } finally {
    if (browser) {
      await browser.close();
...
async function captureScreenshot(
  ...
  url: string,
  screenshot: PublicDemoScreenshotSpec,
  targetDir: string,
): Promise<void> {
...
main().catch((error) => {
  console.error('[public-demo] falhou:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
