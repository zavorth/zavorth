...
if [[ "$install_completions" -eq 1 ]]; then
  echo -e " ${CLR_INFO}ℹ Optional Shell Completions:${CLR_RESET}"
  echo -e "  To enable tab completions, run:"
  echo -e "   - ${CLR_MUTED}zavorth completions bash --install${CLR_RESET}"
  echo -e "   - ${CLR_MUTED}zavorth completions zsh --install${CLR_RESET}"
  echo -e "   - ${CLR_MUTED}zavorth completions fish --install${CLR_RESET}"
  echo ""
fi
