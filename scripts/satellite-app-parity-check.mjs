...
const rules = [
  ruleFilesExist({
    id: 'satellite-app-parity-files',
    label: 'Satellite/App parity phase files exist',
    target: 'Contract, service, tests, docs and package scripts are present',
    files: [
      'src/contracts/SatelliteAppParityContract.ts',
      'src/services/SatelliteAppParityService.ts',
      'src/satellite/satellite.js',
      'src/satellite/sw.js',
      'tests/services/SatelliteAppParityService.test.ts',
      'docs/README.md',
      'docs/README.md',
      '.gitignore',
      'package.json',
  ...
  ruleContainsAll({
    id: 'satellite-app-parity-contract',
    label: 'Contract defines Satellite/App parity vocabulary',
    ...
    files: ['src/contracts/SatelliteAppParityContract.ts'],
    needles: [
      'ZAVORTH_SATELLITE_APP_PARITY_CONTRACT_VERSION',
  ...
  ruleContainsAll({
    id: 'satellite-app-parity-service',
  ...
  ruleContainsAll({
    id: 'satellite-app-parity-tests',
    label: 'Tests prove Satellite/App parity behavior',
    ...
    files: ['tests/services/SatelliteAppParityService.test.ts'],
    needles: [
  ...
  ruleContainsAll({
    id: 'package-exposes-satellite-app-parity-gate',
    label: 'package exposes Satellite/App parity gate',
    target: 'local QA can run satellite-app-parity:check and qa:satellite-app-parity',
    files: ['package.json'],
    needles: [
      'satellite-app-parity:check',
      'qa:satellite-app-parity',
      'scripts/satellite-app-parity-check.mjs',
    ],
  }),
];
...
if (asJson) {
  ...
  console.log('[satellite-app-parity] checking Runtime gateway');
  for (const rule of rules) {
    const marker = rule.status === 'passed' ? 'ok' : 'fail';
    console.log(`[satellite-app-parity] ${marker} ${rule.label}: ${rule.observed} | ${rule.target}`);
