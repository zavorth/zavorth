...
export function runMaturityProductionReadiness(options = {}) {
  ...
  checks.push(checkPhaseDocComplete(root));

  // --- Required: React island sources ---
  checks.push(checkFilesExist(root, {
 ...
 * @returns {CheckResult}
 */
function checkPhaseDocComplete(root) {
  const rel = 'docs/product/ZAVORTH-MATURITY-PHASES.md';
  ...
  const ok = statusLineOk || tablePhasesDone;
  return {
    id: 'phase-doc-status',
    required: true,
    status: ok ? 'pass' : 'fail',
    summary: ok
      ? 'Phase doc reports Phase 8 DONE / phases 1–8 complete'
