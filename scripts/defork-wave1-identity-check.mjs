...
const requiredFiles = [
  ...
  {
    path: 'src/ai-gateway/app/(dashboard)/dashboard/BootstrapBanner.tsx',
    includes: [
  ...
  {
    path: 'src/ai-gateway/app/(dashboard)/dashboard/HomePageClient.tsx',
    includes: [
      'Zavorth Control Plane',
      'operator surface',
      'Open provider arena',
...
for (const entry of requiredFiles) {
  ...
  for (const expected of entry.includes) {
    if (!content.includes(expected)) {
      failures.push(`${entry.path}: missing Wave 1 identity marker "${expected}"`);
    }
  }
}
...
if (failures.length > 0) {
  console.error('[defork-wave1] failed');
...
console.log('[defork-wave1] ok: visible identity surfaces are Zavorth-native and critical shell markers are present.');
