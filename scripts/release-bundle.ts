...
import { PublicReleaseBundleContractService } from '../src/services/PublicReleaseBundleContractService.js';
import {
  PUBLIC_RELEASE_BUNDLE_SCREENSHOTS,
  type PublicReleaseBundleScreenshotSpec,
} from '../src/contracts/PublicReleaseBundleContract.js';
...
const screenshotDir = path.join(websiteRoot, '.qa', 'release-bundle');
...
async function main(): Promise<void> {
  ...
  if (shouldCaptureScreenshots) {
    await captureReleaseBundleScreenshots(websiteRoot, screenshotDir);
  }

  const service = new PublicReleaseBundleContractService({
    websiteRoot,
...
async function captureReleaseBundleScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    for (const screenshot of PUBLIC_RELEASE_BUNDLE_SCREENSHOTS) {
      await captureScreenshot(browser, `${server.url}release`, screenshot, targetDir);
    }
  } finally {
    if (browser) {
      await browser.close();
...
async function captureScreenshot(
  ...
  url: string,
  screenshot: PublicReleaseBundleScreenshotSpec,
  targetDir: string,
): Promise<void> {
...
main().catch((error) => {
  console.error('[release-bundle] failed:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
