...
} from "../src/ai-gateway/app/(dashboard)/control/command-center/adapters/zavorthAgentGatewayCommandCenterAdapter";

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const defaultOutDir = path.join(rootDir, ".tmp", "command-center-real-flow-qa");
...
async function runQa(options: CliOptions): Promise<QaReport> {
  const report: QaReport = {
    ...
    scenario: "command-center-real-approval-artifact-history",
  ...
  const executor: UniversalAgentExecutor = ({ run }) => ({
    ...
    events: [
      ...
      },
      {
        ...
        detail: "Command Center recebeu a conclusão da run.",
        status: "done",
      },
    ],
    artifacts: [
  ...
  pushStep(
    ...
    "Artifact real fica disponível no ViewModel do Command Center.",
  );
  pushStep(
    report,
    "history-and-replay-visible",
    completedViewModel.sessions.some((session) => session.id === "session-command-center-real-qa")
...
runQa(options)
  .then((report) => {
    ...
    console.log(`[command-center-real-flow-qa] ${status} ${path.join(report.outDir, "visual-checklist.md")}`);
  ...
  .catch((error) => {
    ...
    writeReport(report);
    console.error(`[command-center-real-flow-qa] FAIL ${error?.message || error}`);
