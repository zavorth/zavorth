...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase10-check-signing-key-000000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-live-candidate:phase10:check',
...
try {
  ...
  for (const marker of [
    'zavorth-transaction-live-candidate/phase-10',
  ...
  if (!ready.gates.every((gate) => gate.passed === true)) {
    failures.push('candidate-ready should pass every Phase 10 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-live-candidate-phase10-check] failed');
  ...
  console.log('[transaction-live-candidate-phase10-check] ok');
