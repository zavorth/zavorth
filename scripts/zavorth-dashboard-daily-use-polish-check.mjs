...
const order = [
  '<DashboardActiveMissionPanel',
  '<DashboardApprovalsPanel',
  '<DashboardSensitiveActionFlowPanel',
  '<DashboardVisualReceiptsPanel',
  '<DashboardRunPanel',
  '<DashboardDoctorPanel',
  '<DashboardProviderCockpitPanel',
  '<DashboardProviderPreferencePanel',
];
...
rules.push(
  ...
  {
    id: 'workspace:gate',
    status: packageJson.includes('zavorth:dashboard-daily-use-polish:check') ? 'passed' : 'failed',
...
console.log('[dashboard-daily-use-polish] certification');
for (const rule of rules) {
  console.log(`[dashboard-daily-use-polish] ${rule.status === 'passed' ? 'ok' : 'fail'} ${rule.id}: ${rule.summary}`);
}

if (failed.length > 0) {
  console.error(`[dashboard-daily-use-polish] failed rules: ${failed.map((rule) => rule.id).join(', ')}`);
  process.exit(1);
}
