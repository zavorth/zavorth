...
import { ExternalDocsContractService } from '../src/services/ExternalDocsContractService.js';
import {
  EXTERNAL_DOCS_SCREENSHOTS,
  type ExternalDocsScreenshotSpec,
} from '../src/contracts/ExternalDocsContract.js';
...
const screenshotDir = path.join(websiteRoot, '.qa', 'external-docs');
...
async function main(): Promise<void> {
  ...
  if (shouldCaptureScreenshots) {
    await captureExternalDocsScreenshots(websiteRoot, screenshotDir);
  }

  const service = new ExternalDocsContractService({
    websiteRoot,
...
async function captureExternalDocsScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    for (const screenshot of EXTERNAL_DOCS_SCREENSHOTS) {
      const route = screenshot.route.replace(/^\//, '');
      await captureScreenshot(browser, `${server.url}${route}`, screenshot, targetDir);
    }
  } finally {
    if (browser) {
      await browser.close();
...
async function captureScreenshot(
  ...
  url: string,
  screenshot: ExternalDocsScreenshotSpec,
  targetDir: string,
): Promise<void> {
