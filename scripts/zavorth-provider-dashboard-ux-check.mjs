...
const checks = [
  ...
  [files.operationsPanel, 'DashboardProviderPreferencePanel'],
  ...
  [files.packageJson, 'zavorth:provider-dashboard-ux:check'],
];

const failures = checks.filter(([file, needle]) => !read(file).includes(needle));
if (failures.length > 0) {
  console.error('Provider Dashboard UX check failed:');
...
if (operationsPanel.includes('fetch(')) {
  console.error('Provider Dashboard UX check failed: operations panel must not fetch provider data directly.');
  process.exit(1);
}
if (operationsPanel.includes('/api/providers') || operationsPanel.includes('/api/gateway-control/providers/test')) {
  console.error('Provider Dashboard UX check failed: operations panel must not call provider APIs directly.');
  process.exit(1);
}

console.log('[provider-dashboard-ux] ok');
