...
if (vitest.status !== 0) {
  fail('desktop product unit tests failed');
  console.error(vitest.stdout || '');
  console.error(vitest.stderr || '');
} else {
  ok('desktop product unit tests passed');
  const tail = (vitest.stdout || '').split('\n').slice(-8).join('\n');
  if (tail.trim()) console.log(tail);
}

const scan = [cost, graph, exportPanel, vibe, analytics].join('\n');
const foreignBrand = ['her', 'mes'].join('');
const foreignBrandPattern = new RegExp(`${foreignBrand}-(?:example|plugin)|@${foreignBrand}/`, 'i');
if (foreignBrandPattern.test(scan)) {
  fail('foreign branding found in product surfaces');
} else {
  ok('no foreign branding in product surfaces');
}

if (failed > 0) {
  console.error(`\nDesktop product dogfood FAILED (${failed})`);
  process.exit(1);
}
console.log('\nDesktop product dogfood OK');
