...
const defaultOutDir = path.join(rootDir, ".tmp", "command-center-provider-cockpit-live-smoke");
...
async function startInProcessServer(): Promise<{ server: http.Server; baseUrl: string }> {
  const providerCockpitService = new ZavorthCommandCenterProviderCockpitService();
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === "/api/web/command-center") {
        const providerCockpit = await providerCockpitService.buildProjection({
...
async function main(): Promise<SmokeReport> {
  ...
  const commandCenter = await readJson(`${baseUrl}/api/web/command-center`);
  debug(`commandCenter=${commandCenter.status}`);

  const providerCockpit = readiness.payload?.providerCockpit || commandCenter.payload?.snapshot?.providerCockpit || null;
  const commandCenterProviderCockpit = commandCenter.payload?.snapshot?.providerCockpit || null;
  ...
  const report: SmokeReport = {
    ...
    commandCenter: {
      apiLive: commandCenter.payload?.live === true,
      snapshotHasProviderCockpit: Boolean(commandCenterProviderCockpit),
      providerCountLabel: commandCenterProviderCockpit
        ? `${commandCenterProviderCockpit.summary.readyProviders}/${commandCenterProviderCockpit.summary.totalProviders} ready`
        : "missing",
    },
    checks: [],
  };
  ...
  pushCheck(report, "command-center-endpoint-ok", commandCenter.status === 200 && commandCenter.payload?.ok === true, "GET /api/web/command-center returns a runtime snapshot.");
  ...
  pushCheck(report, "provider-cockpit-attached", Boolean(providerCockpit?.surface === "command-center-provider-cockpit"), "Provider Cockpit projection is attached to live API output.");
  ...
  pushCheck(report, "command-center-snapshot-consumes-provider-cockpit", Boolean(commandCenterProviderCockpit), "Command Center live API snapshot includes Provider Cockpit.");
  pushCheck(report, "no-secret-like-values", !containsSecretLikeText({ readiness: readiness.payload, commandCenter: commandCenter.payload }), "Live smoke output contains no raw provider secret patterns.");
...
main()
  ...
  .catch((error) => {
    console.error(`[command-center-provider-cockpit-live-smoke] FAIL ${error?.message || error}`);
