...
function main(): void {
  try {
    ...
    const snapshot = service.buildExecutionSnapshot(source, {
      ...
      skipCanaryEnabled,
      actorId: 'phase-82-gate',
      executionGateReceiptId: 'phase-82-release-execution',
      versionManifestId: versionManifestReady ? 'phase-82-version-manifest' : null,
      tagApprovalReceiptId: tagCreationApproved ? 'phase-82-tag-approval' : null,
      publishApprovalReceiptId: publishApproved ? 'phase-82-publish-approval' : null,
      artifactVerificationReceiptId: releaseBundleVerified ? 'phase-82-artifact-verification' : null,
      provenanceReceiptId: provenanceReady ? 'phase-82-provenance' : null,
      canaryLaunchReceiptId: canaryLaunchApproved ? 'phase-82-canary-launch' : null,
      smokeReceiptId: smokeBeforeCanaryPassed ? 'phase-82-pre-canary-smoke' : null,
      rollbackCheckpointId: rollbackCheckpointReady ? 'phase-82-rollback-checkpoint' : null,
      rollbackDryRunReceiptId: rollbackDryRunPassed ? 'phase-82-rollback-dry-run' : null,
      observabilityDashboardId: observabilityLive ? 'phase-82-observability' : null,
      incidentCommanderId: incidentCommanderAssigned ? 'phase-82-incident-commander' : null,
      supportBridgeId: supportBridgeReady ? 'phase-82-support-bridge' : null,
      auditReceiptId: auditSinkReady ? 'phase-82-audit' : null,
      reason: 'phase-82-release-execution-gate',
    });
