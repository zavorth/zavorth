...
for (const file of [
  'src/contracts/ZavorthCliExperienceCertificationContract.ts',
  'src/services/ZavorthCliExperienceCertificationService.ts',
  'scripts/zavorth-cli-experience-certification.ts',
...
const output = execFileSync(
  runner,
  [...prefix, 'tsx', 'scripts/zavorth-cli-experience-certification.ts', '--json'],
  { cwd: root, encoding: 'utf8' },
);
const snapshot = JSON.parse(output);

if (snapshot.surface !== 'cli-experience-consistency') {
...
if (snapshot.safety.cliCanExecuteTargetAction !== false) {
  throw new Error('cli experience consistency must not execute target actions');
}
...
execFileSync(
  runner,
  [...prefix, 'jest', '--runTestsByPath', 'tests/services/ZavorthCliExperienceCertificationService.test.ts', '--runInBand'],
  { cwd: root, stdio: 'inherit' },
);

console.log('[zavorth-cli-experience-certification-check] ok');
