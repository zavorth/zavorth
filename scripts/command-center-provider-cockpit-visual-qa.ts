...
const outDir = path.join(rootDir, ".tmp", "command-center-provider-cockpit-visual-qa");
...
function generatePreview(): string {
  ...
  const result = spawnSync(
    ...
    [
      ...
      path.join(rootDir, "scripts", "command-center-browser-preview.ts"),
      "--fixture=safe-run",
      `--out=${outDir}`,
    ],
    {
...
async function inspectViewport(
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      safeRenderText: text.includes("sem chamadas de rede no dashboard") ? "safe-render" : "",
...
main()
  ...
  .catch((error) => {
    console.error(`[command-center-provider-cockpit-visual-qa] FAIL ${error?.message || error}`);
