...
} from '../src/services/UniversalSkillRealSourceOnboardingService.js';
import type {
...
type CliOptions = UniversalSkillRealSourceOnboardingInput & {
  json: boolean;
  requirePass: boolean;
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--max-history') {
      options.maxHistoryEntries = Number(argv[index + 1]);
      index += 1;
      continue;
    }
    if (arg === '--apply') {
...
async function main(): Promise<void> {
  ...
  const service = new UniversalSkillRealSourceOnboardingService({
...
main().catch((error) => {
  console.error(`[universal-skill-real-source-onboarding] ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
