...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase4-check-signing-key-000000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-connector:phase4:check',
...
try {
  ...
  for (const marker of ['zavorth-transaction-connector/phase-4', 'externalSideEffects: false', 'supportsLive: false', 'approval_grant_required']) {
  ...
  if (failures.length > 0) {
    console.error('[transaction-connector-phase4-check] failed');
  ...
  console.log('[transaction-connector-phase4-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- real-money connector dry-run requires Phase 3 approval');
