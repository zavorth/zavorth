...
const rules = [
  ruleFilesExist({
    id: 'openshell-sandbox-certification-files',
    label: 'OpenShell sandbox consistency files exist',
    ...
    files: [
      ...
      'scripts/openshell-sandbox-certification-check.mjs',
      'docs/README.md',
      'package.json',
    ],
  }),
  ...
  ruleContainsAll({
    ...
    target: 'Service covers 11 OpenShell consistency features and keeps mirror-back artifact-first',
  ...
  ruleContainsAll({
    ...
    label: 'Tests prove OpenShell sandbox consistency behavior',
  ...
  ruleContainsAll({
    ...
    label: 'package exposes OpenShell sandbox consistency gates',
    target: 'local QA can run openshell-sandbox-certification check',
    files: ['package.json'],
    needles: [
      'openshell-sandbox-certification:check',
      'qa:openshell-sandbox-certification',
      'scripts/openshell-sandbox-certification-check.mjs',
    ],
  }),
];
...
if (asJson) {
  ...
  console.log('[openshell-sandbox-certification] checking Worker 3');
  for (const rule of rules) {
    const marker = rule.status === 'passed' ? 'ok' : 'fail';
    console.log(`[openshell-sandbox-certification] ${marker} ${rule.label}: ${rule.observed} | ${rule.target}`);
