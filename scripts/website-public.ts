...
import { WebsitePublicContractService } from '../src/services/WebsitePublicContractService.js';
import {
  WEBSITE_PUBLIC_SCREENSHOTS,
  type WebsitePublicScreenshotSpec,
} from '../src/contracts/WebsitePublicContract.js';
...
const shouldBuild = buildOnly || argv.includes('--build') || requirePass;
...
const screenshotDir = path.join(websiteRoot, '.qa', 'website-public');
...
async function main(): Promise<void> {
  ...
  if (shouldCaptureScreenshots) {
    await captureWebsiteScreenshots(websiteRoot, screenshotDir);
  }

  const service = new WebsitePublicContractService({
    websiteRoot,
...
async function captureWebsiteScreenshots(root: string, targetDir: string): Promise<void> {
  const outRoot = path.join(root, 'out');
  if (!fs.existsSync(outRoot)) {
    throw new Error(`export estatico ausente em ${outRoot}; rode website:build antes dos screenshots`);
  }
  ...
  try {
    ...
    for (const screenshot of WEBSITE_PUBLIC_SCREENSHOTS) {
...
async function captureScreenshot(
  ...
  baseUrl: string,
  screenshot: WebsitePublicScreenshotSpec,
  targetDir: string,
): Promise<void> {
...
main().catch((error) => {
  console.error('[website-public] falhou:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
