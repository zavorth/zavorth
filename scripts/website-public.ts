...
async function captureWebsiteScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  if (!fs.existsSync(outRoot)) {
    throw new Error(`export estatico missing em ${outRoot}; run website:build before dos screenshots`);
  }
...
main().catch((error) => {
  console.error('[website-public] failed:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
