...
const rules = [
  ruleFilesExist({
    ...
    files: [
      ...
      'src/cli/ZavorthCliProviderMeshConsolidationRenderer.ts',
      'tests/runtime/agent/ProviderMeshConsolidationService.test.ts',
      'tests/runtime/agent/AgentRunServiceProviderMeshConsolidation.test.ts',
      'tests/cli/ZavorthCliProviderMeshConsolidation.test.ts',
      'tests/zavorth-control/zavorthControl/ZavorthControlProviderMeshConsolidation.test.ts',
      'docs/README.md',
    ],
  }),
  ruleContainsAll({
    id: 'provider-mesh-contract',
    label: 'Provider Mesh contract consolidates P0-extra',
    target: 'ProviderMeshConsolidationSnapshot links registry, catalog, routes, picker, selection, onboarding and ProviderFactory',
  ...
  ruleContainsAcross({
    id: 'agent-run-uses-provider-mesh',
    label: 'Agent run publishes Provider Mesh consolidation',
    target: 'AgentRunService writes run.metadata.providerMeshConsolidation and exports the contract',
    files: [
    ...
    needles: [
      ...
      'applyProviderMeshConsolidation',
      'PROVIDER_MESH_CONSOLIDATION_CONTRACT_VERSION',
    ],
  }),
  ruleContainsAcross({
    id: 'cli-exposes-provider-mesh',
    label: 'CLI exposes Provider Mesh consolidation',
    target: 'zavorth provider-mesh renders picker/route/onboarding state in text or JSON',
    files: [
  ...
  ruleContainsAcross({
    id: 'zavorthControl-projects-provider-mesh',
    label: 'ZavorthControl projects Provider Mesh consolidation',
    target: '/zavorthControl reads providerMeshConsolidation from run metadata and renders overview/config surfaces',
    files: [
    ...
    needles: [
      'ZavorthControlProviderMeshConsolidationSnapshot',
      'providerMeshConsolidation',
      'buildProviderMeshConsolidation',
      'mapProviderMeshConsolidation',
      'Provider Mesh',
      'summary.readyRouteCount',
    ],
  }),
  ruleContainsAll({
    ...
    needles: [
      'provider-mesh:check',
      'qa:provider-mesh',
      'scripts/provider-mesh-check.mjs',
    ],
  }),
];
