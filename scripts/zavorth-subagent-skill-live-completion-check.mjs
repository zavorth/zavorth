...
function filesExist() {
  const files = [
    'src/contracts/ZavorthSubagentSkillLiveCompletionContract.ts',
    'src/services/ZavorthSubagentSkillLiveCompletionService.ts',
    'scripts/zavorth-subagent-skill-live-completion.ts',
    'scripts/zavorth-subagent-skill-live-completion-check.mjs',
...
function packageScriptsWired() {
  ...
  const markers = [
    'zavorth:subagent-skill-live-completion',
    'zavorth:subagent-skill-live-completion:json',
    'zavorth:subagent-skill-live-completion:check',
  ];
...
function runCompletionSnapshot() {
  ...
  try {
    ...
    const pass = data.contractVersion === '2026-05-14.gate-6-subagent-skill-live-completion'
      ...
      && data.safety?.noLiveSkillCodeExecutionByDefault === true
      && data.summary?.rawSecretsSerialized === false;
    return rule('snapshot', 'Completion snapshot runs', pass, `status=${data.status}; entries=${data.summary?.entries}`, 'subagents/skills completion without unsafe defaults', pass ? [] : [JSON.stringify(data, null, 2)]);
  } catch (error) {
