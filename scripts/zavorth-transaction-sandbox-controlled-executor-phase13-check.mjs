...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase13-check-signing-key-000000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-sandbox-controlled-executor:phase13:check',
...
try {
  ...
  for (const marker of [
    'zavorth-transaction-sandbox-controlled-executor/phase-13',
  ...
  if (executed.executionReceipt?.sandboxExternalIoPerformed !== false || executed.executionReceipt?.liveExecutionAuthorized !== false || executed.safety.noExternalNetworkCall !== true) {
    failures.push('Phase 13 receipt must keep external and live execution disabled');
  }
  if (!executed.gates.every((gate) => gate.passed === true)) {
    failures.push('sandbox-executed should pass every Phase 13 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-sandbox-controlled-executor-phase13-check] failed');
  ...
  console.log('[transaction-sandbox-controlled-executor-phase13-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- Phase 13 consumes Phase 12 sandbox-certification-ready packets');
...
function killSwitchArgs() {
  return [
    ...
    '--kill-switch-tested',
    '--kill-switch-command',
    'zavorth transaction disable-live --scope phase13',
  ];
}
...
function rollbackArgs() {
  return [
    ...
    '--rollback-artifact',
    'data/runtime/phase13-rollback-receipt.json',
  ];
}
