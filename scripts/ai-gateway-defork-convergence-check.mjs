...
const rules = [
  ruleFilesExist({
    id: 'c8-defork-convergence-service',
    label: 'C8 de-fork convergence service exists',
    target: 'AI Gateway convergence is evaluated through a Zavorth-native snapshot',
    files: [
      'src/services/AIGatewayDeforkConvergenceService.ts',
      'tests/services/AIGatewayDeforkConvergenceService.test.ts',
    ],
  }),
  ruleContainsAll({
...
if (asJson) {
  ...
  console.log('[ai-gateway-defork-convergence] checking C8 convergence');
  for (const rule of rules) {
    const marker = rule.status === 'passed' ? 'ok' : 'fail';
    console.log(`[ai-gateway-defork-convergence] ${marker} ${rule.label}: ${rule.observed} | ${rule.target}`);
