...
const rules = [];
const requiredFiles = [
  'src/contracts/ZavorthCommandCenterVisualApprovalPackContract.ts',
  'src/services/ZavorthCommandCenterVisualApprovalPackService.ts',
  'scripts/zavorth-command-center-visual-approval-pack.ts',
  'tests/services/ZavorthCommandCenterVisualApprovalPackService.test.ts',
];
...
if (asJson) {
  ...
  console.log('[command-center-visual-approval-pack] certification');
  for (const rule of rules) {
    console.log(`[command-center-visual-approval-pack] ${rule.status === 'passed' ? 'ok' : 'fail'} ${rule.id}: ${rule.summary}`);
  }
}
...
function runPack(args) {
  try {
    ...
    const fullArgs = process.platform === 'win32'
      ? ['/d', '/s', '/c', 'npx', 'tsx', 'scripts/zavorth-command-center-visual-approval-pack.ts', ...args]
      : ['tsx', 'scripts/zavorth-command-center-visual-approval-pack.ts', ...args];
