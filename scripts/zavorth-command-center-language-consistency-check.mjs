...
const files = {
  preview: 'scripts/command-center-browser-preview.ts',
...
function main() {
  ...
  assertContains(files.preview, preview, 'const normalizeCommandCenterCopy = (value) => {');
  assertContains(files.preview, preview, 'const normalizeVisibleCommandCenterCopy = (root) => {');
  ...
  assertContains(files.preview, preview, 'normalizeVisibleCommandCenterCopy(document.getElementById("command-center-preview-root"))');
  assertOrder(
    files.preview,
    preview,
  ...
  assertContains(files.packageJson, JSON.stringify(scripts), 'zavorth:command-center-language-consistency:check');
  assertContains(files.packageJson, JSON.stringify(scripts), 'qa:zavorth-command-center-language-consistency');
  assertContains('workspace:check', workspaceCheck, 'zavorth:command-center-language-consistency:check');
  assertOrder(
    'workspace:check',
    workspaceCheck,
    'zavorth:command-center-responsive-visual-qa:check',
    'zavorth:command-center-language-consistency:check',
  );

  console.log('[command-center-language-consistency] ok Command Center visible copy normalization is wired');
}
