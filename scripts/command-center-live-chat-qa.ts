...
const defaultOutDir = path.join(rootDir, ".tmp", "command-center-live-chat-qa");
  ...
  try {
    ...
    try {
      ...
      if (options.requireLive) {
        pushCheck(report, "live-server-reachable", false, `Nao consegui abrir o Command Center real: ${String(error?.message || error)}`);
      } else {
...
runQa(options)
  ...
  .catch((error) => {
    ...
    writeReport(report);
    console.error(`[command-center-live-chat-qa] FAIL ${error?.message || error}`);
