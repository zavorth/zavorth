...
const rules = [
  ruleFilesExist({
    id: 'codex-runtime-certification-files',
    label: 'Codex runtime certification files exist',
    ...
    files: [
      ...
      'scripts/codex-runtime-certification-check.mjs',
      'docs/README.md',
      'package.json',
    ],
  }),
  ...
  ruleContainsAll({
    ...
    target: 'Service covers 14 app-server consistency features, no-live-IO policy and next worker handoff',
    ...
    needles: [
      ...
      'Worker 3 - OpenShell Sandbox Plane consistency',
    ],
  }),
  ruleContainsAll({
  ...
  ruleContainsAll({
    ...
    label: 'Tests prove Codex runtime certification behavior',
    target: 'Tests cover snapshot closure, normalization target, run plan, media/migration, mocked RPC and redacted transports',
    files: ['tests/services/CodexRuntimePlaneService.test.ts'],
    needles: [
      'closes Codex app-server runtime certification as Zavorth-native proof',
  ...
  ruleContainsAll({
    ...
    label: 'package exposes Codex runtime certification gates',
    target: 'local QA can run codex-runtime-certification check',
    files: ['package.json'],
    needles: [
      'codex-runtime-certification:check',
      'qa:codex-runtime-certification',
      'scripts/codex-runtime-certification-check.mjs',
    ],
  }),
];
...
if (asJson) {
  ...
  console.log('[codex-runtime-certification] checking Worker 2');
  for (const rule of rules) {
    const marker = rule.status === 'passed' ? 'ok' : 'fail';
    console.log(`[codex-runtime-certification] ${marker} ${rule.label}: ${rule.observed} | ${rule.target}`);
