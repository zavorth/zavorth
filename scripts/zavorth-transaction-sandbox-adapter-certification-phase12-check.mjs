...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'phase12-check-signing-key-000000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-sandbox-adapter-certification:phase12:check',
...
try {
  ...
  for (const marker of [
    'zavorth-transaction-sandbox-adapter-certification/phase-12',
  ...
  if (ready.certificationPacket?.sandboxExecutionAuthorized !== false || ready.certificationPacket?.liveExecutionAuthorized !== false || ready.safety.noSandboxNetworkCall !== true) {
    failures.push('Phase 12 packet must keep sandbox and live execution disabled');
  }
  if (!ready.gates.every((gate) => gate.passed === true)) {
    failures.push('sandbox-certification-ready should pass every Phase 12 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-sandbox-adapter-certification-phase12-check] failed');
  ...
  console.log('[transaction-sandbox-adapter-certification-phase12-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- Phase 12 consumes Phase 11 review-ready packets');
...
function killSwitchArgs() {
  return [
    ...
    '--kill-switch-tested',
    '--kill-switch-command',
    'zavorth transaction disable-live --scope phase12',
  ];
}
...
function rollbackArgs() {
  return [
    ...
    '--rollback-artifact',
    'data/runtime/phase12-rollback-receipt.json',
  ];
}
