...
if (failures.length) {
  console.error('[wave3-launch] failed:');
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}
console.log('[wave3-launch] ok (honesty gate; not a public launch announcement)');
for (const w of warnings) console.log(`[wave3-launch] note: ${w}`);
