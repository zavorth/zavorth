...
if (asJson) {
  ...
  console.log('[zavorth-temporal-autonomy-daily-use-certification] checking Phase 8');
...
function ruleFilesExist() {
  ...
  return rule('phase-8-files', 'Phase 8 files exist', missing.length === 0, `${files.length - missing.length}/${files.length}`, 'contract, service, CLI, check, tests and docs are present', missing);
}

function ruleMarkers() {
  const checks = [
    ['src/contracts/ZavorthTemporalAutonomyDailyUseCertificationContract.ts', ['temporal-autonomy-daily-use-certification-phase-8', 'scheduled_tasks', 'acp_bridge', 'channel_without_button_fallback']],
  ...
  return rule('phase-8-markers', 'Phase 8 markers are wired', missing.length === 0, missing.length === 0 ? 'all markers' : `${missing.length} missing`, 'contract, service, CLI and SDK markers exist', missing);
}

function runFixture() {
  const result = runTs(['--json', '--now=2026-05-12T10:00:00.000Z']);
  return jsonRule('phase-8-fixture', 'Daily-use certification passes all matrix and abuse gates', result, (snapshot) =>
...
function ruleAbuseCoverage() {
  const result = runTs(['--json', '--now=2026-05-12T10:00:00.000Z']);
  if (!result.stdout.trim()) return rule('phase-8-abuse-coverage', 'Abuse scenarios are represented', false, `exit ${result.status ?? 'unknown'}`, 'valid JSON fixture', compact(result.stderr));
  try {
    ...
    return rule('phase-8-abuse-coverage', 'Abuse scenarios are represented', missing.length === 0 && unsafe.length === 0, `${ids.length}/${expected.length}; unsafe=${unsafe.length}`, 'all Phase 8 abuse scenarios present and safe', [...missing, ...unsafe.map((entry) => JSON.stringify(entry))]);
  } catch (error) {
    return rule('phase-8-abuse-coverage', 'Abuse scenarios are represented', false, 'invalid JSON', 'valid JSON fixture', [String(error), ...compact(result.stderr, result.stdout)]);
  }
}
...
function ruleWorkspaceCheck() {
  ...
  return rule('workspace-check-wire', 'workspace:check includes Phase 8 daily-use gate', text.includes(marker), text.includes(marker) ? 'wired' : 'missing', marker, []);
}
...
function ruleNoPublicExternalNames() {
  ...
  return rule('no-public-external-names', 'Phase 8 public core remains neutral', hits.length === 0, hits.length === 0 ? 'neutral' : `${hits.length} hit(s)`, 'no external product names in new public core files', hits);
}
...
function jsonRule(id, label, result, expect) {
  ...
  try {
    ...
    return rule(id, label, passed, `status=${snapshot.status}; matrix=${snapshot.summary?.passedMatrixAreas}/${snapshot.summary?.matrixAreas}; abuseFailed=${snapshot.summary?.failedAbuseScenarios}`, 'expected Phase 8 certification snapshot', passed ? [] : [JSON.stringify(snapshot, null, 2), ...compact(result.stderr)]);
  } catch (error) {
