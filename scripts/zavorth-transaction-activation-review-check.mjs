...
const env = {
  ...process.env,
  ZAVORTH_TRANSACTION_APPROVAL_SIGNING_KEY: 'intent-model1-check-signing-key-000000000000000000000000000000',
};
...
const requiredPackageScripts = [
  ...
  'zavorth:transaction-live-activation-review:check',
...
try {
  ...
  const contractText = readFileSync(
    join(root, 'src/contracts/ZavorthTransactionLiveActivationReviewContract.ts'),
    'utf8',
  );
  ...
  const credential = runCredential([
    ...
    '--register',
    '--label',
    'Intent model1 exchange paper ref',
    '--connector-kind',
    'exchange',
    '--environment',
    'paper',
  ...
  if (
    ready.reviewPacket?.activationAuthorized !== false ||
    ready.safety.liveExecutionAuthorized !== false ||
    ready.safety.executableNow !== false ||
    ready.safety.liveActionApplied !== false
  ) {
    failures.push('Intent model1 review packet must still keep live execution disabled');
  }
  if (!ready.gates.every((gate) => gate.passed === true)) {
    failures.push('review-ready should pass every Intent model1 gate');
  }
  ...
  if (failures.length > 0) {
    console.error('[transaction-live-activation-review-intent-model1-check] failed');
  ...
  console.log('[transaction-live-activation-review-intent-model1-check] ok');
  console.log('- contract, service, CLI, docs and tests are present');
  console.log('- Intent model1 consumes Intent model0 candidate-ready envelopes');
...
function killSwitchArgs() {
  return [
    '--kill-switch-id',
    'intent-model1-kill-switch',
    '--kill-switch-enabled',
    '--kill-switch-tested',
    '--kill-switch-command',
    'zavorth transaction disable-live --scope intent-model1',
  ];
}
...
function rollbackArgs() {
  return [
    ...
    '--rollback-artifact',
    'data/runtime/intent-model1-rollback-receipt.json',
  ];
}
