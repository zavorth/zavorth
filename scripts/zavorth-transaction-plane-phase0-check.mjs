...
if (asJson) {
  ...
  console.log('[transaction-plane-phase0] checking Zavorth Transaction Plane Phase 0');
  for (const item of rules) {
    console.log(`[transaction-plane-phase0] ${item.status === 'passed' ? 'ok' : 'fail'} ${item.label}: ${item.observed} | ${item.target}`);
...
function ruleFilesExist() {
  ...
  return rule('phase0-files', 'Phase 0 files exist', missing.length === 0, `${files.length - missing.length}/${files.length}`, 'contract, service, CLI, docs and tests present', missing);
}
...
function ruleMarkers() {
  const checks = [
    ...
    ['package.json', [
      ...
      'zavorth:transaction-plane:phase0:check',
    ]],
  ];
  const missing = [];
  ...
  return rule('phase0-markers', 'Phase 0 semantic markers exist', missing.length === 0, missing.length === 0 ? 'all markers' : `${missing.length} missing`, 'security contract exported and scripted', missing);
}

function ruleCliSnapshot() {
  const result = runTs('scripts/zavorth-transaction-plane.ts', ['--json']);
  return jsonRule('phase0-cli-snapshot', 'CLI emits contract snapshot', result, (snapshot) =>
    snapshot.status === 'ready'
    && snapshot.phase0?.llmDirectExecutionAllowed === false
    && snapshot.phase0?.realMoneyRequiresExplicitApproval === true
...
function ruleBlockedLlmFixture() {
  ...
  return jsonRule('phase0-llm-blocked', 'LLM direct execution is blocked', result, (snapshot) =>
...
function ruleGovernedPaymentFixture() {
  ...
  return jsonRule('phase0-governed-payment', 'Governed runtime payment can pass policy', result, (snapshot) =>
...
function ruleTests() {
  ...
  return rule('phase0-tests', 'Phase 0 Jest tests pass', result.status === 0, `exit ${result.status ?? 'unknown'}`, 'jest green', compact(result.stderr, result.stdout));
}
