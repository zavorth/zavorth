...
async function main(): Promise<void> {
  ...
  const snapshot = service.execute({
    ...
    manifestPath: readString(args, 'manifest') || readString(args, 'manifest-path'),
    manifestJson: readString(args, 'manifest-json'),
    pluginId: readString(args, 'plugin') || readString(args, 'plugin-id'),
    lifecycleAction: (readString(args, 'lifecycle') || readString(args, 'lifecycle-action')) as any,
...
function parseArgs(argv: string[]): Args {
  ...
  if (argv.includes('--dev')) result.dev = true;
  return result;
}
...
main().catch((error) => {
  console.error('[zavorth-extension-plugin-sdk] failed');
