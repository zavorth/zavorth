...
import { DistributionPolicyContractService } from '../src/services/DistributionPolicyContractService.js';
import {
  DISTRIBUTION_POLICY_SCREENSHOTS,
  type DistributionPolicyScreenshotSpec,
} from '../src/contracts/DistributionPolicyContract.js';
...
const screenshotDir = path.join(websiteRoot, '.qa', 'distribution-policy');
...
async function main(): Promise<void> {
  ...
  if (shouldCaptureScreenshots) {
    await captureDistributionScreenshots(websiteRoot, screenshotDir);
  }

  const service = new DistributionPolicyContractService({
    websiteRoot,
...
async function captureDistributionScreenshots(root: string, targetDir: string): Promise<void> {
  ...
  try {
    ...
    for (const screenshot of DISTRIBUTION_POLICY_SCREENSHOTS) {
      await captureScreenshot(browser, `${server.url}editions`, screenshot, targetDir);
    }
  } finally {
    if (browser) {
      await browser.close();
...
async function captureScreenshot(
  ...
  url: string,
  screenshot: DistributionPolicyScreenshotSpec,
  targetDir: string,
): Promise<void> {
...
main().catch((error) => {
  console.error('[distribution-policy] falhou:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
