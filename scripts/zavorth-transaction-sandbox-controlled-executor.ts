...
type CliOptions = ZavorthTransactionSandboxControlledExecutorInput & {
  json: boolean;
  ledgerFile?: string;
...
const service = new ZavorthTransactionSandboxControlledExecutorService({
...
const result = service.execute(options);
...
process.exit(result.status === 'sandbox-executed' ? 0 : 1);
