...
const outDir = path.join(rootDir, ".tmp", "dashboard-provider-cockpit-visual-qa");
...
function generatePreview(): string {
  ...
  const result = spawnSync(
    ...
    [
      ...
      path.join(rootDir, "scripts", "dashboard-browser-preview.ts"),
      "--fixture=safe-run",
      `--out=${outDir}`,
    ],
    {
...
main()
  ...
  .catch((error) => {
    console.error(`[dashboard-provider-cockpit-visual-qa] FAIL ${error?.message || error}`);
