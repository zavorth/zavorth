#!/usr/bin/env tsx
import { ZavorthEnsembleService } from '@zavorth/agents/ZavorthEnsembleService.js';
...
function commandFor(index: number): { command: string; args: string[] } {
  const text = `zavorth-ensemble-scale-worker-${index}-ok`;
...
async function main() {
  ...
  const service = new ZavorthEnsembleService();
  const initial = service.launchOfficialSwarm({
    objective: `Run Zavorth Ensemble scale benchmark with ${args.roles} workers.`,
    roles,
    official: true,
    maxRoles: args.roles,
  ...
  if (args.json) {
    ...
    console.log('[zavorth-ensemble-scale-benchmark]');
