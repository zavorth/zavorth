...
const defaultOutDir = path.join(rootDir, ".tmp", "command-center-chat-visual-qa");
...
function commandCenterPayload(state: RuntimeState) {
  return {
    live: true,
    authRequired: false,
...
function createQaServer(state: RuntimeState): Promise<{ server: http.Server; url: string }> {
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname === "/api/web/events") {
      ...
      res.end(": command-center-chat-visual-qa\n\n");
      return;
    }
        ...
        state.runs = [
          {
            ...
            modelProfile: commandCenterPayload(state).modelProfile,
            approvals: [
              {
...
async function runQa(options: CliOptions): Promise<QaReport> {
  const state: RuntimeState = {
    sessionId: "qa-command-center-session",
    messages: [],
    runs: [],
    artifacts: [],
  };
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const chat = (window as any).ZavorthCommandCenterChat;
...
runQa(options)
  ...
  .catch((error) => {
    ...
    writeReport(report);
    console.error(`[command-center-chat-visual-qa] FAIL ${error?.message || error}`);
