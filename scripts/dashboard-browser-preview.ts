...
} from "../src/zavorth-control/app/(zavorthControl)/control/zavorth-control/preview/zavorthControlFixturePreview";
import type { ZavorthControlZavorthControlViewModel } from "../src/zavorth-control/app/(zavorthControl)/control/zavorth-control/contracts";
...
function buildPreviewHtml(defaultFixtureId: string): string {
  const css = fs.readFileSync(
    path.join(rootDir, "src/zavorth-control/app/(zavorthControl)/control/zavorth-control/styles/zavorthControl.css"),
    "utf8",
  );
  ...
  <script>
    ...
    const normalizeZavorthControlCopy = (value) => {
      ...
      const replacements = [
        ...
        ["ZavorthControl carregou, mas o gateway real ainda nao respondeu.", "ZavorthControl loaded, but the real gateway has not responded yet."],
        ...
        ["Resposta atual no painel web /zavorthControl.", "Current response in the /zavorthControl web panel."],
        ...
        ["ZavorthControl pronto", "ZavorthControl ready"],
        ...
        ["O README esta alinhado com a entrada ZavorthControl e a jornada inicial.", "The README is aligned with the ZavorthControl entry point and the first-run journey."],
        ["O README estÃ¡ alinhado com a entrada ZavorthControl e a jornada inicial.", "The README is aligned with the ZavorthControl entry point and the first-run journey."],
        ...
        ["O ZavorthControl abriu, mas os dados reais do runtime exigem o token local desta instalacao.", "ZavorthControl opened, but real runtime data requires the local token for this installation."],
        ...
        ["No /zavorthControl real, este botao chama o proxy server-side e nao expoe token no navegador.", "In real /zavorthControl, this button calls the server-side proxy and does not expose tokens in the browser."],
        ...
        ["Atividade do ZavorthControl", "ZavorthControl activity"],
      ...
      return {
        ...
        runtime: {
          ...
          summary: live
            ...
            : "ZavorthControl carregou, mas o gateway real ainda nao respondeu.",
          blockers,
          wsStatus: "connected"
        },
        agentRun,
        ...
        replyPorts: Array.isArray(run?.replyPorts) ? run.replyPorts : [{ id: "zavorthControl", label: "ZavorthControl", kind: "web", status: "available", primary: true, description: "Resposta atual no painel web /zavorthControl." }],
        modelProfile: {
          providerLabel,
          modelLabel,
    ...
    const renderAuthUnlock = (vm) => {
      ...
      return '<section class="bcc-fixture-preview" data-active="true" aria-label="Desbloquear ZavorthControl ao vivo"><div><span class="bcc-fixture-preview__label">Acesso protegido</span><strong>Desbloquear runtime real</strong><p>O cockpit ja esta carregado. Para mostrar runs, approvals e historico reais, informe o token local do Zavorth nesta sessao.</p></div><form id="zavorthControl-auth-form" class="bcc-compose__input-frame" autocomplete="off"><input id="zavorthControl-auth-token" type="password" inputmode="text" autocomplete="current-password" placeholder="Token local do Zavorth" style="width:100%;box-sizing:border-box;background:transparent;border:0;color:#edf8f6;outline:0;padding:12px 4px;font:inherit" /><div class="bcc-compose__footer"><span id="zavorthControl-auth-message" class="bcc-empty-note">O token fica apenas no sessionStorage desta aba.</span><button class="bcc-button bcc-compose__send" data-variant="primary" type="submit">Desbloquear</button></div></form></section>';
    };

    const renderOnboardingPanel = (vm) => {
      if (vm.authRequired) {
        return '<section class="bcc-card"><p class="bcc-card__label">Acesso</p><h2 class="bcc-card__title">Runtime protegido</h2><div class="bcc-card__body"><div class="bcc-access-card" data-state="protected"><div class="bcc-access-card__header">' + badge("token necessario", "warn") + '<span>Local-first</span></div><p class="bcc-access-card__copy">O ZavorthControl abriu, mas os dados reais do runtime exigem o token local desta instalacao.</p><div class="bcc-access-commands"><code>zavorth zavorthControl</code><code>zavorth zavorthControl repair</code><code>zavorth zavorthControl token</code></div><p class="bcc-access-card__feedback">O token fica somente no sessionStorage desta aba.</p></div></div></section>';
      }
