...
} from "../src/ai-gateway/app/(dashboard)/dashboard/dashboard/preview/dashboardFixturePreview";
import type { DashboardDashboardViewModel } from "../src/ai-gateway/app/(dashboard)/dashboard/dashboard/contracts";

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const defaultOutDir = path.join(rootDir, ".tmp", "dashboard-browser-preview");
...
type BrowserPreviewFixturePayload = {
  option: ReturnType<typeof listDashboardDashboardFixturePreviewOptions>[number];
  viewModel: DashboardDashboardViewModel;
};
...
function buildPreviewHtml(defaultFixtureId: string): string {
  const css = fs.readFileSync(
    path.join(rootDir, "src/ai-gateway/app/(dashboard)/dashboard/dashboard/styles/dashboard.css"),
    "utf8",
  );
  ...
  const fixtures: Record<string, BrowserPreviewFixturePayload> = Object.fromEntries(
    options.map((option) => [
      ...
      {
        option,
        viewModel: buildDashboardDashboardFixturePreviewViewModel(option.id),
      },
    ]),
...
<head>
  ...
  <title>Zavorth Dashboard Preview</title>
  <style>
...
<body>
  ...
  <script>
    ...
    const LIVE_OPTION = {
      ...
      description: "Snapshot real do Zavorth Agent Gateway servido pelo /dashboard."
    };
    const AUTH_STORAGE_KEY = "zavorth.dashboard.webToken";
    ...
    const normalizeDashboardCopy = (value) => {
      ...
      const replacements = [
        ...
        ["Dashboard carregou, mas o gateway real ainda nao respondeu.", "Dashboard loaded, but the real gateway has not responded yet."],
        ...
        ["Resposta atual no painel web /dashboard.", "Current response in the /dashboard web panel."],
        ...
        ["Identidade padrao do Dashboard ao vivo.", "Default live Dashboard identity."],
        ...
        ["Dashboard pronto", "Dashboard ready"],
        ...
        ["O README esta alinhado com a entrada Dashboard e a jornada inicial.", "The README is aligned with the Dashboard entry point and the first-run journey."],
        ["O README estÃ¡ alinhado com a entrada Dashboard e a jornada inicial.", "The README is aligned with the Dashboard entry point and the first-run journey."],
        ...
        ["O Dashboard abriu, mas os dados reais do runtime exigem o token local desta instalacao.", "Dashboard opened, but real runtime data requires the local token for this installation."],
        ...
        ["Gateway aceitando eventos do Dashboard.", "Gateway accepting Dashboard events."],
        ...
        ["No /dashboard real, este botao chama o proxy server-side e nao expoe token no navegador.", "In real /dashboard, this button calls the server-side proxy and does not expose tokens in the browser."],
        ...
        ["Atividade do Dashboard", "Dashboard activity"],
    ...
    const escapeHtml = (value) => normalizeDashboardCopy(value)
      .replaceAll("&", "&amp;")
      .replaceAll("<", "&lt;")
      .replaceAll(">", "&gt;")
      .replaceAll('"', "&quot;");

    const normalizeVisibleDashboardCopy = (root) => {
      if (!root || !document.createTreeWalker) return;
      const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
      const nodes = [];
      while (walker.nextNode()) nodes.push(walker.currentNode);
      for (const node of nodes) {
        const normalized = normalizeDashboardCopy(node.nodeValue);
        if (normalized !== node.nodeValue) node.nodeValue = normalized;
      }
      root.querySelectorAll?.("input[placeholder], textarea[placeholder], [aria-label], [title]").forEach((element) => {
        for (const attr of ["placeholder", "aria-label", "title"]) {
          const value = element.getAttribute(attr);
          if (value) element.setAttribute(attr, normalizeDashboardCopy(value));
        }
      });
    };
    ...
    const buildLiveViewModelFromSnapshot = (snapshot, live) => {
      ...
      return {
        contractVersion: "dashboard-runtime-contract/v1",
        generatedAt,
        adapterSource: {
          kind: "universal-agent-runtime",
          label: live ? "Zavorth Agent Gateway" : "Dashboard live fallback",
        ...
        runtime: {
          ...
          summary: live
            ...
            : "Dashboard carregou, mas o gateway real ainda nao respondeu.",
          blockers,
          wsStatus: "connected"
        },
        agentRun,
        ...
        replyPorts: Array.isArray(run?.replyPorts) ? run.replyPorts : [{ id: "dashboard", label: "Dashboard", kind: "web", status: "available", primary: true, description: "Resposta atual no painel web /dashboard." }],
        modelProfile: {
          providerLabel,
          modelLabel,
        ...
        identity: { agentName: "Zavorth", userName: "Operador", language: "en-US", tone: "operacional", initiative: "balanced", firstRunStatus: "unknown", summary: "Identidade padrao do Dashboard ao vivo." },
    ...
    const renderFixturePreviewBar = (activeId, option) => {
      ...
      return '<section class="bcc-fixture-preview" data-active="true" aria-label="Preview de contrato do Dashboard"><div><span class="bcc-fixture-preview__label">' + escapeHtml(label) + '</span><strong>' + escapeHtml(option.label) + '</strong><p>' + escapeHtml(option.description) + '</p></div><label class="bcc-fixture-preview__select"><span>Cenario visual</span><select id="fixture-select">' + optionHtml + '</select></label></section>';
    };

    const renderAuthUnlock = (vm) => {
      if (!vm.authRequired) return "";
      return '<section class="bcc-fixture-preview" data-active="true" aria-label="Desbloquear Dashboard ao vivo"><div><span class="bcc-fixture-preview__label">Acesso protegido</span><strong>Desbloquear runtime real</strong><p>O cockpit ja esta carregado. Para mostrar runs, approvals e historico reais, informe o token local do Zavorth nesta sessao.</p></div><form id="dashboard-auth-form" class="bcc-compose__input-frame" autocomplete="off"><input id="dashboard-auth-token" type="password" inputmode="text" autocomplete="current-password" placeholder="Token local do Zavorth" style="width:100%;box-sizing:border-box;background:transparent;border:0;color:#edf8f6;outline:0;padding:12px 4px;font:inherit" /><div class="bcc-compose__footer"><span id="dashboard-auth-message" class="bcc-empty-note">O token fica apenas no sessionStorage desta aba.</span><button class="bcc-button bcc-compose__send" data-variant="primary" type="submit">Desbloquear</button></div></form></section>';
    };

    const renderOnboardingPanel = (vm) => {
      if (vm.authRequired) {
        return '<section class="bcc-card"><p class="bcc-card__label">Acesso</p><h2 class="bcc-card__title">Runtime protegido</h2><div class="bcc-card__body"><div class="bcc-access-card" data-state="protected"><div class="bcc-access-card__header">' + badge("token necessario", "warn") + '<span>Local-first</span></div><p class="bcc-access-card__copy">O Dashboard abriu, mas os dados reais do runtime exigem o token local desta instalacao.</p><div class="bcc-access-commands"><code>zavorth dashboard</code><code>zavorth dashboard repair</code><code>zavorth dashboard token</code></div><p class="bcc-access-card__feedback">O token fica somente no sessionStorage desta aba.</p></div></div></section>';
      }
    ...
    const renderActiveRunState = (vm) => {
      ...
      return '<section class="bcc-active-run-state" data-status="' + escapeHtml(run?.status || "idle") + '"><div><span class="bcc-card__label">' + escapeHtml(label) + '</span><h2>' + escapeHtml(run?.title || "Atividade do Dashboard") + '</h2><p>' + escapeHtml(run?.summary || vm.runtime.summary) + '</p></div><div class="bcc-active-run-state__badges">' + badges + '</div>' + renderTrace("Trace seguro", trace, trace?.summary ? trace.summary.eventCount + " eventos seguros" : "") + (events ? '<div class="bcc-run-mini-timeline">' + events + '</div>' : "") + '</section>';
    };
    ...
    const renderLoading = (label) => {
      document.getElementById("dashboard-preview-root").innerHTML = '<div class="bsk-dashboard"><div class="bcc-shell"><main class="bcc-viewport"><section class="bcc-hero">' + fox() + '<div><span class="bcc-hero__eyebrow">Dashboard</span><h1 class="bcc-hero__title">' + escapeHtml(label) + '</h1><p class="bcc-hero__subtitle">Carregando snapshot do Zavorth...</p></div></section></main></div></div>';
    };
    ...
    const render = async (fixtureId) => {
      ...
      const tabs = '<nav class="bcc-tab-strip" aria-label="Dashboard sections">' + vm.sectors.filter((sector) => sector.enabled).map((sector) => '<button type="button" data-active="' + (sector.id === "terminal" ? "true" : "false") + '"><span>' + escapeHtml(sector.label) + '</span>' + (sector.badgeCount ? '<em>' + escapeHtml(sector.badgeCount) + '</em>' : "") + '</button>').join("") + '</nav>';
      ...
      const authForm = document.getElementById("dashboard-auth-form");
      authForm?.addEventListener("submit", async (event) => {
        event.preventDefault();
        const input = document.getElementById("dashboard-auth-token");
        const message = document.getElementById("dashboard-auth-message");
...
function main() {
  ...
  const selectedFixture = options.fixture === "all"
    ? "safe-run"
    : resolveDashboardDashboardFixturePreviewId(options.fixture) ?? "safe-run";
  ...
  console.log(`[dashboard-browser-preview] ${htmlPath}`);
  console.log(`[dashboard-browser-preview] file://${htmlPath.replace(/\\/g, "/")}?fixture=${selectedFixture}`);
}
