...
function main() {
  ...
  assertContains(files.packageJson, JSON.stringify(packageJson.scripts || {}), 'zavorth:command-center-responsive-visual-qa:check');
  assertContains('workspace:check', workspaceCheck, 'zavorth:command-center-responsive-visual-qa:check');
  assertOrder(
    'workspace:check',
    workspaceCheck,
    'zavorth:command-center-daily-use-polish:check',
    'zavorth:command-center-responsive-visual-qa:check',
  );

  console.log('[command-center-responsive-visual-qa] ok responsive visual QA guard is wired');
}
