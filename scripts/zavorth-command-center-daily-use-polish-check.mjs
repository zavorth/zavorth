...
rules.push(
  ...
  {
    id: 'workspace:gate',
    status: packageJson.includes('zavorth:command-center-daily-use-polish:check') ? 'passed' : 'failed',
...
console.log('[command-center-daily-use-polish] certification');
for (const rule of rules) {
  console.log(`[command-center-daily-use-polish] ${rule.status === 'passed' ? 'ok' : 'fail'} ${rule.id}: ${rule.summary}`);
}

if (failed.length > 0) {
  console.error(`[command-center-daily-use-polish] failed rules: ${failed.map((rule) => rule.id).join(', ')}`);
  process.exit(1);
}
