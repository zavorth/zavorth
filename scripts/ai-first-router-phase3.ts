...
function buildAiPlan(userMessage: string): unknown {
  return {
    ...
    intent: {
      ...
      confidence: 0.9,
      summary: 'Preparar configuracao guiada com seguranca e aprovacao.',
    },
    goal: {
      userFacing: userMessage,
      internalSummary: 'Salvar configuracao privada apos preview e aprovacao.',
    },
    proposedActions: [
      {
        id: 'show-preview',
...
async function main(): Promise<void> {
  ...
  const service = new AiFirstPolicyGuardrailService();
...
main().catch((error) => {
  console.error('[ai-first-router-phase3] falhou:', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
