...
(function () {
  ...
  function updateInstancesTable() {
    ...
    tbody.innerHTML = `
      ...
      <tr>
        ...
        <td class="mono">â€”</td>
        <td>â€”</td>
        <td>${state.zavorthControl?.live ? 'live' : 'protected'}</td>
        <td>${statusBadge(gatewayReady || state.zavorthControl?.live ? 'ready' : 'degraded', gatewayReady || state.zavorthControl?.live ? 'Connected' : 'Protected')}</td>
      </tr>
    `;
  }
  ...
  function updateUsage() {
    ...
    updatePremiumStatus('Usage ledger', state.zavorthControl?.live ? 'live' : 'local', state.zavorthControl?.live ? 'ok' : 'info');
  ...
  async function decideApproval(input, ui = {}) {
    ...
    const body = kind === 'agent-run'
      ? { approvalId: id, sessionId, source: 'zavorth-control' }
      : kind === 'task'
        ? { taskId: id, sessionId }
