...
onst AUTH_STORAGE_KEY = 'zavorth.zavorthControl.webToken';
onst SESSION_STORAGE_KEY = 'zavorth.zavorthControl.sessionId';
onst RUN_STORAGE_KEY = 'zavorth.zavorthControl.runId';
onst state = {
 auth: null,
 zavorthControl: null,
...
unction hasStoredToken() {
 return Boolean(readToken());


unction clearStoredToken() {
 try {
...
unction writeSessionId(sessionId) {
...
unction buildZavorthControlQueryString(extra = {}) {
...
unction replaceZavorthControlUrlParams(values = {}) {
 try {
   const url = new URL(window.location.href);
...
unction realtimePath(sessionId) {
 return `/api/web/events?sessionId=${encodeURIComponent(sessionId)}`;


unction resolveRealtimeSessionId() {
...
unction closeRealtimeTransport() {
...
unction markRealtimeConnected(transport, eventType = 'open') {
 const realtime = state.realtime;
...
unction markRealtimeConnecting(transport, sessionId) {
 const realtime = state.realtime;
...
unction markRealtimeDisconnected(error) {
 const realtime = state.realtime;
 realtime.connected = false;
 realtime.connecting = false;
...
unction scheduleRealtimeReconnect() {
 const realtime = state.realtime;
...
unction scheduleRealtimeRefresh(reason = 'event', delayMs = 350) {
 const realtime = state.realtime;
...
unction emitDashboardEvents(events, source = 'runtime-history') {
...
unction dashboardEventTime(value) {
 const normalized = String(value || '').trim();
 return normalized || new Date().toISOString();


unction dashboardEventFromRealtimeEvent(event) {
...
sync function fetchDashboardEvents(ui = window.ZavorthControlChat || {}, query = {}) {
...
sync function openPersistentTrace(query = {}, ui = window.ZavorthControlChat || {}) {
 const normalized = {
...
unction suppressTranscriptRender(durationMs = 5000) {
 state.transcriptRenderSuppressedUntil = Date.now() + Math.max(0, Number(durationMs) || 0);


unction isTranscriptRenderSuppressed(options = {}) {
...
unction handleRealtimeEvent(event) {
...
unction consumeSseBuffer(buffer) {
...
sync function startFetchEventStream(sessionId) {
...
unction startEventSourceStream(sessionId) {
...
unction disconnectRealtime(reason = 'manual') {
 const realtime = state.realtime;
...
sync function readJson(path, options = {}) {
...
sync function readBlob(path, options = {}) {
 const response = await fetch(path, {
   ...options,
   headers: {
     Accept: '*/*',
...
unction text(value, fallback = '---') {
 const normalized = String(value ?? '').trim();
 return normalized || fallback;


unction numberLabel(value, fallback = '0') {
 const numeric = Number(value);
 if (!Number.isFinite(numeric)) return fallback;
 return numeric.toLocaleString('en-US');


unction formatDate(value) {
...
unction statusBadge(status, label) {
...
unction isUnknownModelLabel(value) {
...
unction normalizeModelProfile(profile) {
...
unction resolveCurrentModelProfile() {
 const activeRun = getActiveRun();
 const runs = getRuns();
 const candidates = [
...
unction getCurrentProviderLabel() {
...
unction publishCurrentModelProfile() {
...
unction updateSummaryCard(label, value, sub) {
...
unction updatePremiumMetric(label, value, sub) {
...
unction updatePremiumStatus(label, value, tone = 'info') {
 const normalized = String(label || '').trim().toLowerCase();
...
unction deriveNextRunAction(run) {
...
unction eventCountMatching(pattern) {
...
unction channelReadinessLabel(id) {
...
unction channelReadinessTone(id) {
 return /ready|connected/i.test(channelReadinessLabel(id)) ? 'ok' : 'info';


unction updatePlatformAction(sectionId, title, detail, prompt) {
...
unction setLiveStripValue(selector, value) {
...
unction updateLiveStripFromRuntime(runtimeState, runtimeDetail, gatewayState, gatewayDetail) {
...
unction setPulseAccessState(accessState) {
 const pulse = document.getElementById('core-pulse');
 if (!pulse) return;
 pulse.dataset.authState = String(accessState || 'local');


unction wireUnlockPulse(enabled) {
 const pulse = document.getElementById('core-pulse');
 if (!pulse) return;
...
unction updateRecentActivityTable() {
...
unction updateProviderModelCatalog() {
...
unction updateProviderActivation() {
...
unction badgeToneForStatus(status) {
...
unction setCardGrid(sectionId, html) {
 const grid = document.querySelector(`#${sectionId} .card-grid`);
 if (grid) grid.innerHTML = html;


unction setTableHeaders(sectionId, labels) {
...
unction firstReadyChannelLabel() {
 const ids = configuredChannelIds();
...
unction updateControlTelemetryRail() {
 const modelProfile = resolveCurrentModelProfile();
...
unction runTitle(run, fallback = 'Workspace session') {
 const candidates = [
   run?.title,
   run?.name,
   run?.summary,
...
unction controlSessionItemHtml(run, isActive) {
 const status = normalizeRunStatus(run?.status);
...
unction fallbackControlSessionItemsHtml() {
 return [
   {
...
unction controlRailLinkHtml(label, prompt, attention = false) {
 return `
...
unction updateControlSessionRail() {
...
unction normalizeToolEntry(entry) {
...
unction basename(value) {
 return String(value || '').trim().replace(/\\/g, '/').split('/').filter(Boolean).pop() || '';


unction extensionOf(value) {
 const name = basename(value).toLowerCase();
 const dot = name.lastIndexOf('.');
 return dot >= 0 ? name.slice(dot + 1) : '';


unction inferArtifactKind(entry) {
...
unction normalizeArtifact(entry, defaults = {}) {
...
unction extractArtifacts(payload) {
 const artifacts = [];
...
unction hasDirectExecutionArtifactContext(payload) {
...
unction extractResponseDecision(payload) {
...
unction extractArtifactPolicy(payload) {
...
unction artifactPolicyAllowsChatDisplay(payload) {
 const policy = extractArtifactPolicy(payload);
 if (!policy) return true;
 return policy.shouldShowArtifactInChat === true;


unction shouldDisplayArtifactsInChat(payload, options = {}) {
...
unction renderArtifactsFromPayload(payload, ui = {}, options = {}) {
...
unction normalizeDiffPreview(preview) {
...
unction extractDiffPreviews(payload = state.zavorthControl || {}) {
 const candidates = [
...
unction diffPreviewBodyHtml(preview) {
...
unction renderDiffPreviewsFromPayload(payload = state.zavorthControl || {}) {
...
unction hasExecutionArtifactContext(payload) {
 if (!payload || typeof payload !== 'object') return false;
 if (!artifactPolicyAllowsChatDisplay(payload)) return false;
 if (String(payload.taskId || payload.runId || payload.approvalId || '').trim()) return true;
 if (Array.isArray(payload.artifacts) && payload.artifacts.length > 0) return true;
...
unction isTextArtifact(artifact) {
...
unction isVisualAsset(artifact) {
 const extension = extensionOf(artifact?.path || artifact?.name || artifact?.title);
 return ['pdf', 'png', 'jpg', 'jpeg', 'gif', 'webp'].includes(extension);


unction artifactMetadataHtml(artifact) {
 const rows = [
   ['Type', artifact.kind],
...
unction artifactFallbackHtml(artifact, message = '') {
...
sync function buildArtifactPaneHtml(artifact) {
...
sync function fetchCurrentArtifacts(ui = window.ZavorthControlChat || {}) {
...
sync function openArtifact(id, ui = window.ZavorthControlChat || {}) {
...
unction buildRunReplayHtml(run) {
 const events = Array.isArray(run?.events) ? run.events : [];
 const approvals = pendingRunApprovals(run);
...
unction setUnlockFeedback(message, tone = 'muted') {
...
sync function lockZavorthControlTab() {
 clearStoredToken();
...
unction openUnlockModal(reason = '') {
 const content = `
...
sync function validateUnlockToken(token) {
 if (!token) {
...
unction normalizeTranscriptEntry(message) {
...
unction extractTranscriptMessages(payload) {
 const candidates = [
...
unction renderMessagesFromPayload(payload, ui = {}, options = {}) {
...
unction normalizeApproval(entry, kind) {
 const id = String(
   entry?.id
   || entry?.permission_id
   || entry?.task_id
...
unction extractApprovals(payload) {
 const approvals = [];
 const permissions = [
   payload?.permissions,
...
unction renderApprovalsFromPayload(payload, ui = {}) {
...
unction normalizeRemoteMeshApprovalCard(card) {
...
unction collectRemoteMeshUxSnapshots(payload) {
 const snapshots = [
...
unction extractRemoteMeshApprovalCards(payload) {
 const cards = [];
...
unction renderRemoteMeshApprovalsFromPayload(payload, ui = {}) {
...
sync function fetchCurrentApprovals(ui = window.ZavorthControlChat || {}) {
 const sessionId = readSessionId();
 if (!sessionId) return null;
...
sync function hydrateCurrentSession(ui = window.ZavorthControlChat || {}, options = {}) {
...
unction extractAssistantReply(payload) {
...
unction protectedRuntimeReply(error) {
 if (error?.status === 401) {
   return [
...
sync function sendChat(message, ui = {}, composerPayload = {}) {
...
sync function decideApproval(input, ui = {}) {
...
sync function sendSalesPackDemoInbound() {
...
sync function applyRemoteMeshApproval(input, ui = {}) {
 const id = String(input?.id || '').trim();
 if (!id) {
...
sync function applyDiffPreview(input, ui = {}) {
...
indow.addEventListener('beforeunload', () => {
