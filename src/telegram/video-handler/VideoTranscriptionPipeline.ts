...
import { GeminiVideoAnalyzer, type GeminiVideoAnalysis } from '../GeminiVideoAnalyzer.js';
...
export class VideoTranscriptionPipeline {
  ...
  public async tryGeminiYouTube(videoUrl: string, title: string): Promise<GeminiVideoAnalysis | null> {
    ...
    try {
      ...
      console.warn(`[VideoHandler] Gemini YouTube falhou: ${errorMessage}`);
      return {
        analysisText: '',
  ...
  public async tryGeminiLocalVideo(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`[VideoHandler] Gemini local video falhou: ${errorMessage}`);
      return {
        analysisText: '',
        source: 'Gemini indisponivel',
  ...
  public async tryGeminiLocalAudio(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`[VideoHandler] Gemini local audio falhou: ${errorMessage}`);
      return {
        analysisText: '',
        source: 'Gemini indisponivel',
  ...
  public async tryChunkedMediaTranscript(filePath: string, titleHint?: string): Promise<VideoTranscriptResult | null> {
    if (!this.deps.audioChunker.isAvailable()) {
      return {
        ...
        warnings: [VideoHandlerHelpers.buildMediaCapabilityWarning('A trilha de segmentacao em chunks ainda nao foi provisionada neste host.')],
      };
    }
  ...
  public async tryDedicatedGeminiFallbackTranscript(
    ...
    if (!this.deps.audioChunker.isAvailable()) {
      const directTranscript = await this.tryDedicatedGeminiPureTranscription(
        filePath,
        VideoHandlerHelpers.guessMimeTypeFromPath(filePath),
        titleHint,
      );
  ...
  public async transcribeWithRetries(
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        lastError = error;
        if (attempt < attempts) {
          await VideoHandlerHelpers.sleep(attempt * 1500);
        }
      }
    }
  ...
  private async tryDedicatedGeminiPureTranscription(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`[VideoHandler] Gemini transcription fallback falhou: ${errorMessage}`);
      return {
        analysisText: '',
  ...
  private async processDedicatedFallbackChunk(
    ...
    const label = `Trecho ${chunk.index + 1} (${VideoHandlerHelpers.formatSecondsAsClock(chunk.startSeconds)}-${VideoHandlerHelpers.formatSecondsAsClock(chunk.endSeconds)})`;
    const warnings: string[] = [];

    const chunkTranscript = await this.tryDedicatedGeminiPureTranscription(
      chunk.filePath,
      VideoHandlerHelpers.guessMimeTypeFromPath(chunk.filePath),
  ...
  private async processPrimaryChunk(
    ...
    const label = `Trecho ${chunk.index + 1} (${VideoHandlerHelpers.formatSecondsAsClock(chunk.startSeconds)}-${VideoHandlerHelpers.formatSecondsAsClock(chunk.endSeconds)})`;
    const chunkPrompt = `Transcreva o conteudo falado deste trecho com boa pontuacao. Preserve nomes proprios, termos tecnicos, numeros e referencias importantes. Faixa aproximada do trecho: ${VideoHandlerHelpers.formatSecondsAsClock(chunk.startSeconds)} a ${VideoHandlerHelpers.formatSecondsAsClock(chunk.endSeconds)}.`;
    const warnings: string[] = [];
    let chunkText = '';
    let chunkSource = '';
  ...
  private async tryGeminiSectionSummary(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`[VideoHandler] Gemini section summary falhou: ${errorMessage}`);
      return {
        analysisText: '',
        source: 'Gemini indisponivel',
