...
import { normalizeTelegramCommandToken } from './CommandParser.js';
import { HostIdentityService } from '../services/HostIdentityService.js';
import { getExplicitExecutorForCommand } from './commandCatalog.js';
import {
...
export class AuthGuard {
  ...
  static middleware(hostIdentityService?: HostIdentityService) {
    return async (ctx: Context, next: NextFunction) => {
      ...
      if (!chat_id) {
        console.warn('[Security] Rejeitado: chat_id nulo.');
        return;
      }
      ...
      if (!user_id) {
        ...
        console.warn('[Security] Rejeitado: chat_id ou user_id nulo.');
        return;
      }
      ...
      if (!config.allowedUserIds.includes(user_id)) {
        ...
        console.warn(`[Security] Acesso nao autorizado de user_id: ${user_id} e chat_id: ${chat_id}`);
  ...
  private static logSecurityBlock(userId: string, command: string) {
    try {
      console.warn(`[Security] Vice-Owner bloqueado ao tentar executar: ${command}`);
    } catch {
