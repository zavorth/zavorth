...
export class AudioHandler {
  ...
  private async synthesizeInternal(
    ...
    if (cachedPath) {
      if (traceId) {
        ...
        console.log(`[AudioHandler] TTS cache hit chars=${cleanText.length} path=${cachedPath}`);
      }
      return cachedPath;
    }
    ...
    for (const provider of providerOrder) {
      if (provider === 'edge-tts') {
        try {
          ...
          if (isCapabilityUnavailableError(error)) {
            ...
            console.warn('[AudioHandler] edge-tts indisponivel. Tentando proximo provider...');
          } else {
            console.error(`[AudioHandler] Erro no TTS local: ${error}`);
          }
        }
        continue;
      }
      ...
      if (provider === 'gemini' && this.geminiVoiceService.isConfigured()) {
        try {
          ...
          console.error(`[AudioHandler] Erro no Gemini TTS: ${error}`);
        }
      }
    }
  ...
  private async tryEdgeTts(
    ...
    console.log(`[AudioHandler] Audio TTS gerado via edge-tts: ${outputFile}`);
    return outputFile;
  }
  ...
  private async tryGeminiTts(
    ...
    if (typeof this.geminiVoiceService.synthesizeDetailed === 'function') {
      ...
      if (detailed?.filePath && fs.existsSync(detailed.filePath)) {
        ...
        console.log(`[AudioHandler] Audio Gemini TTS gerado: ${detailed.filePath}`);
    ...
    if (geminiAudio && fs.existsSync(geminiAudio)) {
      ...
      console.log(`[AudioHandler] Audio Gemini TTS gerado: ${geminiAudio}`);
      return geminiAudio;
    }
    return null;
  }
  ...
  public cleanup(filePath: string): void {
    try {
      ...
      console.warn(`[AudioHandler] Falha ao remover temporario: ${error}`);
    }
