...
export class TelegramConversationStateService {
  ...
  public recordAgentGatewayRunOutcome(task: Task, taskGoal: string, result: any): void {
    const finishedAt = new Date().toISOString();
    ...
    const requestedTools = Array.isArray(run?.toolExposure?.tools)
      ? run.toolExposure.tools.map((tool: any) => String(tool?.id || '').trim()).filter(Boolean)
      : [];
  ...
  public recordAgentGatewayRunException(task: Task, taskGoal: string, error: any): void {
  ...
  public recordAutonomousTaskOutcome(task: Task, taskGoal: string, result: any): void {
    const finishedAt = new Date().toISOString();
  ...
  public recordAutonomousTaskException(task: Task, taskGoal: string, error: any): void {
    const continuityContext = buildWorkspaceContinuityContext(task, String(task.source || 'telegram').trim());
    task.planner_used = 'supervisor_graph';
    task.executor_used = 'god_mode';
    task.result_summary = null;
