...
export type TelegramExecutionPlannedTaskServiceDeps = {
  ...
  storeExecutionResult: (task: Task, result: any) => void;
  formatExecutionOutput: (label: string, workspace: string, result: any) => string;
};
...
export class TelegramExecutionPlannedTaskService {
  ...
  public async executePlannedTask(task: Task): Promise<{ output: string; success: boolean }> {
    ...
    const toolSteps = task.actions_planned.filter((step: any) => step.type === 'tool');
    ...
    const shellCommands = task.actions_planned
      .filter((step: any) => step.type === 'shell' && step.command)
      .map((step: any) => step.command as string);
  ...
  private enrichToolArgsWithTrace(args: any, task: Task): any {
