  ...
  public async connectAll(): Promise<void> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === 'rejected') {
        logger.error(
          `[ZavorthMcpClient] Failed to connect to MCP server "${enabled[i].label}": ${result.reason}`,
        );
      }
    }
  }
  ...
  public async callTool(toolName: string, args: Record<string, unknown>): Promise<McpToolCallResult> {
    ...
    try {
      ...
      return {
        serverId: entry.registration.id,
        toolName,
        content: [{ type: 'text', text: `error invoking "${toolName}": ${error instanceof Error ? err.message : String(error)}` }],
        isError: true,
      };
    }
  }
