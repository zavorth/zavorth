...
export interface WhatsAppGatewayLocalMessage {
  userId: string;
  chatId: string;
  rawText: string;
  isGroup?: boolean;
...
export type WhatsAppGatewayStatusSnapshot = {
  ...
  provider: 'local' | 'cloud-api' | 'baileys';
...
export class WhatsAppGateway implements LiveChannelBroadcastGatewayContract {
  ...
  public readStatus(): WhatsAppGatewayStatusSnapshot | null {
    ...
    } catch (error: unknown) {logger.warn('[WhatsApp local gateway] JSON parse failed', error); return null; }
  }
  ...
  public async simulateIncomingMessage(message: WhatsAppGatewayLocalMessage): Promise<void> {
  ...
  public async onMessageReceived(webhookPayload: Record<string, unknown>): Promise<boolean> {
    const result = await this.handleWebhookEvent({ body: webhookPayload });
  ...
  public async broadcast(message: string): Promise<void> {
    ...
    this.writeLocalEnvelope(normalizedMessage, recipients);
  }
  ...
  private getProviderMode(): 'local' | 'cloud-api' | 'baileys' {
  ...
  private async buildLoginQrSnapshot(includeDataUrl: boolean): Promise<WhatsAppLoginQrSnapshot> {
    ...
    try {
      ...
      const err = asErrorLike(error);
      logger.warn('[WhatsApp local gateway] validation failed', error);
    return {
        ...status,
        state: 'error',
        dataUrl: null,
  ...
  private buildLoginQrStatusSnapshot(): WhatsAppLoginQrSnapshot {
    ...
    if (provider === 'cloud-api') {
      return {
        ...
        expiresAt: null,
        updatedAt,
        nextStep: this.isProviderConfigured(provider) ? 'Cloud API uses a persistent webhook/token; monitor /api/webhooks/whatsapp instead of QR.'
    ...
    if (qrText) {
      ...
      return {
        ...
        expiresAt,
        updatedAt,
        nextStep: expired ? 'QR expired; request relink or restart the local bridge to publish a new qr.txt.'
    ...
    return {
      ...
      expiresAt: null,
      updatedAt,
      nextStep: String(config.whatsappSessionDir || '').trim() ? 'Waiting for the local bridge to publish qr.txt inside the WhatsApp session.'
  ...
  private isProviderConfigured(provider: 'local' | 'cloud-api' | 'baileys'): boolean {
    if (provider === 'cloud-api') {
      return Boolean(
  ...
  private async replyToChat(chatId: string, text: string, replyToMessageId: string | null): Promise<void> {
    ...
    this.writeLocalEnvelope(text, [chatId], {
      chatId,
  ...
  private async sendCloudApiTextMessage(
    ...
    } catch (error: unknown) {logger.warn('[WhatsApp local gateway] load operation failed', error);
  ...
  private buildProviderModeLabel(provider: 'local' | 'cloud-api' | 'baileys'): string {
    if (provider === 'cloud-api') {
  ...
  private buildProviderDecision(
    ...
    if (provider === 'cloud-api') {
      return providerConfigured ? 'Cloud API connected; webhook verification, official inbound, and official outbound are active.'
        : 'Cloud API selected as the target provider, but minimum credentials are still missing to activate the runtime.';
    }
    if (provider === 'baileys') {
      return providerConfigured ? 'Baileys selected as the target provider with a configured supervised local session.'
        : 'Baileys selected as the target provider; WHATSAPP_SESSION_DIR still needs to be configured.';
    }
    return 'local transport is kept until the official WhatsApp provider is connected.';
  }
