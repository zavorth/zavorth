...
import { logger } from '../../../logger.js';export interface EmailGatewayLocalMessage {
  from: string;
  subject?: string | null;
  text: string;
...
export class EmailGateway implements LiveChannelBroadcastGatewayContract {
  ...
  public readStatus(): EmailGatewayStatusSnapshot | null {
    ...
    } catch (error: unknown) {logger.warn('[Email way.local] JSON parse failed', error); return null; }
  }
  ...
  public async simulateIncomingMessage(message: EmailGatewayLocalMessage): Promise<void> {
