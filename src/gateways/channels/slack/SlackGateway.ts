...
export interface SlackGatewayLocalMessage {
  userId: string;
  channelId: string;
  rawText: string;
...
export class SlackGateway implements LiveChannelBroadcastGatewayContract {
  ...
  public readStatus(): SlackGatewayStatusSnapshot | null {
    ...
    } catch (error: unknown) {logger.warn('[Slack local gateway] JSON parse failed', error); return null; }
  }

  public async simulateIncomingMessage(message: SlackGatewayLocalMessage): Promise<void> {
  ...
  public getIdentityHints(): { linkedBy: string; verificationMethod: string } {
    ...
    return {
      linkedBy: 'slack-gateway',
      verificationMethod: 'slack-local-outbox',
    };
  }
  ...
  public doctorSnapshot(): {
    ...
    return {
      ...
      summary: allowlistConfigured ? `Slack spine ${mode} ready for local inbound/outbound with allowlist.`
  ...
  private resolveMode(): 'native' | 'local' {
    ...
    return String(config.slackBotToken || '').trim() ? 'native' : 'local';
  }

  private resolveModeLabel(): string {
    return this.resolveMode() === 'native' ? 'native' : 'local';
  }
  ...
  private async replyToChannel(channelId: string, text: string, threadTs: string | null): Promise<void> {
    ...
    this.writeLocalEnvelope(text, [channelId], {
      channelId,
      threadTs,
      kind: 'reply',
    });
  ...
  private async callSlackApi(
    ...
    const token = String(config.slackBotToken || '').trim();
    if (!token) {
      this.lastError = 'Slack native requires SLACK_BOT_TOKEN to be configured.';
      this.writeStatus();
      throw new Error(this.lastError);
    }
    if (!this.fetchImpl) {
    ...
    } catch (error: unknown) {logger.warn('[Slack local gateway] network request failed', error);
  ...
  private persistLocalEnvelope(envelope: Record<string, unknown>): void {
  ...
  private writeLocalEnvelope(message: string, recipients: string[]): void;
