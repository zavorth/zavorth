...
export interface InstagramGatewayLocalMessage {
  userId: string;
  chatId: string;
  rawText: string;
}
...
export type InstagramGatewayMode = 'local' | 'meta-messaging';
...
export class InstagramGateway implements LiveChannelBroadcastGatewayContract {
  ...
  public readStatus(): InstagramGatewayStatusSnapshot | null {
    ...
    } catch (error: unknown) {logger.warn('[Instagram local gateway] JSON parse failed', error); return null; }
  }
  ...
  public async simulateIncomingMessage(message: InstagramGatewayLocalMessage): Promise<void> {
  ...
  public handleWebhookVerification(url: URL): { statusCode: number; textBody: string } {
    ...
    if (!verifyToken) {
      this.lastError = 'Instagram Messaging API requires INSTAGRAM_WEBHOOK_VERIFY_TOKEN to be configured.';
      this.writeStatus();
      return {
        statusCode: 503,
        textBody: 'Instagram webhook verify token missing.',
      };
    }
    ...
    if (mode !== 'subscribe' || token !== verifyToken) {
      this.lastError = 'Instagram Messaging API rejected webhook verification.';
      this.writeStatus();
      return {
        statusCode: 403,
        textBody: 'forbidden',
  ...
  private resolveMode(): InstagramGatewayMode {
    return config.instagramProvider === 'meta-messaging' ? 'meta-messaging' : 'local';
  }

  private resolveModeLabel(): string {
    return this.resolveMode() === 'meta-messaging' ? 'Meta Messaging API' : 'local transport';
  }
  ...
  private async replyToRecipient(recipientId: string, text: string): Promise<void> {
    ...
    this.writeLocalEnvelope(text, [recipientId], {
      recipientId,
      kind: 'reply',
    });
  }
  ...
  private async sendMetaTextMessage(recipientId: string, text: string): Promise<InstagramApiResponse> {
    ...
    } catch (error: unknown) {logger.warn('[Instagram local gateway] load operation failed', error);
      ...
      JSON.stringify(
        {
          ...
          providerDecision: mode === 'meta-messaging'
            ? providerConfigured ? 'Instagram Messaging API configured for real inbound and outbound webhooks through Meta Graph.'
              : 'Instagram Messaging API selected, but business account id, access token, or webhook verify token is missing.'
            : 'Instagram remains on local outbox until official Meta credentials are connected.',
          businessAccountId: String(config.instagramBusinessAccountId || '').trim() || null,
          webhookConfigured,
