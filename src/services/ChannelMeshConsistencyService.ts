...
export class ChannelMeshConsistencyService {
  ...
  constructor(runtime: ChannelMeshConsistencyRuntime = {}) {
  ...
  public buildSnapshot(input: { sourceChannels?: string[] } = {}): ChannelMeshConsistencySnapshot {
    ...
    return {
      ...
      contractVersion: ZAVORTH_CHANNEL_MESH_CONSISTENCY_CONTRACT_VERSION,
  ...
  public buildEntry(sourceName: string): ChannelMeshConsistencyEntry {
  ...
  private buildRoute(sourceName: string, plan: ChannelPlan): ChannelMeshConnectorRoute {
    return {
      ...
      doctorCommand: 'npm run channel-mesh-consistency:check',
  ...
  private buildSimulation(route: ChannelMeshConnectorRoute): ChannelMeshConsistencySimulation {
    ...
    return {
      inbound: {
        ...
        normalized: true,
        metadata: {
          source: 'channel-mesh-consistency',
          channelId,
          dryRun: true,
        },
      },
  ...
  private resolveGatewayStrategy(status: ChannelAdapterStatus): ChannelMeshConsistencyTransportStrategy {
  ...
  private buildFindings(
    ...
    if (!gatewayStatus) {
      findings.push('channel route generated from Credential vault consistency template');
    }
