/**
 * SessionAnalyticsZavorthControl — Analytics and metrics for sessions, tools, and performance.
 *
 * Tracks session activity, tool usage, token consumption, and provides
 * zavorthControl-ready data for monitoring agent performance.
 *
 * Usage:
 *   const zavorthControl = new SessionAnalyticsZavorthControl();
 *   zavorthControl.recordSessionStart('ses_123', { model: 'gpt-4o', workspace: '/project' });
 *   zavorthControl.recordToolCall('ses_123', 'read_file', { success: true, durationMs: 45 });
 *   zavorthControl.recordTokenUsage('ses_123', { input: 1500, output: 800 });
 *   const stats = zavorthControl.getSessionStats('ses_123');
 *   const overview = zavorthControl.getOverview();
 */
...
export interface ZavorthControlOverview {
...
export class SessionAnalyticsZavorthControl {
  ...
  /**
   * Gets zavorthControl overview.
   */
  getOverview(): ZavorthControlOverview {
    const now = Date.now();
