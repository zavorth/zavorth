...
import type {
  ...
  SatelliteAppParityEntry,
  SatelliteAppParitySnapshot,
  SatelliteAppParityStatus,
  SatelliteAppParitySurface,
} from '../contracts/SatelliteAppParityContract.js';
import { ZAVORTH_SATELLITE_APP_PARITY_CONTRACT_VERSION } from '../contracts/SatelliteAppParityContract.js';
...
type SatelliteAppParityRuntime = {
  now?: () => Date;
  rootDir?: string;
...
type SurfaceSpec = {
  ...
  summary: string;
  targetFiles: SatelliteAppParityEntry['targetFiles'];
  markers: Array<{
...
const SPECS: SurfaceSpec[] = [
  ...
  },
  {
    ...
    nodeCapabilityId: null,
    summary: 'Native Android/iOS/desktop wrapper decision for app parity.',
    targetFiles: {
      contract: 'src/contracts/SatelliteAppParityContract.ts',
      service: 'src/services/SatelliteAppParityService.ts',
...
export class SatelliteAppParityService {
  private readonly now: () => Date;
  private readonly files: Record<SatelliteSourceFileKey, string>;

  constructor(runtime: SatelliteAppParityRuntime = {}) {
  ...
  public buildSnapshot(): SatelliteAppParitySnapshot {
    const entries = SPECS.map((spec) => this.buildEntry(spec));
    return {
      generatedAt: this.now().toISOString(),
      contractVersion: ZAVORTH_SATELLITE_APP_PARITY_CONTRACT_VERSION,
      summary: {
  ...
  public buildEntryForSurface(surface: SatelliteAppParitySurface): SatelliteAppParityEntry {
  ...
  private buildEntry(spec: SurfaceSpec): SatelliteAppParityEntry {
    ...
    return {
      ...
      smokeGate: {
        ...
        command: `SatelliteAppParityService.buildEntryForSurface(${JSON.stringify(spec.surface)})`,
        liveDeviceRequired: false,
  ...
  private resolveStatus(spec: SurfaceSpec, present: number, allPresent: boolean): SatelliteAppParityStatus {
  ...
  ): SatelliteAppParityEntry['simulation'] {
    return {
      dryRun: true,
      request: {
        primitiveId: spec.primitiveId,
  ...
  private buildFindings(
    ...
    if (status === 'native' || status === 'backend-ready' || status === 'pwa-shell') {
      return [`${spec.surface} has governed Satellite/App parity evidence`];
    }
    if (status === 'decision-required') {
      return ['native wrapper remains a product decision; keep PWA-first until browser capability parity is real'];
    }
    ...
    if (status === 'template-ready') {
      return [`${spec.surface} needs client implementation template before runtime parity`];
    }
    return [`${spec.surface} is missing Satellite/App parity evidence`];
  }

  private buildPluginManifest(entries: SatelliteAppParityEntry[]): ZavorthPluginManifest {
    ...
    return {
      ...
      moduleKind: 'bridge',
      summary: 'Plugin OS PWA/device companion module for Node Mesh app parity.',
      ...
      source: {
        kind: 'generated',
        locator: 'zavorth-normalized://satellite-app-parity',
        digest: null,
        trusted: false,
      },
      compatibility: {
  ...
  private readSource(runtime: SatelliteAppParityRuntime, key: SatelliteSourceFileKey): string {
...
function baseTargets(client: string): SatelliteAppParityEntry['targetFiles'] {
  return {
    contract: 'src/contracts/SatelliteContract.ts',
