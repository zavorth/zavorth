/**
 * Phase 2 — Dictation-first ingress (Claude Code model).
...
const MEDIA_PLACEHOLDER_RE =
  /^\[(?:audio enviado|automatically transcribed audio|audio|voice)[^\]]*\]\s*/i;
...
export class VoiceDictationIngress {
  ...
  public prepare(input: VoiceDictationIngressInput): VoiceDictationIngressResult {
    ...
    const showTranscript =
      Boolean(input.forceShowTranscript) ||
      mode === 'dictation' ||
      mode === 'conversation' ||
      lowConfidence;
    ...
    return {
      ...
      reason:
        mode === 'conversation'
          ? 'conversation_dictation_to_agent'
          : 'dictation_to_agent',
      metadata: {
        source: 'voice_dictation',
...
export function getVoiceDictationIngress(
  options?: { voicePreferences?: VoicePreferenceService },
