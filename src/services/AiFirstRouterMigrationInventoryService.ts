...
export type AiFirstRouterInventoryEntry = {
  ...
  phaseTarget: 'phase-1' | 'phase-2' | 'phase-3' | 'phase-4' | 'phase-5' | 'phase-7' | 'keep';
  reason: string;
  aiFirstRole: string;
  evidence: string[];
};
...
const INVENTORY_ENTRIES: AiFirstRouterInventoryEntry[] = [
  ...
  },
  {
    ...
    migrationDecision: 'promote-ai-first',
    phaseTarget: 'phase-7',
    reason: 'Deve virar fallback/compatibilidade para surfaces antigas enquanto o router padrao assume texto livre.',
    aiFirstRole: 'Compatibilidade para comandos naturais antigos e fixtures de regressao.',
    evidence: ['extractSharedSurfaceNaturalChannelId', 'parseSharedSurfaceNaturalMemoryIntent', 'parseSharedSurfaceNaturalLearningIntent'],
  },
  {
...
export class AiFirstRouterMigrationInventoryService {
      ...
      gates: [
        {
          ...
          status: 'passed',
          detail: 'Phase 0 so inventaria e classifica; nenhuma rota existente foi alterada.',
        },
        {
        ...
        {
          id: 'phase-0-ai-first-candidates',
  ...
  public renderMarkdown(snapshot = this.buildSnapshot()): string {
    const lines = [
      '# Zavorth AI-First Router - Phase 0 Inventory',
      '',
      ...
      '',
      'Phase 1: criar o contrato do plano IA, sem executar nada ainda.',
    ];
