...
export class ZavorthProductQaLiveService {
  ...
  public formatSnapshotText(snapshot: ZavorthProductQaLiveSnapshot): string {
    const lines = [
      ...
      `- ZavorthControl: ${snapshot.liveReadiness.zavorthControlCovered ? 'covered' : 'missing'}`,
  ...
  private buildLiveReadiness(): ZavorthProductQaLiveReadiness {
    return {
      ...
      zavorthControlCovered: this.hasAll(['scripts/zavorth-zavorthControl-final-product-polish-check.mjs', 'src/ai-gateway/app/(zavorthControl)/control/useControlPageClient.ts']),
  ...
  private buildMatrix(readiness: ZavorthProductQaLiveReadiness): ZavorthProductQaLiveRow[] {
    ...
    return [
      ...
      row({
        ...
        nextSafeAction: 'Create a safe mutation request and approve/reject it through CLI or /zavorthControl.',
      }),
      row({
        id: 'receipt',
      ...
      row({
        id: 'zavorthControl',
        label: 'ZavorthControl /zavorthControl',
        status: readiness.zavorthControlCovered ? 'dry-run-certified' : 'blocked',
        mode: 'hybrid',
        liveProof: 'optional',
        evidence: [
          evidence(readiness.zavorthControlCovered, 'ZavorthControl final product polish check and /zavorthControl client are present'),
        ],
        commands: [
          'npm run zavorth:zavorthControl-final-product-polish:check --silent',
          'zavorth open',
        ],
        nextSafeAction: 'Open /zavorthControl and verify chat, approvals, receipts, learning and zavorthControl health.',
      }),
      row({
        id: 'cli',
