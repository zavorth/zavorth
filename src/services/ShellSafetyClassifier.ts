...
function reasonFor(input: {
  ...
  if (input.attention) return 'Command contains mutation/zavorthControl operators and requires explicit approval.';
...
function hasTreeSitter(): boolean {
  try {
    ...
    try {
      ...
      return true;
    } catch (error: unknown) { logger.warn('[Shell Safety Classifier] lifecycle operation failed', error); return false; }
  }
}
