import {
  ...
  type ZavorthCommandCenterVisualApprovalBlock,
  type ZavorthCommandCenterVisualApprovalPack,
} from '../contracts/ZavorthCommandCenterVisualApprovalPackContract.js';
import type { ZavorthCommandCenterProviderCockpitProjection } from '../contracts/ZavorthCommandCenterProviderCockpitContract.js';
import {
  ZavorthCommandCenterProviderCockpitService,
  type ZavorthCommandCenterProviderCockpitInput,
} from './ZavorthCommandCenterProviderCockpitService.js';

export type ZavorthCommandCenterVisualApprovalPackInput = ZavorthCommandCenterProviderCockpitInput & {
  includeDetailsDrawer?: boolean;
};

export type ZavorthCommandCenterVisualApprovalPackRuntime = {
  now?: () => Date;
  cockpit?: Pick<ZavorthCommandCenterProviderCockpitService, 'buildProjection'>;
};

export class ZavorthCommandCenterVisualApprovalPackService {
  private readonly now: () => Date;
  private readonly cockpit: Pick<ZavorthCommandCenterProviderCockpitService, 'buildProjection'>;

  constructor(runtime: ZavorthCommandCenterVisualApprovalPackRuntime = {}) {
    this.now = runtime.now || (() => new Date());
    this.cockpit = runtime.cockpit || new ZavorthCommandCenterProviderCockpitService({
      now: this.now,
    });
  }

  public async buildPack(input: ZavorthCommandCenterVisualApprovalPackInput = {}): Promise<ZavorthCommandCenterVisualApprovalPack> {
    ...
    return {
      ...
      receipts: [
        {
          ...
          status: 'recorded',
          detail: `${blocks.length} Command Center provider visual block(s) proposed; none rendered yet.`,
        },
        {
      ...
      nextAction: 'Owner approval is required before implementing or rendering these Command Center blocks.',
    };
  }

  public renderText(pack: ZavorthCommandCenterVisualApprovalPack): string {
    return [
      '[command-center-visual-approval]',
      `status=${pack.status}`,
...
function buildBlocks(
  cockpit: ZavorthCommandCenterProviderCockpitProjection,
  includeDetailsDrawer: boolean,
): ZavorthCommandCenterVisualApprovalBlock[] {
  const blocks: ZavorthCommandCenterVisualApprovalBlock[] = [
    {
    ...
    },
    {
      ...
      dataBindings: [
        'actions',
        'cards.actions',
        'safety.commandCenterCannotExecuteProviderCalls',
      ],
      interactionModel: cockpit.actions.map((action) => ({
        id: action.id,
        label: action.label,
