...
export class SelfModificationArtifactStore {
  ...
  public ensureDirectories(extraDirectories: string[] = []): void {
    for (const directory of [this.previewDir, this.goalPreviewDir, this.historyDir, ...extraDirectories]) {
  ...
  public readPreviewArtifact(previewId: string): PreviewArtifact | null {
    ...
    for (const kind of ['goal', 'multi'] as const) {
      const p = this.getPreviewArtifactPath(previewId, kind);
      if (fs.existsSync(p)) {
        return JSON.parse(fs.readFileSync(p, 'utf8')) as ChangeSetManifest;
      }
    }
  ...
  public deletePreviewArtifact(previewId: string, kind: 'file' | 'goal' | 'multi'): void {
  ...
  public tryDeletePreviewArtifact(previewId: string, kind: 'file' | 'goal' | 'multi'): void {
    try {
      this.deletePreviewArtifact(previewId, kind);
    } catch (error: unknown) {
      // Residual artifact must not invalidate apply success.
      logger.warn('[Self Modification Artifact Store] file cleanup failed', error);
    }
  }

  public getPreviewArtifactPath(previewId: string, kind: 'file' | 'goal' | 'multi'): string {
    const baseDir = kind === 'file' ? this.previewDir : this.goalPreviewDir;
    const suffix = kind === 'multi' ? `${previewId}.multi.json` : `${previewId}.json`;
