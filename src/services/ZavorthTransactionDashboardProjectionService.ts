import {
  ...
  type ZavorthTransactionDashboardContractSnapshot,
  type ZavorthTransactionDashboardLane,
  type ZavorthTransactionDashboardNotification,
  type ZavorthTransactionDashboardOperatorAction,
  type ZavorthTransactionDashboardProjectInput,
  type ZavorthTransactionDashboardProjection,
  type ZavorthTransactionDashboardSafety,
  type ZavorthTransactionDashboardTile,
  type ZavorthTransactionDashboardTimelineItem,
  type ZavorthTransactionDashboardTimelineStatus,
  type ZavorthTransactionDashboardTone,
...
type TransactionDashboardDeps = {
  now?: () => Date;
  surfaceGateway?: ZavorthTransactionSurfaceGatewayService;
};

const SAFETY: ZavorthTransactionDashboardSafety = {
...
export class ZavorthTransactionDashboardProjectionService {
  private readonly now: () => Date;
  private readonly surfaceGateway: ZavorthTransactionSurfaceGatewayService;

  public constructor(deps: TransactionDashboardDeps = {}) {
    this.now = deps.now ?? (() => new Date());
    this.surfaceGateway = deps.surfaceGateway ?? new ZavorthTransactionSurfaceGatewayService();
  }

  public buildSnapshot(): ZavorthTransactionDashboardContractSnapshot {
    return buildZavorthTransactionDashboardContractSnapshot();
  }

  public project(input: ZavorthTransactionDashboardProjectInput): ZavorthTransactionDashboardProjection {
    return this.fromSurfaceProjection(this.surfaceGateway.project(input));
  }

  public fromSurfaceProjection(surfaceProjection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardProjection {
    ...
    return {
      ...
      id: `${surfaceProjection.id}.dashboard`,
      generatedAt,
  ...
  public renderReport(projection: ZavorthTransactionDashboardProjection): string {
    return [
      '[transaction-dashboard] Dashboard controls transaction Dashboard projection',
      `[transaction-dashboard] status: ${projection.status}`,
      `[transaction-dashboard] tone: ${projection.tone}`,
      `[transaction-dashboard] headline: ${projection.headline}`,
      `[transaction-dashboard] surface: ${projection.surface}`,
      `[transaction-dashboard] source-projection: ${projection.sourceProjectionId}`,
      `[transaction-dashboard] lanes: ${projection.lanes.length}`,
      `[transaction-dashboard] tiles: ${projection.tiles.length}`,
      `[transaction-dashboard] timeline: ${projection.timeline.length}`,
      `[transaction-dashboard] operator-actions: ${projection.operatorActions.length}`,
      `[transaction-dashboard] no-live-execution: ${projection.safety.noLiveExecution}`,
      `[transaction-dashboard] no-hidden-live-action: ${projection.safety.noHiddenLiveAction}`,
      `[transaction-dashboard] no-raw-secret-serialized: ${projection.safety.noRawSecretSerialized}`,
      `[transaction-dashboard] live-action-applied: ${projection.safety.liveActionApplied}`,
      ...projection.lanes.map((lane) => `[transaction-dashboard] lane: ${lane.kind} status=${lane.status} severity=${lane.severity}`),
      ...projection.operatorActions.map((action) => `[transaction-dashboard] action: ${action.id} enabled=${action.enabled} placement=${action.placement}`),
    ].join('\n');
  }
}

function buildLanes(projection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardLane[] {
  ...
  return [
    ...
    lane('safety', 'Safety', 'Dashboard nao recebe autoridade de execucao live.', 'info', 'live-disabled', [
...
): ZavorthTransactionDashboardTile[] {
  const runtime = projection.runtime;
  return [
...
function buildTimeline(projection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardTimelineItem[] {
  const runtime = projection.runtime;
  return [
...
function buildOperatorActions(actions: ZavorthTransactionSurfaceAction[]): ZavorthTransactionDashboardOperatorAction[] {
...
function buildNotifications(
  projection: ZavorthTransactionSurfaceProjection,
  tone: ZavorthTransactionDashboardTone,
): ZavorthTransactionDashboardNotification[] {
  return [
    {
...
function lane(
  kind: ZavorthTransactionDashboardLane['kind'],
  title: string,
  summary: string,
  severity: ZavorthTransactionSurfaceSeverity,
  status: string,
  facts: Array<[string, string]>,
): ZavorthTransactionDashboardLane {
  return {
    id: `transaction-dashboard.${kind}`,
    kind,
    title,
    summary,
    severity,
...
function tile(
  kind: ZavorthTransactionDashboardTile['kind'],
  label: string,
  value: string,
  detail: string,
  tone: ZavorthTransactionDashboardTone,
  ...
  return {
    id: `transaction-dashboard.tile.${kind}`,
    kind,
    label,
    value,
    detail,
...
): ZavorthTransactionDashboardTimelineItem {
  return {
    id,
    label,
    detail,
...
function toneForStatus(status: ZavorthTransactionRuntimeStatus): ZavorthTransactionDashboardTone {
  if (status === 'simulated') {
    return 'success';
  }
  if (status === 'blocked') {
...
function toneForRisk(risk: string): ZavorthTransactionDashboardTone {
  if (risk === 'danger') {
    return 'blocked';
  }
  if (risk === 'attention') {
...
function approvalTimelineStatus(projection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardTimelineStatus {
...
function credentialTimelineStatus(projection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardTimelineStatus {
...
function connectorTimelineStatus(projection: ZavorthTransactionSurfaceProjection): ZavorthTransactionDashboardTimelineStatus {
...
function actionPlacement(action: ZavorthTransactionSurfaceAction): ZavorthTransactionDashboardOperatorAction['placement'] {
  if (!action.enabled) {
    return 'disabled';
  }
