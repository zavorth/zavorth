  ...
  private buildItems(): ZavorthCapabilityAbsorptionItem[] {
    ...
    return [
      ...
      item({
        ...
        label: 'OpenClaw-style long-tail channels',
        category: 'channel',
        source: 'openclaw',
      ...
      item({
        ...
        label: 'Hermes-style learning loop',
        category: 'learning',
        source: 'hermes',
        ...
        currentEvidence: ['zavorth:native-learning-loop:check', 'zavorth:memory-learning-loop:check', 'docs/native-learning-loop.md', 'docs/mnemos-memory-os.md'],
      ...
      item({
        ...
        label: 'Hermes-scale built-in skills',
        category: 'skill',
        source: 'hermes',
      ...
      item({
        id: 'dashboard-hermes-ui',
        label: 'Hermes-class Dashboard',
        category: 'dashboard',
        source: 'hermes',
        desiredOutcome: 'Expose tool cards, subagent cards, streaming, approval cards, context usage and retry/edit flows in /dashboard.',
        status: script('zavorth:dashboard-final-product-polish:check') ? 'partial' : 'cataloged',
        zavorthNativeSurface: '/dashboard Dashboard',
        currentEvidence: ['zavorth:dashboard-final-product-polish:check', 'docs/web-dashboard.md'],
        ...
        risks: ['Dashboard remains a control surface; execution still routes through Policy Broker.'],
        nextPhase: 'phase-4-dashboard',
      }),
      item({
        id: 'cli-terminal-tui',
      ...
      item({
        ...
        category: 'qa',
        source: 'zavorth',
        desiredOutcome: 'Prove install, provider, Telegram/channel, mutation approval, dashboard and receipt flow end to end.',
...
function summarize(items: ZavorthCapabilityAbsorptionItem[]): ZavorthCapabilityAbsorptionSnapshot['summary'] {
  ...
  return {
    ...
    openclawInspired: items.filter((item) => item.source === 'openclaw').length,
    hermesInspired: items.filter((item) => item.source === 'hermes').length,
    crossAgent: items.filter((item) => item.source === 'cross-agent').length,
    nextPhases,
