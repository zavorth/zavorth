...
function providerFix(
  ...
  if (providerSnapshot && providerSnapshot.summary.ready > 0 && providerSnapshot.summary.defaultRouteAllowed === 0) {
    return fix({
      ...
      label: 'validate provider com prova live',
      kind: 'provider-live-proof',
      risk: 'low',
      summary: `Executa um probe explicit em ${providerId} e salva evidence sanitaria without secret bruto.`,
  ...
  return fix({
    ...
    kind: 'manual-setup',
    risk: 'read_only',
    summary: 'Shows which providers need credentials or a base URL before using a real LLM.',
...
function readyFix(readiness: ZavorthRuntimeReadinessSnapshot): ZavorthRuntimeGuidedFix {
  return {
    ...
    kind: 'open-route',
    risk: 'read_only',
    summary: 'Zavorth is ready; abrir o ZavorthControl e usar normalmente.',
    command: 'zavorth go',
    route: '/zavorthControl',
    telegramCommand: '/zavorthControl',
