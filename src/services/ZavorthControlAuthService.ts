...
export type ZavorthControlAuthenticatedIdentity = {
  authenticated: true;
  source: 'jwt' | 'zavorthControl-token';
  userId: string;
  profileId: string | null;
} | TrustedDeviceIdentity;

export class ZavorthControlAuthService {
  ...
  }): ZavorthControlAuthenticatedIdentity | null {
  ...
  private resolveJwtIdentity(token: string): ZavorthControlAuthenticatedIdentity | null {
    ...
    try {
      ...
      const expectedIssuer = String(process.env.ZAVORTH_CONTROL_JWT_ISSUER || '').trim();
      const expectedAudience = String(process.env.ZAVORTH_CONTROL_JWT_AUDIENCE || '').trim();
