 ...
 */
export class DashboardEchoRouteService {
    ...
    if (canonicalPathname === '/api/v2/echo/execute' && req.method === 'POST') {
      try {
        ...
        if (routeSurface === 'nexus') {
          const parsed = parseDashboardRouteBody(
            NexusExecuteRequestSchema,
            body,
  ...
  private resolveRouteSurface(pathname: string): DashboardRouteSurface | null {
