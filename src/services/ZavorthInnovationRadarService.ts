...
function validateFeedUrl(feedUrl: string, allowedHosts: Set<string>): { ok: boolean; url: string | null; reason: string } {
  ...
  } catch (error: unknown) {logger.warn('[Zavorth Innovation Radar] network request failed', error);
    return { ok: false, url: null, reason: 'Feed URL is invalid.' };
  }
}

function normalizeCategory(value: ZavorthInnovationRadarCategory | null | undefined, _text: string): ZavorthInnovationRadarCategory {
...
function safeEvidenceUrl(value: string | null | undefined): string | null {
  ...
  try {
    ...
    url.hash = '';
    return url.toString();
  } catch (error: unknown) {logger.warn('[Zavorth Innovation Radar] search failed', error); return null; }
}
