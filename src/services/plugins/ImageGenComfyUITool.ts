...
export class ImageGenComfyUITool extends BaseTool {
  ...
  private async checkStatus(serverUrl: string): Promise<string> {
    ...
    } catch (error: unknown) {logger.warn('[Image Gen Comfy U I] parsing failed', error); return ''; }
  }
  ...
  private async generate(args: Record<string, unknown>, serverUrl: string): Promise<string> {
    ...
    try {
      ...
      try { fs.unlinkSync(tmpFile); } catch (error: unknown) {/* ignore */ logger.warn('[Image Gen Comfy U I] file cleanup failed', error); }
