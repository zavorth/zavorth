...
export class WebRemotePwaRouteService {
  private readonly satelliteDir: string;

  constructor(workspaceRoot: string = process.cwd()) {
    const webRemoteCandidate = path.resolve(workspaceRoot, 'src', 'web-remote');
    this.satelliteDir = fs.existsSync(webRemoteCandidate)
      ? webRemoteCandidate
      : path.resolve(workspaceRoot, 'src', 'satellite');
  }
...
export const SatellitePwaRouteService = WebRemotePwaRouteService;
