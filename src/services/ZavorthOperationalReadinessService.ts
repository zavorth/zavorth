...
export const ZAVORTH_OPERATIONAL_READINESS_CONTRACT_VERSION = 'zavorth-operational-consistency/1' as const;

export type ZavorthOperationalReadinessDomainId =
  | 'channels'
  | 'gateway'
  | 'plugins'
  | 'liveQa'
...
export type ZavorthOperationalReadinessDomain = {
  id: ZavorthOperationalReadinessDomainId;
  title: string;
...
export type ZavorthOperationalReadinessSnapshot = {
  contractVersion: typeof ZAVORTH_OPERATIONAL_READINESS_CONTRACT_VERSION;
  generatedAt: string;
  status: 'pass' | 'attention' | 'blocked';
  score: number;
  domains: ZavorthOperationalReadinessDomain[];
  safety: {
    noSecretsRead: true;
    noNetworkCalls: true;
...
export type ZavorthOperationalReadinessRuntime = {
  now?: () => Date;
  nativeIntegrations?: ZavorthNativeIntegrationService;
};

export class ZavorthOperationalReadinessService {
  private readonly now: () => Date;
  private readonly nativeIntegrations: ZavorthNativeIntegrationService;

  constructor(runtime: ZavorthOperationalReadinessRuntime = {}) {
  ...
  public buildSnapshot(projectRoot: string): ZavorthOperationalReadinessSnapshot {
    ...
    const domains: ZavorthOperationalReadinessDomain[] = [
      this.domain({
        id: 'channels',
    ...
    return {
      contractVersion: ZAVORTH_OPERATIONAL_READINESS_CONTRACT_VERSION,
  ...
  public renderText(snapshot: ZavorthOperationalReadinessSnapshot): string {
    return [
      '',
      'o  ZAVORTH CERTIFY',
      '|',
      this.box('Operational readiness', [
      ...
      this.box('Safety', [
        ...
        'Only native Zavorth code was executed.',
  ...
  private domain(input: {
    ...
    evidence: string[];
    nextActions: string[];
  }): ZavorthOperationalReadinessDomain {
    return {
      id: input.id,
      title: input.title,
