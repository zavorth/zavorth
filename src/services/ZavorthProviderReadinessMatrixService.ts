...
function parseJson(text: string): unknown | null {
  ...
  } catch (error: unknown) {logger.warn('[Zavorth  Readiness Matrix] JSON parse failed', error); return null; }
}
...
function sanitizeProbeTarget(rawUrl: string): string {
  ...
  } catch (error: unknown) {logger.warn('[Zavorth  Readiness Matrix] creation failed', error); return 'unknown'; }
}
