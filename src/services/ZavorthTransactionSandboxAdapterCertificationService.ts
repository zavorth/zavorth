...
function safeHost(value: string | undefined): string | null {
  ...
  } catch (error: unknown) {logger.warn('[Zavorth Transaction Sandbox Adapter Certification] network request failed', error); return null; }
}

function safeProtocol(value: string | undefined): string | null {
  try {
    return value ? new URL(value).protocol.toLowerCase() : null;
  } catch (error: unknown) {logger.warn('[Zavorth Transaction Sandbox Adapter Certification] operation failed', error); return null; }
}
