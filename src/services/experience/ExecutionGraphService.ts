...
import type { UniversalAgentEvent, UniversalAgentRun } from '../../runtime/agent/UniversalAgentRuntimeTypes.js';
...
export class ExecutionGraphService {
  public build(input: ExecutionGraphBuildInput = {}): ExperienceExecutionGraph {
    ...
    const nodes = run ? this.nodesFromRun(run) : this.nodesFromTimeline(input.timeline || [], now);
  ...
  private nodesFromRun(run: UniversalAgentRun): ExperienceExecutionGraphNode[] {
    ...
    return [...base, ...eventNodes, ...artifactNodes].sort((left, right) =>
      ...
      return [
        {
          ...
          kind: 'router',
          status: 'pending',
          detail: 'No active journey was found.',
          createdAt: now,
        },
      ];
    }
  ...
  private kindFromEvent(kind: UniversalAgentEvent['kind'], title: string): ExperienceExecutionGraphNode['kind'] {
    ...
    if (kind === 'planning')
      return normalized.includes('safety') || normalized.includes('policy') ? 'safety' : 'router';
