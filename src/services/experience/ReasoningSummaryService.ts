import type { ExperienceReasoningSummary, ExperienceTimelineItem, ExperienceTrustLens } from './ExperienceContracts.js';
...
export class ReasoningSummaryService {
  public build(input: ReasoningSummaryBuildInput = {}): ExperienceReasoningSummary {
    ...
    const risk = highestRisk([input.trust?.risk || 'safe', ...(run?.approvals || []).map((approval) => approval.risk)]);
    ...
    return {
      understood: summarize(run?.input || input.fallbackText || '', 'Waiting for a natural user request.'),
      risk,
      tools,
  ...
  private toolsFor(run: UniversalAgentRun | null, timeline: ExperienceTimelineItem[]): string[] {
    ...
    const fromTimeline = timeline.filter((item) => item.kind === 'tool').map((item) => item.title);
