...
export type CommandCenterAccessAction =
  | 'open'
  | 'url'
  | 'token'
  | 'status'
...
export type CommandCenterAccessTokenSource = 'env' | 'runtime-file' | 'generated-runtime-file';

export type CommandCenterAccessSnapshot = {
  url: string;
  publicUrl: string;
  token: string;
  tokenSource: CommandCenterAccessTokenSource;
  tokenFile: string;
  opened: boolean;
  action: CommandCenterAccessAction;
};

export type CommandCenterAccessDoctorSnapshot = {
  ok: boolean;
  action: 'doctor' | 'repair' | 'generate-token';
  status: 'ready' | 'repaired' | 'repairable' | 'env-locked';
  publicUrl: string;
  tokenSource: CommandCenterAccessTokenSource | 'missing';
  tokenFile: string;
...
type CommandCenterAccessConfig = {
  host: string;
  port: number;
  token: string;
  tokenFile: string;
...
type CommandCenterAccessServiceOptions = {
  config?: Partial<CommandCenterAccessConfig>;
...
function normalizeAction(raw: string | null | undefined): CommandCenterAccessAction {
...
export function parseCommandCenterAccessAction(args: string): CommandCenterAccessAction {
  return normalizeAction(String(args || '').trim().split(/\s+/u)[0] || '');
}

export class CommandCenterAccessService {
  private readonly optionsConfig: Partial<CommandCenterAccessConfig>;
  private readonly env: NodeJS.ProcessEnv;
  private readonly spawnImpl: typeof spawnNativeCommand;

  constructor(options: CommandCenterAccessServiceOptions = {}) {
    this.optionsConfig = options.config || {};
    this.env = options.env || process.env;
    this.spawnImpl = options.spawn || spawnNativeCommand;
  }

  public async run(action: CommandCenterAccessAction): Promise<CommandCenterAccessSnapshot> {
    ...
    const snapshot: CommandCenterAccessSnapshot = {
      url,
      publicUrl,
  ...
  public doctor(): CommandCenterAccessDoctorSnapshot {
    return this.buildDoctorSnapshot('doctor', false, false);
  }

  public repair(): CommandCenterAccessDoctorSnapshot {
  ...
  public generateToken(): CommandCenterAccessDoctorSnapshot {
    const inspection = this.inspectToken();
    if (inspection.source === 'env') {
  ...
  private resolveConfig(): CommandCenterAccessConfig {
    return {
  ...
  private resolveToken(): { token: string; source: CommandCenterAccessTokenSource } {
    const inspected = this.inspectToken();
    if (inspected.token) {
      return { token: inspected.token, source: inspected.source as CommandCenterAccessTokenSource };
    }
  ...
  private inspectToken(): {
    token: string | null;
    source: CommandCenterAccessTokenSource | 'missing';
    tokenFileExists: boolean;
    tokenFileReadable: boolean;
  } {
    const resolved = this.resolveConfig();
  ...
  private buildDoctorSnapshot(
    ...
    generated: boolean,
    extraNotes: string[] = [],
  ): CommandCenterAccessDoctorSnapshot {
    const inspected = this.inspectToken();
    const resolved = this.resolveConfig();
    const rawEnvToken = String(this.env.ZAVORTH_WEB_AUTH_TOKEN || resolved.token || '').trim();
    const weakEnvTokenIgnored = Boolean(rawEnvToken && isWeakCommandCenterToken(rawEnvToken));
