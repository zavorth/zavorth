...
export type AIGatewayDeforkConvergenceStatus = 'ready' | 'partial' | 'blocked';

export type AIGatewayDeforkConvergenceItemId =
...
export type AIGatewayDeforkConvergenceItem = {
  id: AIGatewayDeforkConvergenceItemId;
  label: string;
  status: AIGatewayDeforkConvergenceStatus;
...
export type AIGatewayDeforkConvergenceAcceptance = {
...
export type AIGatewayDeforkConvergenceSnapshot = {
  schemaVersion: 1;
  phase: 'C8';
  generatedAt: string;
  status: AIGatewayDeforkConvergenceStatus;
  ...
  items: AIGatewayDeforkConvergenceItem[];
  acceptance: AIGatewayDeforkConvergenceAcceptance;
...
export type AIGatewayDeforkConvergenceInput = {
...
type AIGatewayDeforkConvergenceRuntime = {
  now?: () => Date;
};
...
export class AIGatewayDeforkConvergenceService {
  private readonly now: () => Date;

  constructor(runtime: AIGatewayDeforkConvergenceRuntime = {}) {
    this.now = runtime.now || (() => new Date());
  }

  public buildSnapshot(input: AIGatewayDeforkConvergenceInput = {}): AIGatewayDeforkConvergenceSnapshot {
  ...
  private buildGatewayHandoffItem(input: AIGatewayDeforkConvergenceInput): AIGatewayDeforkConvergenceItem {
  ...
  private buildProviderPlaneItem(input: AIGatewayDeforkConvergenceInput): AIGatewayDeforkConvergenceItem {
  ...
  private buildControlPlaneItem(input: AIGatewayDeforkConvergenceInput): AIGatewayDeforkConvergenceItem {
  ...
  private buildProxySseBoundaryItem(input: AIGatewayDeforkConvergenceInput): AIGatewayDeforkConvergenceItem {
  ...
  private buildBudgetProviderHealthItem(
    input: AIGatewayDeforkConvergenceInput,
    activeRun: UniversalAgentRun | null,
  ): AIGatewayDeforkConvergenceItem {
  ...
  private buildAcceptance(
    ...
    items: AIGatewayDeforkConvergenceItem[],
    activeRun: UniversalAgentRun | null,
  ): AIGatewayDeforkConvergenceAcceptance {
    const ready = (id: AIGatewayDeforkConvergenceItemId) => this.itemStatus(items, id) === 'ready';
    const controlApi = input.runtimeSnapshot?.gatewayControlApi || null;
  ...
  private resolveStatus(
    ...
    acceptance: AIGatewayDeforkConvergenceAcceptance,
    blockers: string[],
  ): AIGatewayDeforkConvergenceStatus {
  ...
  ): AIGatewayDeforkConvergenceSnapshot['externalCompatibility'] {
