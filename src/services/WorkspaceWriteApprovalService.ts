...
/**
 * Workspace-plane write approval store.
 *
 ...
 * single-use, workspace-scoped database rows consumed atomically against a
 * request hash and expiry (consumeApproval). They have no chat prompt or
 * dismissal machinery to unify — their only presentation plane is the
 * authenticated ZavorthControl dashboard, and routing them through chat menus
 * would replace request-hash binding with replayable chat tokens. Domain
 * decision logic stays here; cross-surface card retirement for these rows is
 * meaningless because no surface ever renders them as actionable cards.
 */
