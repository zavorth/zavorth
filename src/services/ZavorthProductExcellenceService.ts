...
export class ZavorthProductExcellenceService {
  ...
  private hasMarker(relativePath: string, marker: string): boolean {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Product Excellence] filesystem operation failed', error); return false; }
  }
  ...
  private packageScripts(): Record<string, string> {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Product Excellence] JSON parse failed', error); return {}; }
  }
}
