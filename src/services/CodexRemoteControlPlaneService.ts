          ...
          },
{
            ...
            description: 'Creates an additional Codex Remote profile for another account, workspace, or CODEX_HOME.',
            requiresInput: true,
          },
{
            id: 'update-profile',
            label: 'Update profile',
            description: 'Updates the operational metadata of an existing Codex Remote profile.',
            requiresInput: true,
          },
{
            id: 'delete-profile',
            label: 'Remove profile',
            description: 'Removes a stored Codex Remote profile without affecting the host default profile.',
            requiresInput: true,
          },
{
            id: 'start-session',
            label: 'Start session',
            description: 'Creates a trackable Codex Remote session and starts the Codex CLI in background.',
            requiresInput: true,
          },
        {
          id: 'resume-session',
        ...
        },
{
            ...
            description: 'Prepares remote Codex handoff in a new Zavorth web session.',
            requiresInput: false,
          },
{
            id: 'open-web-session',
            label: 'Attach web session',
            description: 'Opens a web handoff for an existing Codex Remote session.',
            requiresInput: true,
          },
      ],
      handoff: {
...
narrative: {
        headline: `Codex Remote with ${profileSnapshot.profiles.length} profile(s), ${remotePaths.length} transport(s) and ${sessionBroker.summary.totalSessions} session(s).`,
        operatorSummary: [
          cliReady ? 'Codex CLI responding.' : 'Codex CLI still unavailable.',
          `Active profile: ${activeProfile.label}.`,
          `Registry health: ${registryHealthStatus}.`,
          `${readyRemotePaths} remote transport(s) ready.`,
        ...
        nextAction: sessionBroker.summary.totalSessions > 0
          ? 'review the most recent session and open web handoff if need to switch context.'
          : registryRecommendedAction === 'create-profile'
            ? 'Create an additional Codex Remote profile before the first cross-account handoff.'
            : webSpawnReady ? 'Create the first Codex Remote session or open a web handoff session.'
              : 'review remote transport and host readiness before handoff.',
      },
    };
  }
}
