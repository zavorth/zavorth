...
function normalizeHttpUrl(value: string | null | undefined, trimTrailingSlash = false): string | null {
  ...
  } catch (error: unknown) {logger.warn('[Integration Connector Mesh] network request failed', error); return null; }
}
...
export class IntegrationConnectorMeshService {
  ...
  private redactExecutionTarget(manifest: IntegrationConnectorManifest, target: string): string {
    ...
    } catch (error: unknown) {logger.warn('[Integration Connector Mesh] process execution failed', error); return ''; }
  }
  ...
  private async readResponse(response: Response): Promise<unknown> {
    ...
    if (contentType.includes('application/json')) {
      ...
      } catch (error: unknown) {logger.warn('[Integration Connector Mesh] operation failed', error); return null; }
    }
    try {
