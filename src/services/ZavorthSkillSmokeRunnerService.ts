...
export class ZavorthSkillSmokeRunnerService {
  ...
  private runSmokeTest(
    ...
    if (smokeTest.destructive) {
      return this.result({
        ...
        status: 'fail',
        mode: 'denial',
        observed: 'destructive smoke test is not allowed in ZavorthControl controls',
      });
    }
  ...
  private result(input: {
    ...
    return {
      id: `zavorth.zavorthControl-controls.skill-smoke.${safeId(input.manifest.id)}.${safeId(input.smokeTest.id)}.${this.now().getTime()}.receipt`,
