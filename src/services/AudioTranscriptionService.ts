...
export class AudioTranscriptionService {
 ...
 // language: preference/env wins over caller unless caller is more specific;
 ...
 // hard timeout budget for whole cascade (not only per provider)
 ...
 /**
 * prefer user VoicePreference; no silent product cascade.
 */
 ...
 String(entry || '')
 .trim()
 .toLowerCase() as AudioTranscriptionProvider,
 )
 ...
 const startedAt = Date.now();
 try {
 const text = await this.withTimeout(this.callProvider(provider, input, readiness.model), perTryTimeout);
 ...
 model: override || config.geminiTranscriptionModel || config.geminiVideoModel || config.geminiModel || null,
 ...
 const text =
 result.analysis?.extractedText || result.analysis?.answer || result.analysis?.description || result.summary;
 if (!result.ok) {
 ...
 ? (
 ((payload.results as Record<string, unknown>).channels as Array<Record<string, unknown>>)?.[0]
