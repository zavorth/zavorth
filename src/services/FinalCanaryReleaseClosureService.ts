...
export class FinalCanaryReleaseClosureService {
  ...
  public buildSnapshot(): FinalCanaryReleaseClosureSnapshot {
    ...
    return {
      ...
      summary: {
        ...
        phase20Linked: items.some((item) => item.id === 'phase-20-approval-ledger-link' && item.status === 'closure-ready'),
        phase21Linked: items.some((item) => item.id === 'phase-21-launch-rehearsal-link' && item.status === 'closure-ready'),
        phase22Linked: items.some((item) => item.id === 'phase-22-monitoring-rollback-link' && item.status === 'closure-ready'),
        phase23Linked: items.some((item) => item.id === 'phase-23-promotion-decision-link' && item.status === 'closure-ready'),
        phaseChainComplete,
  ...
  public formatClosureText(snapshot: FinalCanaryReleaseClosureSnapshot = this.buildSnapshot()): string {
    return [
      ...
      `Phase range: ${snapshot.closure.phaseRange}`,
      ...
      `No further automated phase: ${snapshot.closure.noFurtherAutomatedPhase}`,
      `Sequence closes at Phase 24: ${snapshot.closure.sequenceClosesAtPhase24}`,
      ...
      `Phase chain complete: ${snapshot.summary.phaseChainComplete}`,
  ...
  private items(input: {
    ...
    return [
      ...
      closureItem({
        id: 'phase-20-approval-ledger-link',
        surface: 'phase-chain',
        command: 'dry-run:verify-phase-20-approval-ledger-link --no-network',
        evidence: 'Phase 20 approval ledger is part of the closed canary chain.',
      }),
      closureItem({
        id: 'phase-21-launch-rehearsal-link',
        surface: 'phase-chain',
        command: 'dry-run:verify-phase-21-launch-rehearsal-link --no-network',
        evidence: 'Phase 21 launch rehearsal is part of the closed canary chain.',
      }),
      closureItem({
        id: 'phase-22-monitoring-rollback-link',
        surface: 'phase-chain',
        command: 'dry-run:verify-phase-22-monitoring-rollback-link --no-network',
        evidence: 'Phase 22 monitoring rollback gate is part of the closed canary chain.',
      }),
      closureItem({
        id: 'phase-23-promotion-decision-link',
        surface: 'phase-chain',
        command: 'dry-run:verify-phase-23-promotion-decision-link --no-network',
        evidence: 'Phase 23 promotion decision ledger is part of the closed canary chain.',
      }),
      closureItem({
  ...
  private gates(input: {
    ...
    return [
      gate({
        ...
        nextAction: 'finish Phase 23 before final closure',
      }),
      gate({
      ...
      gate({
        ...
        nextAction: 'restore Phase 20, 21, 22, or 23 closure links',
      }),
      gate({
