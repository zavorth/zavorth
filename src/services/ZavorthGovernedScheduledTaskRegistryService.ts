...
const DEFAULT_INTENT = 'Gerar um resumo operacional recorrente do workspace';
...
function buildChecks(input: {
  ...
  return [
    check(
      ...
      'Use every 15m, every 2h, or daily 09:00.',
    ),
    check(
      'scope-boundary',
      true,
  ...
  return [
    ...
    },
    {
      ...
      summary: verification.ok
        ? `Scope envelope verified until ${envelope?.expiresAt || 'manual revocation'}.`
    ...
    },
    {
      ...
      summary: registration.schedulerServiceCompatible
        ? 'Payload is compatible with the existing SchedulerService adapter.'
...
function detectsCompoundScheduling(scope: ZavorthScheduledTaskScope): boolean {
  const haystack = [
    scope.intent,
    scope.command,
    scope.allowedTools.join(' '),
  ].join('\n').toLowerCase();
  return COMPOUND_TERMS.some((term) => haystack.includes(term));
}
