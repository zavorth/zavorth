...
export class SupervisedExecutionGatewayRecordBuilder {
  ...
  public buildRecord(input: BuildRecordInput): SystemOverlordActionRecord {
    ...
    return {
      ...
      profile: input.decision?.profile || input.request.profile || 'trusted',
      autonomyLevel: input.decision?.autonomyLevel || input.request.autonomyLevel || 2,
      capability: input.request.capability,
      target: input.decision?.runtimeTarget || input.decision?.target || 'host',
      riskLevel: input.decision?.riskLevel || 'medium',
      startedAt: input.createdAt,
      requiresApproval: Boolean(input.decision?.requiresApproval),
      approved: input.request.approved === true,
      auditTrail: input.decision?.auditTrail || [],
      createdAt: input.createdAt,
      updatedAt,
      status: input.status,
      request,
  ...
  public buildMutationRecord(
    ...
    return {
      ...
      metadata: {
        ...this.sanitizeMetadata(action.metadata || undefined),
        ...this.sanitizeMetadata(input.metadata),
        execution_lifecycle: this.appendLifecycleRecords(
          ...
          this.buildActionLifecycleRecord({
            ...
            errorCode: input.errorCode,
            errorMessage: this.sanitizeText(input.reason),
            createdAt: action.createdAt || action.startedAt,
            updatedAt,
          }),
        ),
      },
  ...
  public buildApprovalDecisionRecord(input: {
    ...
    const nextLifecycle = this.buildActionLifecycleRecord({
      ...
      errorCode: input.decision === 'reject' ? 'approval_rejected' : (input.action.errorCode || null),
      errorMessage: input.decision === 'reject' ? this.sanitizeText(reason) : this.sanitizeText(input.action.errorMessage),
      createdAt: input.action.createdAt || input.action.startedAt,
      updatedAt: decidedAt,
    ...
    return {
      ...
      request: this.sanitizeRequest(input.action.request),
      metadata: {
        ...this.sanitizeMetadata(input.action.metadata || undefined),
        execution_lifecycle: this.appendLifecycleRecords(
