...
function parseEndpoint(value: string | null): { ok: true; url: URL } | { ok: false; error: string } {
  ...
  try {
    ...
    return { ok: true, url };
  } catch (error: unknown) {logger.warn('[Remote Mesh Notebook Mcp] network request failed', error);
...
async function readJson(response: Response): Promise<JsonRecord> {
  try {
    ...
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed)
      ...
      : {};
  } catch (error: unknown) {logger.warn('[Remote Mesh Notebook Mcp] parsing failed', error); return {}; }
}
