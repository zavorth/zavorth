import {
  ...
  type ZavorthCommandCenterProviderCockpitAction,
  type ZavorthCommandCenterProviderCockpitCard,
  type ZavorthCommandCenterProviderCockpitProjection,
  type ZavorthCommandCenterProviderCockpitStatus,
} from '../contracts/ZavorthCommandCenterProviderCockpitContract.js';
import type {
...
export type ZavorthCommandCenterProviderCockpitInput = ZavorthProviderReadinessMatrixInput & {
  selectedProviderId?: string | null;
};

export type ZavorthCommandCenterProviderCockpitRuntime = {
  now?: () => Date;
  providerReadiness?: Pick<ZavorthProviderReadinessMatrixService, 'buildLiveSnapshot'>;
};

export class ZavorthCommandCenterProviderCockpitService {
  private readonly now: () => Date;
  private readonly providerReadiness: Pick<ZavorthProviderReadinessMatrixService, 'buildLiveSnapshot'>;

  constructor(runtime: ZavorthCommandCenterProviderCockpitRuntime = {}) {
  ...
  public async buildProjection(
    input: ZavorthCommandCenterProviderCockpitInput = {},
  ): Promise<ZavorthCommandCenterProviderCockpitProjection> {
  ...
  public renderText(projection: ZavorthCommandCenterProviderCockpitProjection): string {
    return [
      '[provider-cockpit]',
...
function buildCard(entry: ZavorthProviderReadinessEntry): ZavorthCommandCenterProviderCockpitCard {
  return {
...
function buildEntryActions(entry: ZavorthProviderReadinessEntry): ZavorthCommandCenterProviderCockpitAction[] {
  const actions: ZavorthCommandCenterProviderCockpitAction[] = [
    {
  ...
  if (entry.status === 'ready' || entry.status === 'needs_probe') {
    actions.push({
      ...
      requiresApproval: false,
      dashboardCanExecute: false,
      summary: 'Operator-triggered live provider probe; Command Center only projects the action.',
    });
  } else {
    actions.push({
...
): ZavorthCommandCenterProviderCockpitAction[] {
...
function buildHealthChecks(matrix: ZavorthProviderReadinessMatrixSnapshot): ZavorthCommandCenterProviderCockpitProjection['healthChecks'] {
  return [
    {
    ...
    {
      id: 'command-center-authority',
      label: 'Command Center authority',
      status: 'ready',
      detail: 'Command Center projection is read-only and cannot execute provider calls.',
    },
  ];
}

function buildReceipts(matrix: ZavorthProviderReadinessMatrixSnapshot): ZavorthCommandCenterProviderCockpitProjection['receipts'] {
  const receipts: ZavorthCommandCenterProviderCockpitProjection['receipts'] = [
    {
...
function resolveProjectionStatus(matrix: ZavorthProviderReadinessMatrixSnapshot): ZavorthCommandCenterProviderCockpitStatus {
...
function buildNextAction(matrix: ZavorthProviderReadinessMatrixSnapshot, selectedProviderId: string): string {
  ...
  if (matrix.summary.livePassed > 0) return 'Provider live evidence is available for Command Center projection.';
