...
import type { ClassicTeamCatalogDto as ZavorthTeamCatalogSnapshot } from './ZavorthControlClassicClientContracts.js';
...
function zavorthControlClassicClientOverviewMeshTeams() {
  function renderOperationsTeams(teamCatalog: ZavorthTeamCatalogSnapshot | TeamCatalogErrorPayload | null | undefined) {
    ...
    const summary: ZavorthTeamCatalogSnapshot['summary'] =
      teamCatalog.summary || ({} as ZavorthTeamCatalogSnapshot['summary']);
      ...
      ? teams
          .map((team) => {
            ...
            return (
              ...
              '<strong>' +
              escapeHtml(team.label || team.id || 'Team') +
              '</strong>' +
              '<span class="badge ' +
              (team.status === 'resumable'
                ? 'badge-warning'
                : team.status === 'active'
                  ? 'badge-info'
                  : 'badge-allowed') +
    ...
    node.innerHTML =
      ...
      '<span class="badge ' +
      (summary.resumable ? 'badge-warning' : 'badge-allowed') +
      '">' +
