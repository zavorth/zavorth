  ...
  return (async () => {
    ...
    if (isLegacyWebSurfacePath(pathname) && !shouldServeLegacyWebSurfaces()) {
      service.responseWriter.writeJson(
        ...
        {
          ok: false,
          error: 'This legacy web surface is internal. Use /zavorthControl.',
          zavorthControlUrl: '/zavorthControl',
          visibleSurfaces: ['/zavorthControl', '/zavorthControl', '/satellite', 'cli'],
        },
        404,
      );
      return;
...
function serveZavorthControlAsset(res: http.ServerResponse, relativePath: string): boolean {
  const root = path.resolve(process.cwd(), 'assets', 'zavorthControl');
