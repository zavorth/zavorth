...
function zavorthControlClassicClientCore() {
  ...
  async function loadMetrics() {
    try {
      ...
      if (stats.error) {
        cnt.innerHTML = `<div class="metric-card" style="grid-column: 1/-1"><strong>Error</strong><div>${stats.error}</div></div>`;
        if (sidecarNode) {
          sidecarNode.innerHTML = '<div class="muted">Could not load sidecars.</div>';
        }
        return;
      }
      ...
      console.warn('[auto-fix] Empty catch block', _e);
    }
  }
