...
export class SharedSurfacePresentationCommandPack {
  ...
  public renderHelp(ctx: Pick<IMessageContext, 'platform' | 'userId' | 'isGroup'>): string {
    ...
    const lines = [
      ...
      '',
      'Natural UX (all commands):',
      '  /command                  → home/status',
      '  /command <plain language> → main action (no need for run/flags)',
      '  /command status|list|help → explicit control still works',
      '  /command … --flag         → power-user optional',
      '',
      '/task <pedido> para abrir uma tarefa comum.',
      '/plan <pedido> para planejar antes de agir.',
      '/auto <pedido> para automacao guiada.',
      '/consensus <pergunta> multi-model com seus modelos.',
      ...
      '/channels consistency [id] para auditar paridade de experiencia por canal: status card, resposta rica, botoes, QR/login e callbacks seguros.',
      ...
      'Natural requests like "show pending permissions" and "approve permission perm-123" also open the permission plane.',
      ...
      'Natural requests like "approve task task-123", "approve the latest pending task", and "reject the discord onboarding task" now use the canonical task approval flow.',
      ...
      '/skills <objetivo natural> (recommend) ou /skills [library|bridge|run <skill>|live <skill> --approval-id <id>|origin <skill>|plan <id>|plan recipe <id>|id|filtro|recipe <id>|recommend <objetivo>|mcp].',
      ...
      '/hub <actionId> (natural), /hub sync, /hub doctor e /hub run <actionId> para a action plane do Hub.',
      '/evals [workspace X|surface Y|executor Z|workflow W] para ver a Eval observability com scorecards, traces e historico.',
      '/qa [alpha|beta] para ver budgets, regressions, smokes e release gates da QA release.',
      '/governance [limit N] para ver tenants, trust decisions, allowlists e policy da Governance.',
      '/replayloop [limit N] para ver replay, artifacts reutilizaveis e learning loop da Replay learning.',
      '/ecosystem [id|filtro] para ver SDKs, guides, publish e receitas publicas da Ecosystem.',
      '/automations [status|maintenance on|maintenance off|maintenance run|pause <id>|resume <id>|remove <id>|<pedido natural>] para operar a Scheduled runs.',
      ...
      '/learning [status|candidates|approve <id>|reject <id>|forget <id>|promote <id>|promote-skill <id>|promote-procedure <id>] para revisar o learning plane.',
      '/perm [pending|approved|rejected|all|show <id>|approve <id>|reject <id>] para operar o permission plane.',
      '/approve <task_id> and /reject <task_id> to decide pending tasks in the canonical flow.',
      ...
      '/codexremote <prompt> (natural) ou /codexremote start [titulo] -- <prompt> para abrir sessao do Codex CLI.',
      ...
      '/sessionsend <sessionId|chatId> <mensagem> (natural) ou com -- <mensagem> para despachar a outra sessao.',
    ...
    lines.push(
      ...
      '/channels consistency [id] para auditar paridade de experiencia por canal sem envio real.',
  ...
  public formatSecurityMeshReply(): string {
    ...
    const lines = [
      ...
      '',
      snapshot.narrative?.trustBoundary || 'Sem trust boundary detalhado right now.',
      '',
      'Trust Plane',
      '',
  ...
  public formatSystemStatusReply(
    ...
    const lines = [
      ...
      `${discordLabel}: ${snapshot.runtime.discordBridge.started ? 'ativo' : 'pending'}${
    ...
    if (lastFailure) {
      lines.push(
        ...
        `Reason: ${String(lastFailure.errorSummary || 'sem resumo').substring(0, 120)}`,
      );
    }
