...
export type DashboardOperationalSnapshotDeps = {
...
const CLASSIC_SESSION_ID = 'classic-dashboard';
const CLASSIC_CHAT_ID = 'dashboard:classic';
const CLASSIC_SOURCE_USER_ID = 'classic-dashboard';

export class DashboardOperationalSnapshotService {
  private readonly lifecycleReadModel = new ExecutionLifecycleReadModelService();

  public readContinuitySnapshot(
    deps: DashboardOperationalSnapshotDeps,
  ): Record<string, any> {
    const continuity = this.readContinuity(deps);
    if (!continuity) {
      return {
        available: false,
        reason: 'Runtime de chat ainda nao anexado ao dashboard classico.',
      };
    }
  ...
  private safeReadRecentTasks(deps: DashboardOperationalSnapshotDeps): any[] {
    try {
  ...
  private safeReadHostActions(deps: DashboardOperationalSnapshotDeps): any[] {
    try {
