...
function dashboardClassicClientOverviewOperationsHost() {
    function renderOperationsHealth(operations) {
      ...
      const dashboardAuth = security.dashboardAuth || {};
      ...
      node.innerHTML =
        ...
        + '<small>Web: ' + escapeHtml((dashboardAuth.source || 'missing')) + ' | Mailbox: ' + escapeHtml((mailboxSecret.source || 'missing')) + ' | DB: ' + escapeHtml((dbEncryption.source || 'missing')) + '</small>'
        + '<small>' + escapeHtml(dashboardAuth.note || 'Sem detalhe de autenticacao.') + '</small>'
...
export function getDashboardClassicClientOverviewOperationsHostScript(): string {
  return extractFunctionBody(dashboardClassicClientOverviewOperationsHost);
}
