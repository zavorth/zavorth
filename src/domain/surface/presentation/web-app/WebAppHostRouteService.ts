...
import type { RuntimeInstallJourneyReport } from '../../../../runtime/access/RuntimeInstallJourneyService.js';
import type { RuntimeBootstrapRepairReport } from '../../../../services/RuntimeBootstrapRepairService.js';
import type { RuntimeStartupResult } from '../../../../services/RuntimeStartupService.js';
import type {
...
type InstallJourneyActionResult = RuntimeInstallJourneyReport | RuntimeBootstrapRepairReport | RuntimeStartupResult;
...
export class WebAppHostRouteService {
  ...
  public async handleRequest(
    ...
    if (pathname === '/api/web/project-constitution/import' && req.method === 'POST') {
      ...
      } catch (error: unknown) {deps.writeJson(res, { ok: false, error: this.extractErrorMessage(error) || 'Failed to preparar import da constituicao.' }, 400);
      }
      return true;
    }
    ...
    if (pathname === '/api/web/disk-mutation-gate' && req.method === 'POST') {
      ...
      try {
        ...
        const preview = this.diskMutationGate.createPreview({
          ...
          requestedBy: String(body.requestedBy || deps.runtime.webUserId || 'zavorthControl').trim() || 'zavorthControl',
          sourceSurface: String(body.sourceSurface || 'zavorthControl').trim() || 'zavorthControl',
          reason: String(body.reason || '').trim() || null,
        });
        deps.writeJson(res, { ok: true, preview }, preview.status === 'blocked' ? 409 : 200);
      } catch (error: unknown) {deps.writeJson(res, { ok: false, error: this.extractErrorMessage(error) || 'Failed to preparar gate de mutation em disco.' }, 400);
      }
      return true;
    }
    ...
    if (gitWorkflowMatch && req.method === 'POST') {
      ...
      } catch (error: unknown) {deps.writeJson(res, { ok: false, error: this.extractErrorMessage(error) || 'Failed to run Git workflow.' }, 400);
      }
      return true;
    }
    ...
    if (pathname === '/api/web/review' && req.method === 'POST') {
      ...
      try {
        ...
        const snapshot = await this.agentReview.run({
          ...
          approvalId: String(body.approvalId || '').trim() || null,
          userId: String(body.userId || deps.runtime.webUserId || 'zavorthControl').trim() || 'zavorthControl',
    ...
    if ((pathname === '/api/web/zavorth-runtime-adapters/register' || pathname === '/api/web/external-agents/register') && req.method === 'POST') {
      ...
      try {
        const receipt = this.externalAgentGateway.registerProfile({
          ...
          requestedBy: this.cleanOptionalString(body.requestedBy) || deps.runtime.webUserId || 'zavorthControl',
    ...
    if (pathname === '/api/web/ops/keepalive' && req.method === 'GET') {
      ...
      try {
        ...
        deps.writeJson(
          ...
          {
            ok: false,
            reason: 'failure ao ler snapshot',
  ...
  private normalizeExternalAgentAcpTransport(value: unknown): 'local-jsonrpc' | 'stdio-jsonrpc' | 'acp-sdk-stdio' | null {
    const normalized = String(value || '').trim().toLowerCase();
    if (normalized === 'local-jsonrpc' || normalized === 'stdio-jsonrpc' || normalized === 'acp-sdk-stdio') {
      return normalized;
    }
    return null;
  }
  ...
  private resolveLocalWorkspace(value: unknown): string {
    ...
    if (workspaceRoot !== projectRoot && !this.isInsidePath(allowedRoot, workspaceRoot)) {
      throw new Error('Workspace is outside the approved local scope.');
    }
  ...
  private async runInstallJourneyAction(
    ...
    dryRun: boolean,
    body: InstallJourneyActionBody,
    deps: WebAppRuntimeRouteDeps,
  ): Promise<InstallJourneyActionResult> {
    switch (action) {
      case 'go':
      case 'ready': {
      ...
      default:
        throw new Error('Action de jornada local unavailable.');
    }
  }
  ...
  private async inspectReadiness(
    ...
    deps: WebAppRuntimeRouteDeps,
  ): Promise<RuntimeAccessReadinessReport> {
