...
export async function handleDashboardRoutes(
  ...
  helpers: WebAppRuntimeStateRouteHelpers,
      ...
      if (pathname === '/api/web/dashboard' && req.method === 'GET') {
        ...
        const contractAdapter = await service.buildDashboardContractAdapterProjection(url, deps);
        ...
        const dashboardSnapshot = service.attachDashboardContractAdapter(enrichedSnapshot, contractAdapter);
        deps.writeJson(
          res,
          {
            ok: true,
            ...
            modelProfile: service.buildCurrentModelProfile(dashboardSnapshot),
          },
          200,
        );
        return true;
      ...
      if ((pathname === '/api/web/dashboard/actions' || pathname === '/api/web/zavorthControl/actions') && req.method === 'POST') {
        await service.handleDashboardActionRequest(req, res, deps);
        return true;
      }
  
      if ((pathname === '/api/web/dashboard/chat-v1' || pathname === '/api/web/zavorthControl/chat-v1') && req.method === 'POST') {
        await service.handleDashboardChatRequest(req, res, deps);
        return true;
      }
  
      if (pathname === '/api/web/chat/side' && req.method === 'POST') {
        await service.handleDashboardSideChatRequest(req, res, deps);
        return true;
      }
  
      if (pathname === '/api/web/chat/steer' && req.method === 'POST') {
        await service.handleDashboardSteerChatRequest(req, res, deps);
        return true;
      }
