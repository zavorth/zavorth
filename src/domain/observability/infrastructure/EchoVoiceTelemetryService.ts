...
export class EchoVoiceTelemetryService {
  ...
  private async safeRecord(
    ...
    } catch (error: unknown) {// Observability must never break the calling surface.
  ...
  private readEvents(): VoiceTelemetryEvent[] {
    try {
      ...
      return raw
        ...
        .map((line) => {
          ...
          } catch (error: unknown) {logger.warn('[Voice Telemetry] JSON parse failed', error); return null; }
        })
    ...
    } catch (error: unknown) {logger.warn('[Voice Telemetry] JSON parse failed', error); return []; }
  }
