 ...
 *   - cliToolInitialConfig: JSON snapshot of pre-ZavorthGateway configuration
 *
...
function parseJsonValue(raw: string): unknown {
  ...
  } catch (error: unknown) {logger.warn('[cli  State] JSON parse failed', error); return null; }
}
...
/**
 * Save the initial (pre-ZavorthGateway) config snapshot for a CLI tool.
