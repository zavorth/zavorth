...
export class ZavorthBridgeService {
  ...
  /**
   * Build ZavorthBridge authorization URL
   */
  ...
  /**
   * Get common headers for ZavorthBridge API calls
   */
  ...
  async loadCodeAssist(accessToken: string) {
    ...
    // Extract tier ID, falling back to Google's Code Assist default tier marker.
  ...
  /**
   * Fetch Project ID from loadCodeAssist API for older callers.
   */
  ...
  /**
   * Complete ZavorthBridge OAuth flow
   */
  async connect() {
    const spinner = createSpinner("Starting ZavorthBridge OAuth...").start();
    ...
    try {
      ...
      // Wait for callback
      spinner.start("Waiting for ZavorthBridge authorization...");
