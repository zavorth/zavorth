...
function getLHCState() {
  if (!globalThis.__ZavorthGatewayLocalHC) {
    globalThis.__ZavorthGatewayLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__ZavorthGatewayLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return isEnvFlagEnabled("ZavorthGateway_DISABLE_LOCAL_HEALTHCHECK") || process.env.NODE_ENV === "test";
}
...
function isLocalhostUrl(baseUrl: string): boolean {
  ...
  } catch (error: unknown) {logger.warn('[local  Check] network request failed', error); return false; }
}
...
export async function sweep(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const err = asErrorLike(error);
      logger.error(LOG_PREFIX, "Failed to load provider_nodes:", err);
      return;
    }
...
export function initLocalHealthCheck(): void {
  ...
  state.sweepTimer = setTimeout(() => {
    sweep().catch((err) => logger.error(LOG_PREFIX, "Initial sweep failed:", err));
  }, INITIAL_DELAY_MS);
}
