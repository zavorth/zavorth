# ZavorthGateway A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use ZavorthGateway as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes ZavorthGateway as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
...
└──────────────────────────────────────────────────────────────────┘
                       │
                       ▼  ZavorthGateway Gateway (internal)
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through ZavorthGateway intelligent pipeline",
...
Routes prompts through ZavorthGateway's intelligent pipeline with full observability.
...
Discovers ZavorthGateway agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use ZavorthGateway A2A as a custom LLM.
"""
...
class ZavorthGatewayA2A(BaseLLM):
...
An orchestrator agent delegates code generation to ZavorthGateway, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via ZavorthGateway A2A
    ...
    # Step 2: Review the code via ZavorthGateway A2A (different model)
...
Multiple agents share quota through ZavorthGateway, using the quota skill to coordinate.
...
Part of [ZavorthGateway](https://github.com/zavorth-core/ZavorthGateway) — MIT License.
