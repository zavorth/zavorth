# AI Gateway Native Hygiene Freeze

Status: active

This subtree is under an active Zavorth native hygiene freeze.
...
- Keep convergence with the new agent loop out of this subtree until the planned Track 5 preparation work.

## Operational Inventory

This inventory is intentionally high-signal for Track 0 and should be refined as later waves land.

| Path family | State | Direction |
| --- | --- | --- |
| `src/ai-gateway/app/**` | `adapted/forked` | Preserve behavior; rewrite visible surfaces by domain in Tracks 1-2. |
| `src/ai-gateway/shared/**` | `forked/adapted` | Quarantine presentation helpers and rework them into Zavorth-native shared UI. |
| `src/ai-gateway/lib/db/**` | `forked/adapted` | Keep storage working now; replace schema and backup canon in Track 3. |
| `src/ai-gateway/lib/oauth/**` | `adapted` | Preserve flows, then rebuild under a Zavorth auth plane in Track 3. |
| `src/ai-gateway/mitm/**` | `forked` | Keep proxy utility, but move canon to Zavorth naming and transport contracts. |
| `src/ai-gateway/sse/**` | `forked/adapted` | Treat as high-risk inherited surface and replace in Channel mesh. |
| `src/ai-gateway/instrumentation-node.ts` | `adapted` | Preserve background bootstrapping while aligning imports and naming later. |
| `src/ai-gateway/proxy.ts` | `adapted` | Keep operational, revisit after transport native hygiene work. |
...
- expand scope beyond the gateway native hygiene track.
...
Any new compatibility file must be added here before it is treated as intentional. If it is not listed here, it is not part of the Track 0 freeze boundary.
...
The gate is implemented in `scripts/zavorth-native-boundary-check.mjs` and enforces:
