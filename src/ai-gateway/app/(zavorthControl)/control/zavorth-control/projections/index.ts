...
export function scanZavorthControlSnapshotForSourceIdentityLeaks(
  ...
  const cleanTerms = terms.map((term) => text(term)).filter(Boolean);
  ...
  const visit = (node: unknown, path: string) => {
    ...
    if (node && typeof node === 'object') {
      Object.entries(node as AnyRecord).forEach(([key, entry]) => visit(entry, `${path}.${key}`));
    }
  };
...
export function buildZavorthControlAssimilationSnapshot(input: {
  ...
  const snapshot: AnyRecord = {
    contractVersion: ZAVORTH_ZAVORTH_CONTROL_ASSIMILATION_VERSION,
