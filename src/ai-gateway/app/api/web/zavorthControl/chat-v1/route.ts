...
function inferOperation(message: string, explicit: unknown): "chat" | "read" | "summarize" | "code-question" | "write" | "delete" | "shell" | "network" | "deploy" | "transaction" {
  if (
  ...
  if (/\b(read|summari[sz]e|resuma|resume|summary)\b/i.test(message)) return "summarize";
