...
function IconSettings() {
  return (
    <svg viewBox="0 0 24 24">
      ...
      <path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09a1.65 1.65 0 00-1-1.51 1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.6 15a1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09a1.65 1.65 0 001.51-1 1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06A1.65 1.65 0 009 4.6a1.65 1.65 0 001-1.51V3a2 2 0 012-2 2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06A1.65 1.65 0 0019.4 9c.14.31.22.65.22 1H21a2 2 0 012 2 2 2 0 01-2 2h-1.38c0 .35-.08.69-.22 1z" />
    </svg>
  );
}
...
function ConsoleSelect({ prompt, label, value }: ConsoleSelectProps) {
  return (
    <button className="chat-console-select" type="button" data-prompt={prompt}>
      <span>{label}</span>
      <strong>{value}</strong>
    </button>
  );
...
function SuggestionChip({ prompt, children, mission, autoSubmit }: SuggestionChipProps) {
  return (
    <button
  ...
  return (
    <section className="sector active" id="sector-terminal">
      <div className="terminal-split">
        <div className="control-workspace-shell">
          <div className="terminal-main">
            <div className="terminal-view is-empty" id="terminal-view">
            <div className="chat-console-bar" aria-label="Chat controls">
              ...
              <div className="chat-console-bar__actions">
                <button
                  className="chat-console-icon"
                  type="button"
                  data-prompt="Refresh readiness and summarize only what needs attention."
                ...
                <button
                  className="chat-console-icon"
                  type="button"
                  data-prompt="Show pending approvals with clear accept or reject actions."
                ...
                <button
                  className="chat-console-icon"
                  type="button"
                  data-prompt="Open memory status, Mnemos scopes and recent file understanding results."
                  aria-label="Memory"
                >
                  <IconMemory />
                </button>
            ...
            <div className="neural-stream" id="neural-stream">
              <div className="neural-stream__inner" id="neural-feed" />
            </div>
            <div className="compose-dock">
              <div className="suggestion-chips" id="suggestion-chips">
                <SuggestionChip
                  mission="organize-day"
                  autoSubmit
                  prompt="Organize my day safely. Start read-only and tell me what needs approval."
                >
                  <IconCalendar />
                ...
                <SuggestionChip
                  ...
                  autoSubmit
                  prompt="Review this workspace safely. Read first, list risks, and do not change files."
                >
                  <IconSearch />
                  <span>Review workspace</span>
                </SuggestionChip>
                <SuggestionChip prompt="Show what Zavorth can remember right now and which scopes are active.">
                  <IconMemory />
                  <span>Check memory</span>
                </SuggestionChip>
                <SuggestionChip prompt="Summarize a document or PDF safely. Start with a preview and receipt.">
                  <IconDocument />
                  <span>Summarize document</span>
                </SuggestionChip>
              </div>
              <div className="compose-dock__input-frame">
                <div className="compose-settings-panel hidden" id="compose-settings-panel" role="dialog" aria-label="Chat settings">
                  <label className="compose-settings-field">
                    <span>Voice</span>
                    <select id="compose-setting-voice" data-composer-setting="voice" defaultValue="default">
                  ...
                  <label className="compose-settings-field">
                    ...
                    <select id="compose-setting-sensitivity" data-composer-setting="sensitivity" defaultValue="default">
                      <option value="default">Default</option>
                      <option value="low">Low</option>
                      <option value="high">High</option>
                    </select>
                  </label>
                  <details className="compose-settings-advanced">
                    <summary>Advanced</summary>
                    <div className="compose-settings-toggles">
                      <button className="compose-settings-toggle" type="button" data-composer-toggle="thinking" aria-pressed="false">
                        Reasoning
                      </button>
                      <button className="compose-settings-toggle" type="button" data-composer-toggle="tools" aria-pressed="true">
                        Tools
                      </button>
                      <button className="compose-settings-toggle" type="button" data-composer-toggle="focus" aria-pressed="false">
                        Focus
                      </button>
                    </div>
                  </details>
                </div>
                <textarea id="compose-input" placeholder="Ask Zavorth" rows={1} />
                <div className="compose-dock__controls">
                  <div className="compose-dock__controls-left">
                    <button
                      className="compose-dock__action"
                      id="attach-file-trigger"
                      title="Attach file"
                      aria-label="Attach file"
                      data-tooltip="Attach files for this message"
                    >
                      <IconPaperclip />
                      <span>Attach file</span>
                      <small className="compose-dock__badge" id="attachment-count-badge" hidden>
                        0
                      </small>
                    </button>
                    <button
                      ...
                      data-tooltip="Dictate the request"
                    >
                      <IconMic />
                      <span>Start voice</span>
                      <small className="compose-dock__badge" id="voice-state-badge" hidden>
                        on
                      </small>
                    </button>
                    <button
                      className="compose-dock__btn"
                      id="tool-sheet-trigger"
                      title="Tools"
                      aria-label="Tools"
                      aria-haspopup="dialog"
                      aria-expanded="false"
                      data-tooltip="Choose tool input"
                    >
                      <IconPlus />
                      <small className="compose-dock__badge" id="tool-count-badge" hidden>
                        0
                      </small>
                    </button>
                    <button
                      className="compose-dock__btn"
                      id="trace-sheet-trigger"
                      title="History"
                      aria-label="History"
                      aria-haspopup="dialog"
                      aria-expanded="false"
                      data-tooltip="Open trace and receipts"
                    >
                      <IconTrace />
                      <small className="compose-dock__badge" id="history-count-badge" hidden>
                        0
                      </small>
                    </button>
                    <button
                      className="compose-dock__btn"
                      id="compose-settings-trigger"
                      title="Settings"
                      aria-label="Settings"
                      aria-haspopup="dialog"
                      aria-expanded="false"
                      data-tooltip="Message settings"
                    >
                      <IconSettings />
                    </button>
                  </div>
                    ...
                    <button
                      className="compose-dock__action"
                      id="export-chat-trigger"
                      title="Export"
                      aria-label="Export chat"
                      data-tooltip="Export this conversation"
                    >
                      <IconDownload />
                      <span>Export</span>
                    </button>
        ...
        <div className="artifact-pane hidden" id="artifact-pane">
          <div className="artifact-pane__header">
            <span className="artifact-pane__title" id="artifact-title">
              Artifact
            </span>
            <button className="artifact-pane__close" id="artifact-close">
              <IconClose />
            </button>
          </div>
          <div className="artifact-pane__body" id="artifact-body" />
        </div>
      </div>
    </section>
  );
