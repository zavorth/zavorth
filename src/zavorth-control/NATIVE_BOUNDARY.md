...
| --- | --- | --- |
| `src/zavorth-control/app/**` | Dashboard and local web app surfaces. | Zavorth dashboard |
| `src/zavorth-control/shared/**` | Shared UI components and presentation helpers. | Zavorth UI system |
| `src/zavorth-control/lib/db/**` | Local gateway persistence and backup adapters. | Zavorth storage plane |
| `src/zavorth-control/lib/oauth/**` | Local auth profile resolution. | Zavorth auth plane |
| `src/zavorth-control/mitm/**` | Local proxy support. | Zavorth proxy plane |
| `src/zavorth-control/sse/**` | Realtime event transport. | Zavorth transport plane |
| `src/zavorth-control/instrumentation-node.ts` | Runtime boot instrumentation. | Zavorth gateway runtime |
| `src/zavorth-control/proxy.ts` | Request proxy entrypoint. | Zavorth gateway runtime |
| `src/zavorth-control/server-init.ts` | Server initialization. | Zavorth gateway runtime |
...
| --- | --- | --- |
| `src/zavorth-control/lib/db/storagePlane.ts` | Migration/table compatibility fallback only. | Zavorth storage plane |
| `src/zavorth-control/lib/db/jsonBackupAdapters.ts` | Import/export compatibility fallback only. | Zavorth settings backup contract |
| `src/zavorth-control/lib/oauth/authPlane.ts` | OAuth environment alias fallback only. | Zavorth auth plane |
| `src/zavorth-control/mitm/proxyPlane.cjs` | Proxy environment alias fallback only. | Zavorth proxy plane |
| `src/zavorth-control/sse/transportPlane.ts` | Transport/header fallback only. | Zavorth transport plane |
| `src/zavorth-control/sse/compat/openSseCompat.ts` | SSE package bridge only. | Zavorth SSE transport |
