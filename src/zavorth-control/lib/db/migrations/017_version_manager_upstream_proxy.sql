...
CREATE TABLE IF NOT EXISTS version_manager (
  ...
  tool              TEXT NOT NULL UNIQUE,           -- 'cliproxyapi' plus any legacy tool ids retained for compatibility
  ...
  auto_start        INTEGER NOT NULL DEFAULT 0,    -- 1 = start with the configured upstream proxy tool
...
-- Determines whether each provider uses the Zavorth-native executor,
-- delegates to CLIProxyAPI, or uses fallback (native first, then CLIProxyAPI).
-- --------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS upstream_proxy_config (
  id                        INTEGER PRIMARY KEY AUTOINCREMENT,
  provider_id               TEXT NOT NULL UNIQUE,          -- e.g., 'zavorthBridge', 'claude', 'codex'
