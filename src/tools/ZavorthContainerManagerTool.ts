...
export class ZavorthContainerManagerTool extends BaseTool {
  ...
  private async dockerCmd(cmdArgs: string[]): Promise<string> {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Container Manager] process execution failed', error); return ''; }
  }
  ...
  private async inspect(args: Record<string, unknown>): Promise<string> {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Container Manager] JSON parse failed', error); return ''; }
  }
