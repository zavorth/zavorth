...
export class ZavorthCodeIntelligenceTool extends BaseTool {
  ...
  private async formatCode(filePath: string, args: Record<string, unknown>): Promise<string> {
    ...
    } catch (error: unknown) { logger.warn('[Zavorth Code Intelligence] process execution failed', error); return ''; }
  }
  ...
  private listFiles(dir: string, extensions?: string[]): string[] {
    ...
    } catch (error: unknown) { /* ignore */ logger.warn('[Zavorth Code Intelligence] operation failed', error); }
    return results;
  }
}
