...
export class ZavorthDependencyAnalyzerTool extends BaseTool {
  ...
  private async checkOutdated(args: Record<string, unknown>): Promise<string> {
    ...
    } catch (error: any) { logger.warn('[Zavorth Dependency Analyzer] operation failed', error); return ''; }
  }
    ...
    try {
      switch (resolved) {
        case 'npm': {
          ...
          } catch (error: any) { logger.warn('[Zavorth Dependency Analyzer] parsing failed', error); return ''; }
        }
        case 'yarn':
  ...
  private async checkLicenses(args: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      } catch (error: any) { logger.warn('[Zavorth Dependency Analyzer] JSON parse failed', error); return ''; }
    }
  }
