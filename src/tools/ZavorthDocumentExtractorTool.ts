...
export class ZavorthDocumentExtractorTool extends BaseTool {
  ...
  private async extractPdf(
    ...
    try {
      ...
      if (pages.length === 0) {
        ...
        if (isContiguous) {
          ...
          for (const page of pages) {
            ...
            } catch (error: any) { /* skip individual page errors */ logger.warn('[Zavorth Document Extractor] process execution failed', error); }
          }
          text = parts.join('\n\n');
        }
      }
      ...
      } catch (error: any) { /* ignore */ logger.warn('[Zavorth Document Extractor] process execution failed', error); }
    ...
    try {
      ...
      for (const page of pages) {
        ...
        try {
          ...
          try { fs.unlinkSync(pagePath); } catch (error: any) { /* ignore */ logger.warn('[Zavorth Document Extractor] file cleanup failed', error); }
        }
      }
    ...
    return { ...base, error: `OCR failed: ${this.sanitizePath(error instanceof Error ? error.message : String(error))}. Install tesseract-ocr.` };
      ...
      try {
        ...
        for (const f of remaining) {
          try { fs.unlinkSync(path.join(tmpDir, f)); } catch (error: any) { /* ignore */ logger.warn('[Zavorth Document Extractor] file cleanup failed', error); }
        }
      } catch (error: any) { /* ignore */ logger.warn('[Zavorth Document Extractor] file cleanup failed', error); }
    }
  }
...
print('\\n'.join(texts))`;
      ...
      try {
        ...
        try { fs.unlinkSync(tmpScript); } catch (error: any) { /* ignore */ logger.warn('[Zavorth Document Extractor] file cleanup failed', error); }
      }
    } catch (error: any) {
      try {
