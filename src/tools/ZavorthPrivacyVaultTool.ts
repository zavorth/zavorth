...
export class ZavorthPrivacyVaultTool extends BaseTool {
  ...
  private loadVault(): VaultEntry[] {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Privacy Vault] JSON parse failed', error); return []; }
  }
  ...
  private logAudit(action: string, entryId: string, details: string): void {
    ...
    if (fs.existsSync(this.auditPath)) {
      try { auditLog = JSON.parse(fs.readFileSync(this.auditPath, 'utf-8')); } catch (error: unknown) {/* ignore */ logger.warn('[Zavorth Privacy Vault] JSON parse failed', error); }
    }
  ...
  private store(args: Record<string, unknown>): string {
    ...
    let tags: string[] = [];
    if (typeof args.tags === 'string') { try { tags = JSON.parse(args.tags); } catch (error: unknown) {/* ignore */ logger.warn('[Zavorth Privacy Vault] JSON parse failed', error); } }
