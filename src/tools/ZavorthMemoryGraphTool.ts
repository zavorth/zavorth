...
export class ZavorthMemoryGraphTool extends BaseTool {
  ...
  private loadGraph(): void {
    ...
    try { if (fs.existsSync(nodesPath)) { const d = JSON.parse(fs.readFileSync(nodesPath, 'utf-8')); this.nodes = new Map(Object.entries(d)); } } catch { /* ignore */ }
    try { if (fs.existsSync(edgesPath)) this.edges = JSON.parse(fs.readFileSync(edgesPath, 'utf-8')); } catch { /* ignore */ }
  }
