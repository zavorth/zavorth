...
export class ZavorthNetworkDiagnosticsTool extends BaseTool {
  ...
  private async runCmd(cmd: string, cmdArgs: string[], timeout = 30000): Promise<string> {
    try {
      ...
      return result.trim();
    } catch (error: unknown) {logger.warn('[Zavorth Network Diagnostics] process execution failed', error); return ''; }
  }
  ...
  private async portScan(args: Record<string, unknown>): Promise<string> {
    ...
    } catch (error: unknown) {logger.warn('[Zavorth Network Diagnostics] network request failed', error); return ''; }
  }
