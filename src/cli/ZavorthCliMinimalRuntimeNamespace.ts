...
export type ZavorthCliMinimalRuntimeNamespaceInput = {
  command: string;
  restArgs: string[];
  projectRoot: string;
  logCliError: (message: string, title?: string) => Promise<void>;
  printCliPanel: (
    title: string,
    lines: string[],
    type?: 'default' | 'info' | 'success' | 'warning' | 'error',
  ) => Promise<number>;
  readDurationMsFlag: (argv: string[], name: string) => number | null;
  readNumberFlag: (argv: string[], name: string) => number | null;
  readStringFlag: (argv: string[], name: string) => string | null;
};

export async function runZavorthMinimalRuntimeNamespace(
  input: ZavorthCliMinimalRuntimeNamespaceInput,
): Promise<number | null> {
  const {
    command,
    restArgs,
