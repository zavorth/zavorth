...
export async function runProductSubsystemCli(rawArgs: string[] = []): Promise<number> {
  ...
  if (!rawArgs.length || sub === 'status' || sub === 'inventory') {
    ...
    console.log('');
    console.log('Tip: zavorth product certify  # hermetic subsystem matrix');
  ...
  if (sub === 'certify' || sub === 'certification' || sub === 'qa') {
    ...
    console.log('');
    console.log(`Subsystems: capability=${snap.fabrics.capability} reach=${snap.fabrics.reach} power=${snap.fabrics.power} product=${snap.fabrics.product}`);
    console.log(`Next: ${snap.narrative.nextSafeAction}`);
    return snap.certification.blocked > 0 ? 1 : 0;
  }
...
export { runProductSubsystemCli as runProductFabricCli };
