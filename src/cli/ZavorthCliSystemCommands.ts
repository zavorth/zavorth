...
export async function runOperationalSecurityDoctor(rawArgs: string[]): Promise<number> {
  const { buildOperationalSecurityDoctorReport, formatOperationalSecurityDoctorReport } = await import('../security/OperationalSecurityDoctor.js');
...
export async function runDiagnosticsExport(rawArgs: string[]): Promise<number> {
  ...
  try {
    ...
    if (rawArgs.includes('--json')) {
      ...
      await printCliPanel(
        'Diagnostics export',
        [`Status: exported`, `Output path: ${outputPath}`, `Logs gathered: ${report.logs.length}`, `Exported at: ${report.exportedAt}`, `Sanitization: all secrets, local paths and sensitive keys have been redacted.`],
        'success',
      );
    }
    return 0;
...
export async function runZavorthHomeCommand(rawArgs: string[]): Promise<number> {
  ...
  const subcommand = String(rawArgs[0] || 'status')
    .trim()
    .toLowerCase();
  ...
  if (subcommand === 'switch') {
    lines.push(
      '',
      rawArgs.includes('--apply') ? `switched: ${switchResult?.written ? `ZAVORTH_HOME written to ${switchResult.envFile}` : 'no .env update was needed'}` : 'switch preview only. Add --apply to write ZAVORTH_HOME into .env.',
  ...
  if (subcommand === 'migrate') {
    lines.push(
      '',
      'migration preview:',
      ...snapshot.migration.entries.map((entry) => `- ${entry.kind.padEnd(9)} ${entry.exists ? 'found' : 'missing'} ${entry.redactedSource} -> ${entry.redactedDestination} risk=${entry.risk}`),
      '',
      snapshot.migration.writesPerformed ? `${snapshot.migration.status} with approval ${snapshot.migration.approvalId}` : 'no data was written without --apply --approval-id=<id>',
    );
  }
...
export async function runZavorthEchoWakeCommand(rawArgs: string[]): Promise<number> {
  ...
  if (subcommand === 'setup' || subcommand === 'configure') {
    ...
    const snapshot = setup.buildPlan({
      choice: rawArgs.includes('--disabled') ? 'disabled' : rawArgs.includes('--custom-command') ? 'custom-command' : 'default-local',
  ...
  const session = subcommand === 'arm' ? service.arm(ttlMs) : subcommand === 'disarm' ? service.disarm() : service.status();
...
export async function runZavorthTasksCommand(rawArgs: string[]): Promise<number> {
  ...
  const { ZavorthHomePathService } = await import('../services/ZavorthHomePathService.js');
  const subcommand = String(rawArgs[0] || 'list')
    .trim()
    .toLowerCase();
  const explicitHome = readFlexibleStringFlag(rawArgs, 'home');
  const home = new ZavorthHomePathService({ projectRoot, explicitHome, env: process.env }).resolveSnapshot();
  ...
  if (subcommand === 'create' || subcommand === 'add') {
    ...
    result = service.claimTask(readTaskPositional(rawArgs, 1) || '', readFlexibleStringFlag(rawArgs, 'owner') || process.env.USERNAME || process.env.USER || 'operator', readDurationMsFlag(rawArgs, 'lease'));
...
export async function runZavorthFriendlyWorkCommand(command: 'todo' | 'later' | 'work' | 'done' | 'retry' | 'cancel', rawArgs: string[]): Promise<number> {
...
export async function runPremiumHatch(rawArgs: string[]): Promise<number> {
  if (rawArgs.includes('--start')) {
    return runPromotedScript(
      'ops-go',
      rawArgs.filter((arg) => arg !== '--start'),
    );
  }
...
export function resolveDailyHudArgs(rawArgs: string[]): string[] {
  ...
  const hasApprovalAction =
    ...
    first === 'review' ||
    first === 'guide';
  const explicitRuntime = first === 'runtime' || first === 'tui' || rawArgs.includes('--runtime') || rawArgs.includes('--tui');
...
export async function runGitWorkflowCommand(action: 'status' | 'branch' | 'commit' | 'pr', rawArgs: string[]): Promise<number> {
  ...
  const approvalId = readFlexibleStringFlag(rawArgs, 'approval-id') || readFlexibleStringFlag(rawArgs, 'approval') || (rawArgs.includes('--yes') ? 'cli-local-owner' : null);
  const args = rawArgs.filter((arg) => arg !== '--json' && arg !== '--apply' && arg !== '--yes').join(' ');
  ...
  if (asJson) {
    ...
    await printCliPanel(
      ...
      [
        ...
        snapshot.plannedCommands.length ? `plan: ${snapshot.plannedCommands.map((entry) => `${entry.command} ${entry.args.join(' ')}`).join(' && ')}` : null,
        snapshot.approval.required ? `approval: ${snapshot.approval.satisfied ? snapshot.approval.approvalId : 'required for --apply'}` : null,
        snapshot.receipt ? `receipt: ${snapshot.receipt.receiptId}` : null,
      ].filter((line): line is string => Boolean(line)),
      snapshot.status === 'applied' || snapshot.status === 'ready' ? 'success' : snapshot.status === 'blocked' || snapshot.status === 'failed' ? 'error' : 'warning',
    );
  }
...
export async function runContinuousSecurityMonitor(rawArgs: string[]): Promise<number> {
  const { buildContinuousSecurityMonitorReport, formatContinuousSecurityMonitorReport, writeContinuousSecurityBaseline } = await import('../security/ContinuousSecurityMonitor.js');
  const strict = rawArgs.includes('--strict') || rawArgs.includes('--require-pass');
  ...
  if (updateBaseline) {
    ...
    if (rawArgs.includes('--json')) {
      ...
      await printCliPanel('Security baseline', ['Baseline updated.', `updated at: ${baseline.updatedAt}`], 'success');
    }
    return 0;
  }
  ...
  if (!rawArgs.includes('--apply') && !rawArgs.includes('apply')) {
    if (asJson) {
      ...
      await printCliPanel(
        ...
        [`${preset.id}: ${preset.label}`, `profile: ${preset.securityProfile}`, `MCP: ${preset.mcpPolicy.profile}`, `skills: ${preset.skillPolicy.defaultPolicy}`, preset.summary, '', `Apply: zavorth security preset ${preset.id} --apply`],
        'warning',
      );
    }
    return 0;
...
export async function runAiFirstOwnerControlledDefault(rawArgs: string[]): Promise<number> {
  ...
  const action = String(rawArgs[0] || 'status')
    .trim()
    .toLowerCase();
  if (action === 'prepare') {
...
export async function runPromotedScript(scriptBaseName: 'setup-v3' | 'ops-go', forwardedArgs: string[] = []): Promise<number> {
