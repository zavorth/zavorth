...
const COMMANDS: ZavorthCliCertificationCommand[] = [
  ...
  { name: 'background', summary: 'Create and inspect separated background agent work.', usage: 'zavorth background [objective|status]', description: 'Creates visible Task Plane work for long-running agent tasks without starting unsafe execution immediately.', status: 'ready', commands: [['status', 'Show background tasks.'], ['<objective>', 'Create a queued background task.']], examples: [['zavorth background "audit this workspace"', 'Queue separated background work.'], ['zavorth background status', 'Show visible status.']] },
  { name: 'board', summary: 'Alias for TaskBoard work planning.', usage: 'zavorth board [status|triage|decompose]', description: 'Simple Kanban-like projection over Task Plane for daily work.', status: 'ready', commands: [['status', 'Show board lanes.'], ['triage', 'Create one board card.'], ['decompose', 'Split an objective into planner/worker/verifier cards.']], examples: [['zavorth board decompose "ship release"', 'Create board tasks.']] },
  { name: 'agent', summary: 'Run one governed agent turn.', usage: 'zavorth agent [options]', description: 'Run a single LLM-first turn through the local harness.', status: 'ready', options: ['--json           Output JSON when supported'], commands: [['run', 'Run a task.']], examples: [['zavorth agent run "summarize this repo"', 'Run one governed turn.']] },
  { name: 'agents', summary: 'Manage isolated Zavorth agent workspaces and routing.', usage: 'zavorth agents [command]', description: 'Inspect internal agent profiles, workspaces and routing boundaries.', status: 'ready', commands: [['status', 'Inspect local runtime state.']], examples: [['zavorth agents status', 'Inspect local agent state.']] },
  ...
  { name: 'control', summary: 'Open the visual ZavorthControl.', usage: 'zavorth control', description: 'Open the local visual control plane.', status: 'ready', examples: [['zavorth control', 'Open ZavorthControl.']] },
  ...
  { name: 'go', summary: 'Create a persistent goal for the current work.', usage: 'zavorth go <objective>', description: 'Registers an objective in the Goal Plane and backs it with a Task Plane item.', status: 'ready', commands: [['<objective>', 'Create a goal.']], examples: [['zavorth go "finish release readiness"', 'Create a tracked goal.']] },
  { name: 'goals', summary: 'Inspect and manage persistent goals.', usage: 'zavorth goals [status|pause|resume|done|tick|loop|worker]', description: 'Goal Plane status, explicit state transitions and safe Goal Loop continuation for longer work.', status: 'ready', commands: [['status', 'Show goals.'], ['pause', 'Pause a goal.'], ['resume', 'Resume a goal.'], ['done', 'Complete a goal.'], ['tick', 'Record progress.'], ['loop', 'Judge a goal and queue a continuation task without executing it silently.'], ['worker', 'Claim a queued continuation, run an AgentRun step, then re-judge the goal.']], examples: [['zavorth goals status', 'Show active goals.'], ['zavorth goals loop <goal-id> --summary "tests still running"', 'Queue a governed continuation step.'], ['zavorth goals worker --max-items 1', 'Process one queued continuation through AgentRun.']] },
  ...
  { name: 'mnemos', summary: 'Recall, index and manage local memory.', usage: 'zavorth mnemos [recall|session-recall|forget|correct|promote]', description: 'Mnemos memory commands, including session recall for prior conversation context.', status: 'ready', commands: [['recall', 'Search memory.'], ['session-recall', 'Recall previous sessions.'], ['session-append', 'Append a local session event.'], ['forget', 'Forget through native contract.']], examples: [['zavorth mnemos session-recall "provider search"', 'Find previous session context.']] },
  ...
  { name: 'state', summary: 'Inspect the unified operational SQLite state.', usage: 'zavorth state [status]', description: 'Shows the local StateDB backing sessions, events, receipts, tasks, goals and boards.', status: 'ready', commands: [['status', 'Show StateDB path, journal mode, FTS and counts.']], examples: [['zavorth state status', 'Inspect the operational StateDB.']] },
  ...
  { name: 'setup', summary: 'Initialize local config and workspace.', usage: 'zavorth setup [provider|channels|tools|agent] [options]', description: 'First Light for provider, channels, tools, workspace and trust.', status: 'ready', options: ['--setup-mode safe           Conservative guided defaults', '--setup-mode blank-slate    Minimal opt-in setup', '--config-handling reset     Back up .env and remove setup-managed keys', '--dry-run                   Preview setup without writing files', '--json                      Output JSON when supported'], commands: [['provider', 'Provider, model and credential setup.'], ['channels', 'Communication surfaces and channel credentials.'], ['tools', 'Web/search, skills and automation templates.'], ['agent', 'Memory, trust, runtime and first hatch controls.']], examples: [['zavorth setup', 'Open First Light guided flow.'], ['zavorth setup provider', 'Start at provider setup.'], ['zavorth setup channels', 'Start at channel setup.'], ['zavorth setup tools', 'Start at tools setup.'], ['zavorth setup agent', 'Start at agent setup.'], ['zavorth setup --setup-mode safe', 'Use conservative guided defaults.'], ['zavorth setup --setup-mode blank-slate', 'Start minimal and opt in later.'], ['zavorth setup --config-handling reset', 'Back up .env and remove setup-managed keys.'], ['zavorth setup --dry-run', 'Preview setup without writing files.'], ['zavorth onboard --dry-run', 'Alias for setup preview.'], ['zavorth ready', 'Check whether setup is ready for daily use.'], ['zavorth start', 'Start or resume the local runtime.'], ['zavorth open', 'Open the visual zavorthControl.']] },
  ...
  { name: 'taskboard', summary: 'Plan work as simple Task Plane-backed board cards.', usage: 'zavorth taskboard [status|triage|decompose|note]', description: 'Daily Kanban-style board over Task Plane with planner, worker and verifier cards.', status: 'ready', commands: [['status', 'Show lanes.'], ['triage', 'Create a card.'], ['decompose', 'Split an objective.'], ['note', 'Add a blackboard note.']], examples: [['zavorth taskboard triage "review providers"', 'Create one board card.'], ['zavorth taskboard decompose "improve channels"', 'Create planner/worker/verifier cards.']] },
  ...
  { name: 'xai', summary: 'Diagnose and run xAI-native provider search.', usage: 'zavorth xai [doctor|search]', description: 'xAI provider route with credential doctor and operator-gated native search.', status: 'ready', commands: [['doctor', 'Show readiness.'], ['search', 'Preview or run native search.']], examples: [['zavorth xai doctor', 'Show xAI readiness.'], ['zavorth xai search "release news"', 'Preview native search.']] },
];
...
const IMPLEMENTED_COMMANDS = new Set([
  ...
  'certify', 'completions', 'configure', 'control', 'doctor', 'gateway', 'migrate', 'models',
  'onboard', 'onboarding', 'sandbox', 'security', 'setup', 'skills', 'status', 'terminal', 'tui', 'update',
  'background', 'backup', 'board', 'commitments', 'config', 'cron', 'daemon', 'devices', 'directory', 'dns',
  'docs', 'exec-policy', 'go', 'goals', 'health', 'hooks', 'infer', 'logs', 'mcp', 'message', 'mnemos', 'node',
  'nodes', 'pairing', 'plugins', 'proxy', 'qr', 'reset', 'secrets', 'sessions', 'state', 'system',
  'taskboard', 'tasks', 'uninstall', 'webhooks', 'xai',
]);
