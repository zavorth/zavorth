...
const COMMANDS: ZavorthCliParityCommand[] = [
  ...
  { name: 'backup', summary: 'Create and verify local state backups.', usage: 'zavorth backup [command]', description: 'Prepare local backup and verification flows for Zavorth state, receipts and config.', status: 'prepared', commands: [['create', 'Create a local backup archive.'], ['verify', 'Verify a backup archive.'], ['list', 'List known backups.']], examples: [['zavorth backup create', 'Create a governed backup when configured.']] },
  { name: 'capability', summary: 'Run capability and provider capability commands.', usage: 'zavorth capability [command]', description: 'Inspect provider, tool, media, search and embedding capabilities.', status: 'ready', commands: [['catalog', 'Show capability catalog.'], ['doctor', 'Diagnose capability readiness.'], ['infer', 'Route to provider-backed capability commands.']], examples: [['zavorth capability catalog', 'Inspect available capabilities.']] },
  ...
  { name: 'commitments', summary: 'List and manage inferred follow-up commitments.', usage: 'zavorth commitments [command]', description: 'Inspect follow-ups and operator commitments inferred from runs and receipts.', status: 'prepared', commands: [['list', 'List commitments.'], ['resolve', 'Resolve a commitment.']], examples: [['zavorth commitments list', 'Show pending follow-ups.']] },
  ...
  { name: 'infer', summary: 'Run provider-backed model, media and search commands.', usage: 'zavorth infer [command]', description: 'Route model, image, audio, video, search and embedding capabilities.', status: 'prepared', commands: [['text', 'Run text generation.'], ['image', 'Run image generation when configured.'], ['audio', 'Run audio capability when configured.'], ['video', 'Run video capability when configured.'], ['search', 'Run search capability.']], examples: [['zavorth infer search "latest TypeScript release"', 'Run governed search when configured.']] },
  ...
  { name: 'setup', summary: 'Initialize local config and workspace.', usage: 'zavorth setup [options]', description: 'Setup Studio for provider, channels, Mnemos, workspace and trust.', status: 'ready', options: ['--dry-run        Preview setup without writing files', '--json           Output JSON when supported'], commands: [['legacy', 'Use the legacy setup path.'], ['conversation', 'Calibrate profile and tone.']], examples: [['zavorth setup', 'Open Setup Studio.'], ['zavorth setup --dry-run', 'Preview setup without writing files.'], ['zavorth onboard --dry-run', 'Alias for setup preview.'], ['zavorth ready', 'Check whether setup is ready for daily use.'], ['zavorth start', 'Start or resume the local runtime.'], ['zavorth open', 'Open the visual dashboard.']] },
...
export function formatZavorthParityPreparedNotice(name: string, args: string[] = []): string | null {
  ...
  return premiumBox(`Zavorth ${command.name}`, [
    ...
    'configuration, credentials, an adapter proof receipt, or a dedicated backend before it',
    'can run live.',
    '',
    'Next:',
