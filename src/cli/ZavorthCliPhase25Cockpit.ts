...
type CliPhase25DoctorSnapshot = Awaited<ReturnType<typeof buildCliOperationsDoctorSnapshot>>;

type CliPhase25MemorySummary = {
  artifacts: number;
  replayTasks: number;
...
export type CliPhase25CockpitSnapshot = OperationsCockpitSnapshot & {
  phase: '25';
  surface: 'zavorth-cockpit';
  unified: {
    headline: string;
    posture: 'healthy' | 'attention' | 'degraded';
    sourceHealth: CliPhase25SourceHealth;
    cards: CliPhase25CockpitCard[];
    nextActions: CliPhase25UnifiedAction[];
    memory: CliPhase25MemorySummary | null;
    doctorError: string | null;
  };
  statusSnapshot: CliStatusSnapshot | null;
  briefSnapshot: OperatorBriefSnapshot | null;
  doctorSnapshot: CliPhase25DoctorSnapshot | null;
};
...
function resolvePosture(cockpit: OperationsCockpitSnapshot): CliPhase25CockpitSnapshot['unified']['posture'] {
...
function summarizeMemory(memorySnapshot: any): CliPhase25MemorySummary | null {
...
function pushAction(
  actions: CliPhase25UnifiedAction[],
  candidate: Partial<CliPhase25UnifiedAction> | null | undefined,
): void {
...
function buildUnifiedActions(params: {
  ...
  doctor: CliPhase25DoctorSnapshot | null;
  memory: CliPhase25MemorySummary | null;
  actionDefinitions: Array<{ id: string; label: string; command: string; args?: string[]; priority?: 'high' | 'normal' }>;
}): CliPhase25UnifiedAction[] {
  const actions: CliPhase25UnifiedAction[] = [];
...
export async function buildCliPhase25CockpitSnapshot(
  runtime: ZavorthCliRuntime,
  flags: ZavorthCliFlags,
): Promise<CliPhase25CockpitSnapshot | null> {
  ...
  let doctor: CliPhase25DoctorSnapshot | null = null;
  ...
  let memory: CliPhase25MemorySummary | null = null;
  ...
  const sourceHealth: CliPhase25SourceHealth = {
...
export function formatCliPhase25CockpitSnapshot(snapshot: CliPhase25CockpitSnapshot): string {
...
export type {
  CliPhase25CockpitCard,
  CliPhase25MemorySummary,
  CliPhase25SourceHealth,
  CliPhase25UnifiedAction,
};
