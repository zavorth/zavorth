...
function renderManagedConfigPlan(plan: ZavorthManagedConfigPlan): string {
  const statusTone = plan.status === 'applied'
    ? 'success'
    : plan.status === 'blocked'
      ? 'danger'
  ...
  const panels: CliVisualPanel[] = [
    {
      ...
      tone: statusTone,
      lines: [
        `status: ${paintCliTone(plan.status, statusTone)}`,
    ...
    {
      title: 'Findings',
      tone: plan.findings.length ? 'warning' : 'success',
    ...
    },
    {
      title: 'Next actions',
      tone: 'info',
      lines: plan.nextActions.map((action) => `${paintCliTone('>', 'brand')} ${action}`),
    },
  ];
