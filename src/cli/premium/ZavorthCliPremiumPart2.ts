...
import { runRuntimeGuidedFixes, runRuntimeReadinessFix } from './ZavorthCliPremiumPart3.js';
...
export async function runPromotedScript(
...
export async function runProductizationProtectedRuntime(
...
export async function runAgentManager(subCommand: string, rawArgs: string[] = []): Promise<number> {
  const scriptMap: Record<string, string> = {
    add: 'scripts/zavorth-agent-add.ts',
    list: 'scripts/zavorth-agent-list.ts',
    run: 'scripts/zavorth-agent-run.ts',
    chain: 'scripts/zavorth-agent-chain.ts',
    discover: 'scripts/zavorth-agent-add.ts',
  };
  const script = scriptMap[subCommand] || scriptMap.add;
  return npmInherited(['exec', 'tsx', '--', script, ...rawArgs], projectRoot);
}

export async function runCapabilities(rawArgs: string[] = []): Promise<number> {
  return npmInherited(['exec', 'tsx', '--', 'scripts/zavorth-capabilities.ts', ...rawArgs], projectRoot);
}
