...
function isSkillAllowlisted(root: string, id: string): boolean {
  ...
  } catch (error: unknown) {logger.warn('[Zavorth Cli Live Namespaces] JSON parse failed', error); return false; }
}
...
function redactUrl(value: string): string {
  try {
    ...
    return url.toString();
  } catch (error: unknown) {logger.warn('[Zavorth Cli Live Namespaces] search failed', error);
    return redact(value);
  }
}
