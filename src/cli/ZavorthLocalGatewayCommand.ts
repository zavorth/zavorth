...
import { SlackGateway } from '../gateways/channels/slack/SlackGateway.js';
import { WhatsAppGateway } from '../gateways/channels/whatsapp/WhatsAppGateway.js';
import { TeamsGateway } from '../gateways/channels/teams/TeamsGateway.js';
import { IMessageGateway } from '../gateways/channels/imessage/IMessageGateway.js';
import { SignalGateway } from '../gateways/channels/signal/SignalGateway.js';
import { EmailGateway } from '../gateways/channels/email/EmailGateway.js';
import { InstagramGateway } from '../gateways/channels/instagram/InstagramGateway.js';
import { DiscordGateway } from '../gateways/channels/discord/DiscordGateway.js';
...
export async function runZavorthLocalGatewayCommand(rawArgs: string[]): Promise<number> {
  const args = rawArgs.filter(Boolean);
  const channel = (readStringFlag(args, 'channel') || 'slack').toLowerCase();
  const userId = readStringFlag(args, 'userId') || 'local-user';
  const chatId = readStringFlag(args, 'chatId') || 'local-chat';
