...
export type SelfingDashboardSectionId =
  | 'identity'
  | 'tone'
  | 'user'
  | 'environment'
...
export type SelfingDashboardCard = {
  id: string;
  section: SelfingDashboardSectionId;
  title: string;
  value: string;
  source: string;
...
export type SelfingDashboardSuggestion = {
  id: string;
  section: SelfingDashboardSectionId;
  title: string;
  detail: string;
...
export type SelfingDashboardReceipt = {
  id: string;
  kind:
    | 'identity-file'
    | 'workspace-profile'
...
export type SelfingDashboardSnapshot = {
  contractVersion: typeof SELFING_DASHBOARD_CONTRACT_VERSION;
  ...
  cards: SelfingDashboardCard[];
  suggestions: SelfingDashboardSuggestion[];
  receipts: SelfingDashboardReceipt[];
  policy: {
...
export type SelfingDashboardInput = {
...
import {
  ...
  uniqueCards,
  type LooseRecord,
} from './SelfingDashboardHelpers.js';

export class SelfingDashboardService {
  ...
  public buildSnapshot(input: SelfingDashboardInput): SelfingDashboardSnapshot {
    const { run } = input;
  ...
  private buildIdentity(
    ...
    canonicalContext: LooseRecord | null,
    workspaceProfile: LooseRecord,
  ): SelfingDashboardSnapshot['identity'] {
  ...
  private buildIdentityFileCards(identityFiles: LooseRecord[]): SelfingDashboardCard[] {
  ...
  private buildProfileCards(
    identity: SelfingDashboardSnapshot['identity'],
    workspaceProfile: LooseRecord,
  ): SelfingDashboardCard[] {
    const profilePath = normalizeText(workspaceProfile.firstRunProfilePath) || null;
    const entries: Array<[SelfingDashboardSectionId, string, string | null, boolean]> = [
  ...
  private buildEnvironmentCards(
    ...
    const cards: SelfingDashboardCard[] = [];
  ...
  private buildSuggestions(input: {
    ...
    const suggestions: SelfingDashboardSuggestion[] = [];
    if (input.identityFileCount === 0) {
      suggestions.push({
        ...
        reason: 'Selfing Dashboard fica mais confiavel quando a identidade vem de fontes editaveis e versionadas.',
        sensitive: true,
  ...
  private buildReceipts(input: {
    ...
    const receipts: SelfingDashboardReceipt[] = [];
