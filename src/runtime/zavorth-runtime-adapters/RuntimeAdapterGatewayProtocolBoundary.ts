...
export type RuntimeAdapterGatewayProtocolFrameKind =
  | 'request'
  | 'event'
  | 'response'
  | 'error'
...
export type RuntimeAdapterGatewayProtocolSourceEvidence = {
...
export type RuntimeAdapterGatewayProtocolFrame = {
  fixtureCase?: string;
  frameKind: RuntimeAdapterGatewayProtocolFrameKind;
  id?: string;
  runtimeId?: string;
  sessionId?: string;
  operation?: string;
  ...
  sourceEvidence?: RuntimeAdapterGatewayProtocolSourceEvidence;
};

export type RuntimeAdapterGatewayProtocolStructuredError = {
...
export type RuntimeAdapterGatewayProtocolNormalizationResult =
  | {
      ok: true;
      nativeContract: 'RuntimeAdapterEventEnvelope' | 'NormalizedInboundMessage';
      envelope: RuntimeAdapterEventEnvelope;
  ...
  | {
      ok: false;
      error: RuntimeAdapterGatewayProtocolStructuredError;
    };

export type RuntimeAdapterGatewayProtocolBoundaryOptions = {
  runtimeId: string;
  observedAt: string;
  session?: RuntimeAdapterSessionDescriptor | null;
...
function sourcePathsFor(frame: RuntimeAdapterGatewayProtocolFrame): string[] {
  return frame.sourceEvidence?.sourcePaths || [];
}

function frameOperation(frame: RuntimeAdapterGatewayProtocolFrame): string {
...
function structuredGatewayFrameError(
  frame: RuntimeAdapterGatewayProtocolFrame,
  code: RuntimeAdapterGatewayProtocolStructuredError['code'],
  message: string,
): RuntimeAdapterGatewayProtocolNormalizationResult {
  return {
    ok: false,
    error: {
      nativeContract: 'ZavorthStructuredGatewayError/v1',
...
function buildEnvelope(
  frame: RuntimeAdapterGatewayProtocolFrame,
  options: RuntimeAdapterGatewayProtocolBoundaryOptions,
...
function normalizeEnvelopeToInboundMessage(
  ...
  return {
    ...
    requestedTools,
    metadata: {
      source: 'runtime-adapter-gateway-protocol-boundary',
...
export function normalizeRuntimeAdapterGatewayProtocolFrame(
  frame: RuntimeAdapterGatewayProtocolFrame,
  options: RuntimeAdapterGatewayProtocolBoundaryOptions,
): RuntimeAdapterGatewayProtocolNormalizationResult {
  ...
  return {
    ok: true,
    nativeContract: diagnosticOnly ? 'RuntimeAdapterEventEnvelope' : 'NormalizedInboundMessage',
    envelope,
