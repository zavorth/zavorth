...
export type RuntimeAdapterPluginServiceSurfaceKind =
  | 'background'
  | 'hook';

export type RuntimeAdapterPluginServiceSurfaceFixtureCase =
  | 'service-surface-descriptor'
  | 'service-launch-blocked';

export type RuntimeAdapterPluginServiceSurfaceExecutionGate = {
...
export type RuntimeAdapterPluginServiceSurfaceSourceRecord = {
  fixtureCase: RuntimeAdapterPluginServiceSurfaceFixtureCase;
  publicServiceIdSeed: string;
  sourceHooks: string[];
  sourceLifecycleHints: string[];
  kind: RuntimeAdapterPluginServiceSurfaceKind;
  risk: UniversalToolRiskLevel;
  requestedTools: string[];
};

export type RuntimeAdapterZavorthServiceSurface = {
  id: string;
  label: string;
  kind: RuntimeAdapterPluginServiceSurfaceKind;
  hooks: Array<{
...
export type RuntimeAdapterServiceCatalogRow = {
  id: string;
  serviceId: string;
  label: string;
...
export type RuntimeAdapterPluginServiceSurfaceBoundaryOptions<TRuntimeId extends string = string> = {
  records: RuntimeAdapterPluginServiceSurfaceSourceRecord[];
  generatedAt: string;
  runtimeId: TRuntimeId;
  idPrefix: string;
  executionGate: RuntimeAdapterPluginServiceSurfaceExecutionGate;
};

export type RuntimeAdapterPluginServiceSurfaceBoundaryNormalization<TRuntimeId extends string = string> = {
  nativeContract: 'ZavorthPluginServiceSurfaceConsistency/v1';
  generatedAt: string;
  runtimeId: TRuntimeId;
  services: RuntimeAdapterZavorthServiceSurface[];
  serviceRegistry: {
    serviceRows: RuntimeAdapterServiceCatalogRow[];
  };
...
function serviceLabel(index: number, fixtureCase: RuntimeAdapterPluginServiceSurfaceFixtureCase): string {
...
function serviceStatus(risk: UniversalToolRiskLevel): RuntimeAdapterServiceCatalogRow['status'] {
  return risk === 'danger' ? 'blocked' : 'available';
}

export function normalizeRuntimeAdapterPluginServiceSurfaces<TRuntimeId extends string>(
  options: RuntimeAdapterPluginServiceSurfaceBoundaryOptions<TRuntimeId>,
): RuntimeAdapterPluginServiceSurfaceBoundaryNormalization<TRuntimeId> {
  const services = options.records.map((record, index): RuntimeAdapterZavorthServiceSurface => ({
