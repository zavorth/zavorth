...
export type ExternalExecutorReadOnlyProbeExecutionGate = ExternalAgentSidecarReadOnlyExecutionGate & {
...
export type ExternalExecutorReadOnlyProbeNormalization<TRuntimeId extends string = string> = {
  ...
  observability: Pick<ZavorthSidecarObservabilityProjection, 'commandCenter' | 'nativeContract'> & {
...
function probeHealthStatus(results: ExternalExecutorReadOnlyProbeCommandResult[]): ExternalAgentHealthSnapshot['status'] {
  if (results.length === 0) {
    return 'offline';
  }
