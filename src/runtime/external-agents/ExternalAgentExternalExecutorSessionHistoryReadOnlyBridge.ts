...
export type ExternalExecutorSessionHistoryZavorthControlView = {
  nativeContract: 'ZavorthExternalSessionZavorthControlView/v1';
  id: string;
  sessionViewId: string;
  label: string;
  status: ZavorthExternalSessionViewStatus;
...
export type ExternalExecutorSessionHistoryReadOnlyBridgeNormalization = {
  ...
  zavorthControlViews: ExternalExecutorSessionHistoryZavorthControlView[];
  ...
  nextGateRecommended: 'future-read-only-session-schema-fingerprint-or-zavorthControl-session-panel';
};
...
function buildZavorthControlView(view: ZavorthExternalSessionView): ExternalExecutorSessionHistoryZavorthControlView {
  return {
    nativeContract: 'ZavorthExternalSessionZavorthControlView/v1',
    id: `${view.id}:zavorthControl`,
...
export function normalizeExternalExecutorSessionHistoryReadOnlyBridge<TRuntimeId extends string>(
  ...
  return {
    ...
    readOnly: true,
    sessionViews: views,
    zavorthControlViews: views.map(buildZavorthControlView),
