...
export type ExternalAgentCommandHttpPolicyPreflightDecision =
  | 'allowed'
  | 'approval_required'
  | 'blocked';

export type ExternalAgentCommandHttpPolicyPreflightFixtureCase =
  | 'policy-preflight-approval-required'
  | 'policy-preflight-blocked-invocation'
  | 'policy-preflight-safe-metadata';

export type ExternalAgentCommandHttpPolicyPreflightExecutionGate = {
...
export type ExternalAgentCommandHttpPolicyPreflightSourceRecord = {
  fixtureCase: ExternalAgentCommandHttpPolicyPreflightFixtureCase;
...
export type ExternalAgentCommandHttpPolicyPreflightResult = {
  id: string;
  label: string;
  invocationEnvelopeId: string;
  intentKind: ExternalAgentCommandHttpInvocationIntentKind;
...
export type ExternalAgentCommandHttpPolicyPreflightBoundaryOptions<TRuntimeId extends string = string> = {
  records: ExternalAgentCommandHttpPolicyPreflightSourceRecord[];
  generatedAt: string;
  runtimeId: TRuntimeId;
  idPrefix: string;
  executionGate: ExternalAgentCommandHttpPolicyPreflightExecutionGate;
};

export type ExternalAgentCommandHttpPolicyPreflightBoundaryNormalization<TRuntimeId extends string = string> = {
  nativeContract: 'ZavorthCommandHttpPolicyPreflightBoundary/v1';
  generatedAt: string;
  runtimeId: TRuntimeId;
  preflights: ExternalAgentCommandHttpPolicyPreflightResult[];
...
function preflightLabel(index: number, decision: ExternalAgentCommandHttpPolicyPreflightDecision): string {
...
function decidePreflight(record: ExternalAgentCommandHttpPolicyPreflightSourceRecord): ExternalAgentCommandHttpPolicyPreflightDecision {
  if (
...
export function normalizeExternalAgentCommandHttpPolicyPreflight<TRuntimeId extends string>(
  options: ExternalAgentCommandHttpPolicyPreflightBoundaryOptions<TRuntimeId>,
): ExternalAgentCommandHttpPolicyPreflightBoundaryNormalization<TRuntimeId> {
  const preflights = options.records.map((record, index): ExternalAgentCommandHttpPolicyPreflightResult => {
