...
export type ExternalExecutorLiveReadOnlyBridgeSurface = {
  ...
  label: string;
  availability: ExternalAgentLiveReadinessCapabilityInventoryRow['availability'];
  classification: ExternalAgentLiveReadinessCapabilityInventoryRow['importClassification'];
...
export type ExternalExecutorLiveReadOnlyBridgeObservability = {
  ...
  commandCenterProjectionRows: number;
...
function buildObservability(
  ...
  return {
    ...
    commandCenterProjectionRows: snapshot.commandCenterProjection.rows.length,
    healthProbeAuthenticated: true,
    statusRpcOk: true,
    probeOk: true,
