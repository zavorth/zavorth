...
**The dependable autonomous AI agent runtime built for real work.** Zavorth connects the same governed agent across **Desktop**, **Web Control Dashboard**, **Interactive Terminal (CLI/TUI)**, and **Messaging Gateways**. It introduces human approvals for sensitive actions, runs tasks in protected sandboxes, and keeps verified receipts for every completed run.
...
<tr><td><b>🖥️ Multi-Surface Sync</b></td><td>Unified agent truth across Desktop, Web Control dashboard, Terminal CLI/TUI, and remote messaging gateways.</td></tr>
...
graph TD
    ...
    subgraph Step3["3. Proof & Sync"]
        Execute --> Receipt["📜 Generates verifiable execution receipt"]
        Receipt --> Surfaces["📱 Syncs across Desktop, Web, CLI & Channels"]
    end
```
...
| **Unified Surfaces** | Fragmented CLI scripts | **Synchronized Desktop, Web Control & Channels** |
...
| Surface | Best For | Description |
|---|---|---|
| **🖥️ Desktop App** | Daily Workflow | Native visual workspace with file navigation, real-time diffs, live run approvals, and local receipts. |
| **🌐 Zavorth Control** | Operations Hub | Browser dashboard (`/control`) for node telemetry, provider status, session history, and diagnostics. |
| **⌨️ CLI & TUI** | Keyboard-First | High-speed terminal interface with multi-line editing, command completion, and automated scripting. |
| **💬 Messaging Gateways** | Remote Dispatch | Governed remote interaction via Telegram bot (and WhatsApp adapter) with voice memo transcription. |
