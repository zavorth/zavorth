...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    zavorth_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    zavorth_WORKSPACE_ID: config.id,
    zavorth_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.zavorth_EXPERIMENTAL_WORKSPACES) return
