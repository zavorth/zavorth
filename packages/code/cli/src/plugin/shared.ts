...
// Plugin package names that should be skipped (formerly built-in auth plugins).
// Empty: no deprecated external plugin package names for this project.
...
export async function checkPluginCompatibility(target: string, zavorthVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(zavorthVersion) || semver.major(zavorthVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.zavorth
  if (typeof range !== "string") return
  if (!semver.satisfies(zavorthVersion, range)) {
    throw new Error(`Plugin requires zavorth ${range} but running ${zavorthVersion}`)
  }
}
