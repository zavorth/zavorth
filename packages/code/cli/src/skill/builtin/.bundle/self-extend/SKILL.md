...
You can extend your own capabilities by writing files to `.zavorth/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.

## Creating Tools

Write to `.zavorth/tools/<name>.ts`:

```ts
import { tool } from "@zavorth/plugin"
...
Write to `.zavorth/hooks/<name>.ts` — export a Hooks object:
...
```ts
// .zavorth/tools/bash.ts — overrides built-in bash
import { tool } from "@zavorth/plugin"
...
Write to `.zavorth/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.zavorth/tools/*.ts` | next turn |
| Hooks | `.zavorth/hooks/*.ts` | next turn |
| Skills | `.zavorth/skills/*/SKILL.md` | next turn |
| TUI | `.zavorth/tui/*.tsx` | restart |
