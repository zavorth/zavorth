...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".zavorth-project-id")
  const idFile = nodePath.join(mainGit, "zavorth-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .zavorth-project-id
  ...
  if (!existing.includes(".zavorth-project-id")) {
...
export class Service extends Context.Service<Service, Interface>()("@zavorth/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.zavorth_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening zavorth in $HOME.
      ...
      if (Flag.zavorth_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
