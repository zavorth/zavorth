...
declare const ZAVORTH_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const binding = require(
      `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${ZAVORTH_LIBC || "glibc"}` : ""}`,
    )
...
export class Service extends Context.Service<Service, Interface>()("@zavorth/FileWatcher") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("FileWatcher.state")(
        function* () {
          if (yield* Flag.zavorth_EXPERIMENTAL_DISABLE_FILEWATCHER) return
          ...
          if (yield* Flag.zavorth_EXPERIMENTAL_FILEWATCHER) {
