...
function create(opts: { cors?: string[] }) {
  ...
  if (Flag.zavorth_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.zavorth_WORKSPACE_ID ? WorkspaceID.make(Flag.zavorth_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
...
export async function listen(opts: {
  ...
  if (!isLoopback && !Flag.zavorth_SERVER_PASSWORD && !opts.noAuth) {
    throw new Error(
      "Refusing to bind to non-loopback address without zavorth_SERVER_PASSWORD. " +
