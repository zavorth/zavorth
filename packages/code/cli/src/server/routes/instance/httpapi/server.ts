...
const Headers = Schema.Struct({
  ...
  "x-zavorth-directory": Schema.optional(Schema.String),
})
...
class Authorization extends HttpApiMiddleware.Service<Authorization>()("@zavorth/ExperimentalHttpApiAuthorization", {
  error: Unauthorized,
  security: {
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.zavorth_SERVER_PASSWORD) return yield* effect

        const user = Flag.zavorth_SERVER_USERNAME ?? "zavorth"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.zavorth_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-zavorth-directory"] || process.cwd()
