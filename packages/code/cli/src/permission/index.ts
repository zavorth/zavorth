...
import { evaluatePermissionViaGateway, mergeWithGatewayPolicy } from "@/util/gateway-policy"
...
export class Service extends Context.Service<Service, Interface>()("@zavorth/Permission") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: AskInput, abortSignal?: AbortSignal) {
      ...
      const { ruleset: localRuleset, ...request } = input
      // 1) File-based product policy (config/runtime-permissions.json) merged last
      const ruleset = mergeWithGatewayPolicy(localRuleset, process.env)

      // 2) Live gateway evaluate when product-hosted (same product profile over HTTP)
      const remote = yield* Effect.promise(() =>
        evaluatePermissionViaGateway({
          permission: request.permission,
          patterns: [...request.patterns],
          env: process.env,
        }),
      )
      if (remote.ok && remote.action === "deny") {
        log.info("gateway-policy-deny", {
          permission: request.permission,
      ...
      let needsAsk = false
      // Remote allow for the whole request: skip human ask when gateway says allow
      // (local deny still wins via ruleset loop below).
      const remoteAllows = remote.ok && remote.action === "allow"
