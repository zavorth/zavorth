...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall zavorth and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall zavorth")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @zavorth/cli",
      pnpm: "pnpm uninstall -g @zavorth/cli",
      bun: "bun remove -g @zavorth/cli",
      // TODO(zavorth): uncomment when published to these channels
      // brew: "brew uninstall zavorth",
      // choco: "choco uninstall zavorth",
      // scoop: "scoop uninstall zavorth",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using zavorth!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# zavorth") || content.includes(".zavorth/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".zavorth/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".zavorth/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".zavorth"))
    ) {
      continue
    }
