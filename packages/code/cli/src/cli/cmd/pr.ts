...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run zavorth",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for zavorth session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found zavorth session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["zavorth", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting zavorth...")
        UI.println()

        const zavorthArgs = sessionId ? ["-s", sessionId] : []
        const zavorthProcess = Process.spawn(["zavorth", ...zavorthArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await zavorthProcess.exited
        if (code !== 0) throw new Error(`zavorth exited with code ${code}`)
      },
    })
  },
})
