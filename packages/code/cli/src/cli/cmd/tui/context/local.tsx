  ...
  init: () => {
    ...
    const model = iife(() => {
      ...
      const fallbackModel = createMemo(() => {
        ...
        // No args/config/recent match: prefer the free zavorth-auto channel so a
        // clean install defaults to a usable free model rather than whatever
        // provider happens to sit first (e.g. paid Zavorth cloud / ultraspeed).
        const zavorth = sync.data.provider.find((p) => p.id === "zavorth")
        if (zavorth && "zavorth-auto" in zavorth.models) {
          return { providerID: "zavorth", modelID: "zavorth-auto" }
        }
      ...
      return {
        ...
        parsed: createMemo(() => {
          ...
          return {
            ...
            model:
              value.modelID === "zavorth-auto"
                ? t("tui.model.zavorth_auto.name")
