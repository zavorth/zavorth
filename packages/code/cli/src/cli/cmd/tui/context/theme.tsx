...
import zavorth from "./theme/zavorth.json" with { type: "json" }
import mono from "./theme/mono.json" with { type: "json" }
...
type ThemeColor = Exclude<keyof TuiThemeCurrent, "thinkingOpacity" | "thinking">
...
export type ThemeJson = {
  ...
  theme: Omit<Record<ThemeColor, ColorValue>, "selectedListItemText" | "backgroundMenu"> & {
    ...
    /** Soft sage for reasoning UI; falls back to textMuted+primary mix when omitted. */
    thinking?: ColorValue
...
export const DEFAULT_THEMES: Record<string, ThemeJson> = {
  ...
  zavorth,
  /** Alias — same Event Horizon green palette as zavorth. */
  ["event-horizon"]: zavorth,
  /** Light skin: grayscale with muted green accents (not a full skin engine). */
  mono,
  orng,
...
export function resolveTheme(theme: ThemeJson, mode: "dark" | "light") {
  ...
  ) as Partial<Record<ThemeColor, RGBA>> & { thinking?: RGBA }
  ...
  // Soft sage thinking color — never amber/warning. Mix muted text with primary when missing.
  if (theme.theme.thinking !== undefined) {
    resolved.thinking = resolveColor(theme.theme.thinking)
  } else if (resolved.textMuted && resolved.primary) {
    const a = resolved.textMuted
    const b = resolved.primary
    resolved.thinking = RGBA.fromValues(
      a.r * 0.7 + b.r * 0.3,
      a.g * 0.7 + b.g * 0.3,
      a.b * 0.7 + b.b * 0.3,
      1,
    ...
    resolved.thinking = resolved.textMuted
  }
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "zavorth")
        draft.active = typeof active === "string" ? active : "zavorth"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.zavorth, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const brandOrange = RGBA.fromInts(255, 103, 0)
...
function getSyntaxRules(theme: Theme) {
  return [
    ...
    },
    {
      scope: ["extmark.file"],
      style: {
        // Soft brand accent — not OpenCode yellow
    ...
    },
    {
      scope: ["extmark.paste"],
      style: {
        // Quiet paste chip: muted text on panel, no yellow slab
