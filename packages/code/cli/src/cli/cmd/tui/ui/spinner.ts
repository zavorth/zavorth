...
export type KnightRiderStyle = "blocks" | "diamonds" | "plane" | "horizon"
...
export function createFrames(options: KnightRiderOptions = {}): string[] {
  ...
  const frames = Array.from({ length: totalFrames }, (_, frameIndex) => {
    ...
    return Array.from({ length: width }, (_, charIndex) => {
      if (style === "plane") {
        // Legacy style kept for plugins; prefer "horizon" for Zavorth-native UX.
      ...
      // Zavorth horizon: soft scanning bar (half-blocks), no mascot-unrelated glyphs.
      if (style === "horizon") {
        const dd = state.isMovingForward ? state.activePosition - charIndex : charIndex - state.activePosition
        if (dd === 0) return "█"
        if (dd === 1 || dd === -1) return "▓"
        if (dd === 2 || dd === -2) return "▒"
        if (dd > 0 && dd < trailOptions.colors.length) return "░"
        return "·"
      }
