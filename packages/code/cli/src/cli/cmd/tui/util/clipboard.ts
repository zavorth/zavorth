...
import { stripProtection } from "./copy-safe"
...
export async function read(): Promise<Content | undefined> {
  ...
  if (os === "darwin") {
    const tmpfile = path.join(tmpdir(), "zavorth-clipboard.png")
    try {
      await Process.run(
        [
          "osascript",
...
/**
 * Write text to the system clipboard (native + OSC-52).
 * Always runs `stripProtection` so soft-break markers (U+200B/etc.) and
 * OSC-8 wrappers from TUI display never land in editors.
 */
export async function copy(text: string): Promise<void> {
  const clean = stripProtection(text)
  writeOsc52(clean)
