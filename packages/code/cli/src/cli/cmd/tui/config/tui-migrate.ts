...
const TUI_SCHEMA_URL = "https://zavorth.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from zavorth.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const zavorth = await zavorthFiles(input)
  for (const file of zavorth) {
...
async function zavorthFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "zavorth"),
    ...(await Filesystem.findUp(["zavorth.json", "zavorth.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zavorth"))
  }
  if (Flag.zavorth_CONFIG) files.push(Flag.zavorth_CONFIG)
