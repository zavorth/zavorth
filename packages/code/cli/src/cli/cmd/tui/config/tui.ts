...
export class Service extends Context.Service<Service, Interface>()("@zavorth/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.zavorth`
  // folders between cwd and home, and zavorth_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.zavorth_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit zavorth_TUI_CONFIG override, if set.
  if (Flag.zavorth_TUI_CONFIG) {
    const configFile = Flag.zavorth_TUI_CONFIG
  ...
  // 4. `.zavorth` directories (and zavorth_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".zavorth") || dir === Flag.zavorth_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".zavorth") && dir !== Flag.zavorth_CONFIG_DIR) continue
