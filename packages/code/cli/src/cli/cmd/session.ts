...
function pagerCmd(): string[] {
  ...
  if (Flag.zavorth_GIT_BASH_PATH) {
    const less = path.join(Flag.zavorth_GIT_BASH_PATH, "..", "..", "usr", "bin", "less.exe")
...
export const SessionImportClaudeCommand = cmd({
  ...
  describe: "import external agent sessions (~/.claude/projects) into zavorth",
  builder: (yargs: Argv) =>
    yargs.option("force", {
  ...
  handler: async (args) => {
    ...
    UI.println(
      `External session import: scanned ${stats.scanned}, imported ${stats.imported}, resynced ${stats.resynced}, skipped ${stats.skipped}` +
