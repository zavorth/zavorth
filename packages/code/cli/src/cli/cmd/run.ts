...
import { createZavorthClient, type ZavorthClient, type ToolPart } from "@zavorth/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run zavorth with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running zavorth server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to zavorth_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: ZavorthClient) {
    ...
    async function share(sdk: ZavorthClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.zavorth_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: ZavorthClient) {
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.zavorth_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.zavorth_SERVER_USERNAME ?? "zavorth"
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createZavorthClient({ baseUrl: args.attach, directory, headers })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createZavorthClient({ baseUrl: "http://zavorth.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
