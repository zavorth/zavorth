...
const AGENT_USERNAME = "zavorth-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/zavorth.yml"
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://zavorth.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
...
jobs:
  zavorth:
    if: |
      ...
      contains(github.event.comment.body, ' /zavorth') ||
      startsWith(github.event.comment.body, '/zavorth')
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.zavorth.dev" : "https://zavorth.dev"
      ...
      try {
        ...
        console.log("zavorth session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.zavorth.dev"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/zavorth,/zv")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to zavorth...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("zavorth-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `zavorth/${type}-${hex}-${timestamp}`
        }
        return `zavorth/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the zavorth infrastructure after your response",
