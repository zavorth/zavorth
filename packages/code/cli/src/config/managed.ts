...
const MANAGED_PLIST_DOMAIN = "dev.zavorth.managed"

// Keys injected by macOS/MDM into the managed plist that are not Zavorth config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/zavorth"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "zavorth")
    default:
      return "/etc/zavorth"
  }
}
...
export function managedConfigDir() {
  return process.env.zavorth_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
