...
const ZAVORTH_GATEWAY_PROVIDERS = new Set(["xiaomi", "zavorth"])

// zavorth gateway error.code values worth relabeling: moderation (421) and
...
function message(providerID: ProviderID, e: APICallError) {
  return iife(() => {
    // zavorth gateway: relabel known block codes and surface error.param (the real
    ...
    const gw = ZAVORTH_GATEWAY_PROVIDERS.has(providerID) ? json(e.responseBody)?.error : undefined
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `zavorth auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
