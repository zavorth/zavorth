...
import { Npm } from "../npm"
import { Hash } from "@zavorth/shared/util/hash"
...
// Tuned for zavorth-v2.5-pro on zavorth Router whose cold-path TTFT after context
...
// per-provider via zavorth.json's `chunkTimeout` config for tighter or looser
// bounds.
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    zavorth: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["zavorth"]?.options?.apiKey)

      // Keep free/public tier (zavorth-auto, cost.input === 0) always selectable.
      // Paid models still require auth/key — without credentials only free remains.
      for (const [key, value] of Object.entries(input.models)) {
        const free = value.cost.input === 0
        if (!ok && !free) delete input.models[key]
      }

      // When monorepo-hosted, point OpenAI-compatible base at monorepo ai-gateway
      // unless config/auth already set baseURL. Standalone Code is unchanged.
      // Only inject baseURL — do not re-spread catalog/config options here.
      const existingBase =
        typeof input.options?.baseURL === "string" && input.options.baseURL.trim() !== ""
          ? input.options.baseURL
          : null
      const monorepoBase = resolveOpenAiCompatibleBaseUrl({
        existingBaseUrl: null,
        env: env as NodeJS.ProcessEnv,
        providerID: "zavorth",
      })
      const options =
        monorepoBase && !existingBase ? { baseURL: monorepoBase } : ({} as Record<string, string>)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from zavorth.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://zavorth.dev/coder/",
            "X-Title": "zavorth",
            "X-Source": "zavorth",
          },
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `zavorth/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `zavorth/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `zavorth auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `zavorth/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "zavorth",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@zavorth/Provider") {}
...
/**
 * Temporary free Auto channel.
 *
 * models.dev currently has no `zavorth` provider entry (only `opencode` free
 * MiMo tiers + `xiaomi` paid). Until Zavorth ships its own free catalog entry,
 * seed `zavorth/zavorth-auto` from the public MiMo V2.5 free tier so the picker
 * always exposes Auto/free under the Zavorth brand.
 */
function ensureZavorthFreeAuto(database: Record<string, Info>) {
  const providerID = ProviderID.make("zavorth")
  const modelID = ModelID.make("zavorth-auto")
  const existing = database[providerID]?.models[modelID]
  if (existing && existing.status !== "deprecated") return
  ...
  const auto: Model = freeSrc
    ? {
        ...
        id: modelID,
        providerID,
        name: "zavorth Auto (MiMo V2.5 free)",
        family: "zavorth-auto",
        status: "active",
        cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
      }
  ...
  if (!database[providerID]) {
    database[providerID] = {
      ...
      name: "Zavorth",
      source: "custom",
      env: ["ZAVORTH_API_KEY", "OPENCODE_API_KEY"],
      options: {},
      models: { [modelID]: auto },
    }
    return
  ...
  database[providerID].models[modelID] = auto
}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        ensureZavorthFreeAuto(database)
                ...
                context: (() => {
                  ...
                  if (!warnedContextDefaults.has(key)) {
                    ...
                    log.warn("limit.context not configured and not found in models.dev", {
                      ...
                      fix: `Set limit.context explicitly in zavorth.json under provider.${providerID}.models.${modelID}`,
                    })
                  }
        ...
        // load env (skipped in zavorth-only mode so ANTHROPIC_API_KEY etc. don't auto-light other providers)
        if (!Flag.zavorth_DISABLE_PROVIDER_ENV) {
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.zavorth_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        const baseURL = iife(() => {
          ...
          let url = explicitBase ?? model.api.url

          // Product host: allowlisted providers without explicit baseURL may use product gateway.
          // - openai-compatible packages: …/v1 (zavorth always; others via ZAVORTH_ROUTE_PROVIDERS)
          // - @ai-sdk/anthropic: automatic when product-hosted → gateway …/v1 (POST /v1/messages);
          //   opt-out with ZAVORTH_ROUTE_ANTHROPIC=0 or ZAVORTH_ANTHROPIC_DIRECT=1
          if (!explicitBase) {
            const pid = String(model.providerID)
            if (model.api.npm.includes("@ai-sdk/openai-compatible")) {
              const productUrl = resolveOpenAiCompatibleBaseUrl({
                existingBaseUrl: null,
                env: envs as NodeJS.ProcessEnv,
                providerID: pid,
              })
              if (productUrl) url = productUrl
            } else if (model.api.npm.includes("@ai-sdk/anthropic")) {
              const productUrl = resolveAnthropicCompatibleBaseUrl({
                existingBaseUrl: null,
                env: envs as NodeJS.ProcessEnv,
                providerID: pid,
              })
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const zavorth = s.providers[ProviderID.make("zavorth")]
      if (zavorth?.models[ModelID.make("zavorth-auto")]) {
        return { providerID: zavorth.id, modelID: ModelID.make("zavorth-auto") }
      }
