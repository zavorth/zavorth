...
import type { AssistantMessage, Event, ZavorthClient, SessionMessageResponse, ToolPart } from "@zavorth/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: ZavorthClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `zavorth auth login` in the terminal",
      name: "Login with zavorth",
      id: "zavorth-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Zavorth stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const zavorthProvider = providers.find((p) => p.id === "zavorth")
  if (zavorthProvider) {
    if (zavorthProvider.models["big-pickle"]) {
      return { providerID: ProviderID.zavorth, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(zavorthProvider.models))
    if (best) {
      return {
