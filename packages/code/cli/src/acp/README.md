...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for Zavorth.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal zavorth sessions
...
zavorth acp --cwd /path/to/project
```
...
```bash
zavorth_ENABLE_QUESTION_TOOL=1 zavorth acp
```
...
- **Terminal Integration**: Full terminal support via zavorth's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | zavorth acp
```
...
ACP sessions map cleanly to zavorth's internal session model:
