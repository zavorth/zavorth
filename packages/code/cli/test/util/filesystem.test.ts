...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "zavorth.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "zavorth.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zavorth.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zavorth.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["zavorth.json", "zavorth.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "zavorth.json"),
        path.join(tmp.path, "zavorth.jsonc"),
        path.join(project, "zavorth.json"),
        path.join(project, "zavorth.jsonc"),
      ])
    })
  })
