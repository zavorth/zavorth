...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under zavorth.", () => {
    expect(paramToAttributeKey("name")).toBe("zavorth.name")
    expect(paramToAttributeKey("slug")).toBe("zavorth.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["zavorth.sessionID"]).toBeUndefined()
    expect(attrs["zavorth.messageID"]).toBeUndefined()
    expect(attrs["zavorth.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under zavorth. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["zavorth.name"]).toBe("exa")
