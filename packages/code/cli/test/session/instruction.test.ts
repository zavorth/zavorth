...
describe("Instruction.systemPaths zavorth_CONFIG_DIR", () => {
  let originalConfigDir: string | undefined

  beforeEach(() => {
    originalConfigDir = process.env["zavorth_CONFIG_DIR"]
  })
  ...
  test("prefers zavorth_CONFIG_DIR AGENTS.md over global when both exist", async () => {
    ...
    process.env["zavorth_CONFIG_DIR"] = profileTmp.path
    const originalGlobalConfig = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
  ...
  test("falls back to global AGENTS.md when zavorth_CONFIG_DIR has no AGENTS.md", async () => {
    await using profileTmp = await tmpdir()
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
  ...
  test("uses global AGENTS.md when zavorth_CONFIG_DIR is not set", async () => {
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
      },
