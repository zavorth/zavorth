...
afterEach(async () => {
  delete process.env.zavorth_CONFIG
  delete process.env.zavorth_TUI_CONFIG
  await fs.rm(path.join(Global.Path.config, "zavorth.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "zavorth.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const local = path.join(dir, ".zavorth")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "zavorth.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, ".zavorth", "tui.json"),
...
test("migrates tui-specific keys from zavorth.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "zavorth.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "zavorth.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when zavorth.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zavorth.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "zavorth.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "zavorth.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zavorth.json.tui-migration.bak"))).toBe(false)
})
...
test.skip("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "zavorth.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "zavorth.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "zavorth.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple zavorth.json levels", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "zavorth.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "zavorth.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over zavorth_TUI_CONFIG (matches zavorth_CONFIG)", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      ...
      process.env.zavorth_TUI_CONFIG = custom
    },
  })
...
test("zavorth_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from zavorth_CONFIG", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "zavorth.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.zavorth_CONFIG = path.join(customDir, "zavorth.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .zavorth/tui.json", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".zavorth"), { recursive: true })
      await Bun.write(path.join(dir, ".zavorth", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
