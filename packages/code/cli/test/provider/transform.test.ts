...
describe("ProviderTransform.maxOutputTokens", () => {
  ...
  test("uses 128K for zavorth provider models", () => {
    expect(
      ProviderTransform.maxOutputTokens({
        ...baseModel,
        id: ModelID.make("zavorth-auto"),
        providerID: ProviderID.make("zavorth"),
      }),
    ).toBe(128_000)
  })

  test("uses 128K for zavorth cloud gateway (id xiaomi) models", () => {
    expect(
      ProviderTransform.maxOutputTokens({
        ...baseModel,
        id: ModelID.make("zavorthr"),
        providerID: ProviderID.make("xiaomi"),
      }),
    ).toBe(128_000)
  })

  test("keeps the default cap for non-zavorth models", () => {
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, zavorthModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.zavorth?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.zavorth?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              zavorth: { itemId: "msg_zavorth_part" },
    ...
    expect(result[0].providerOptions?.zavorth?.itemId).toBe("msg_zavorth")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.zavorth?.itemId).toBe("msg_zavorth_part")
...
describe("ProviderTransform.message - cache control on gateway", () => {
  ...
  test("openai-compatible with claude in model id does NOT trigger caching", () => {
    const model = createModel({
      id: "customrouter/claude-opus-4-8",
      providerID: "custom",
      api: {
        id: "customrouter/claude-opus-4-8",
