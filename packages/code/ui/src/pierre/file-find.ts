...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("zavorth-find")
  api.delete("zavorth-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("zavorth-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("zavorth-find", new Highlight(...rest))
    return true
  }
